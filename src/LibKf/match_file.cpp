...
const string kpmodestr[] = {
		"SPP", "TDCP", "DGPS", 
		"PPK-KINE", "PPK-STATIC", "PPK-S-START", "PPK-MOVEB", "PPK-FIXED", 
...
extern int loadprcfiles(const char *dir, const prcopt_t *popt, filopt_t *fopt, sta_t *sta, int *nsta) {
	...
	if (!no_gnss && popt->sateph == EPHOPT_PREC) {
		for (int i = 0; i < 3; i++) {
			...
			if (!(fopt->obxf[i] = (char*)malloc(MAXSTRPATH)))
				for (i--; i >= 0; i--) free(fopt->obxf[i]);
			else fopt->obxf[i][0] = '\0';
		}

		if (!match_sp3file(popt, ts, te, dir, fopt->sp3f)) return 0;
		if (!match_clkfile(popt, ts, te, dir, fopt->clkf)) return 0;
		if (popt->posopt[6]&&!match_obxfile(popt, ts, te, dir, fopt->obxf)) return 0;
	}
	if (!no_gnss && popt->refpos == POSOPT_FILE) {
		if (!match_snxfile(popt, ts, te, dir, fopt->stapos)) return 0; ///should to match p1c1 dcb file
	}
	...
	//else if (!no_gnss && (popt->cbiaopt == CBIAS_OPT_CNT_BIA)) {
	//	if (!match_biafile(popt, ts, te, dir, fopt->dcb)) return 0;
	//}
	...
	if (!no_gnss && ppp) {
		...
		if (!match_rcvatxfile(ts, te, dir, fopt->rcvantp, popt))
		{
			showmsg("error : no rcv ant pcv in %s", fopt->rcvantp);
		}
	...
	if (ins) {
		if (!match_imufile(popt, popt->ts, popt->prcdir, fopt->imuf, fopt->imupf)) return 0;
		if (popt->mode == PMODE_LC_POS) {
			if (!match_lcfile(popt, popt->ts, dir, fopt->posf)) return 0;
		}
	}
	return 1;
}
...
extern int match_imufile(const prcopt_t* opt, gtime_t ts, const char* prcdir, char* imupath, char* imuppath) {
	DIR* dir;
	struct dirent* file;
	char* ext;
	int ok = 0, doy, yyyy;
	...
	char path[MAXSTRPATH] = { 0 };
	if (opt->obsdir[0] != '\0')
		sprintf(path, "%s%c%4d%c%03d%c%s", prcdir, FILEPATHSEP, yyyy, FILEPATHSEP, doy, FILEPATHSEP, opt->obsdir);
	else {
		sprintf(path, "%s%c%4d%c%03d%cobs", prcdir, FILEPATHSEP, yyyy, FILEPATHSEP, doy, FILEPATHSEP);
	}


	if (!(dir = opendir(path))) {
		fprintf(stderr, "No imu file\n");
		fflush(stderr);
		return 0;
	}
	...
	while ((file = readdir(dir)) != nullptr) {
		...
		if (!strcmp(ext + 1, "imu")) {
			sprintf(imupath, "%s%c%s", path, FILEPATHSEP, file->d_name);
			ok++;
			continue;
		}
		else if (!strcmp(ext + 1, "imup")) {
			sprintf(imuppath, "%s%c%s", path, FILEPATHSEP, file->d_name);
			ok++;
			continue;
		}
		else continue;
...
extern int match_navfile(const prcopt_t *popt, gtime_t ts, gtime_t te, const char prc_dir[], char *navpaths[]) {
	...
	int tttt = _access(tmp, 0);
	if (_access(tmp, 0) != -1) setstr(navpaths[1], tmp, MAXSTRPATH);
	else {
		...
		if (access(tmp, 0) != -1) setstr(navpaths[1], tmp, MAXSTRPATH);
			...
			if (access(tmp, 0) != -1) setstr(navpaths[1], tmp, MAXSTRPATH);
				...
				if (access(tmp, 0) != -1) setstr(navpaths[1], tmp, MAXSTRPATH);
					...
					trace(0, "Miss navigation file: %s\n", tmp);
					return 0;
				}
			}
		}
...
extern int match_obxfile(const prcopt_t* popt, gtime_t ts, gtime_t te, const char prc_dir[], char* obxpaths[]) {
	char dir[1024];
	char sep = (char)FILEPATHSEP;
	...
	if (popt->aclong) {
		if (popt->arprod == AR_PROD_OSB_WHU && !strcmp(popt->ac_name, "wum")) {
			sprintf(file_name, "%3s0MGXFIN_%04d%03d0000_01D_30S_ATT.OBX", "WUM", yyyy1, doy1);
		}
		else if (popt->arprod == AR_PROD_OSB_COM && !strcmp(popt->ac_name, "cod")) {
			sprintf(file_name, "%3s0MGXFIN_%04d%03d0000_01D_15M_ATT.OBX", "COD", yyyy1, doy1);
		}
		else if (popt->arprod == AR_PROD_OSB_GRM && !strcmp(popt->ac_name, "gbm")) {
			sprintf(file_name, "%3s0MGXRAP_%04d%03d0000_01D_30S_ATT.OBX", "GBM", yyyy1, doy1);
		}
		else if(!strcmp(popt->ac_name, "cnt")) {
			sprintf(file_name, "%s%04d%d.%s", "cnt", wwww1, dow1, "obx");
		}

	}
	else {
		if (popt->arprod == AR_PROD_OSB_WHU && !strcmp(popt->ac_name, "wum")) {
			sprintf(file_name, "%s%04d%d.%s", "wum", wwww1, dow1, "obx");
		}
		else {
			if (strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", popt->ac_name, wwww1, dow1, "obx");
			//if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "COM", wwww1, dow1, "CLK");
			if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "com", wwww1, dow1, "obx");
		}
	}
	sprintf(tmp, "%s%c%s", dir, sep, file_name);
	if (access(tmp, 0) != -1) setstr(obxpaths[0], tmp, MAXSTRPATH);

	/* current day */
	sprintf(dir, "%s%c%4d%c%03d%cproducts%c%s", prc_dir, sep, yyyy2, sep, doy2, sep, sep, popt->ac_name);
	if (popt->aclong) {
		if (popt->arprod == AR_PROD_OSB_WHU && !strcmp(popt->ac_name, "wum")) {
			sprintf(file_name, "%3s0MGXFIN_%04d%03d0000_01D_30S_ATT.OBX", "WUM", yyyy2, doy2);
		}
		else if (popt->arprod == AR_PROD_OSB_COM && !strcmp(popt->ac_name, "cod")) {
			sprintf(file_name, "%3s0MGXFIN_%04d%03d0000_01D_15M_ATT.OBX", "COD", yyyy2, doy2);
		}
		else if (popt->arprod == AR_PROD_OSB_GRM && !strcmp(popt->ac_name, "gbm")) {
			sprintf(file_name, "%3s0MGXRAP_%04d%03d0000_01D_30S_ATT.OBX", "GBM", yyyy2, doy2);
		}
		else if (!strcmp(popt->ac_name, "cnt")) {
			sprintf(file_name, "%s%04d%d.%s", "cnt", wwww2, dow2, "obx");
		}
	}
	else {
		if (popt->modear == ARMODE_PPPAR_ILS && popt->arprod == AR_PROD_OSB_WHU && !strcmp(popt->ac_name, "wum")) {
			sprintf(file_name, "%s%04d%d.%s", "wum", wwww2, dow2, "obx");
		}
		else {
			//sprintf(file_name, "%s%04d%d.%s", popt->ac_name, wwww2, dow2, "clk");
			if (strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", popt->ac_name, wwww2, dow2, "obx");
			//if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "COM", wwww2, dow2, "CLK");
			if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "com", wwww2, dow2, "obx");

		}
	}
	sprintf(tmp, "%s%c%s", dir, sep, file_name);

	if (access(tmp, 0) != -1) setstr(obxpaths[1], tmp, MAXSTRPATH);
	else {
		trace(0, "Miss precise obx file: %s\n", tmp);
		return 0;
	}
	...
	if (popt->aclong) {
		if (popt->arprod == AR_PROD_OSB_WHU && !strcmp(popt->ac_name, "wum")) {
			sprintf(file_name, "%3s0MGXFIN_%04d%03d0000_01D_30S_ATT.OBX", "WUM", yyyy3, doy3);
		}
		else if (popt->arprod == AR_PROD_OSB_COM && !strcmp(popt->ac_name, "cod")) {
			sprintf(file_name, "%3s0MGXFIN_%04d%03d0000_01D_15M_ATT.OBX", "COD", yyyy3, doy3);
		}
		else if (popt->arprod == AR_PROD_OSB_GRM && !strcmp(popt->ac_name, "gbm")) {
			sprintf(file_name, "%3s0MGXRAP_%04d%03d0000_01D_30S_ATT.OBX", "GBM", yyyy3, doy3);
		}
		else if (!strcmp(popt->ac_name, "cnt")) {
			sprintf(file_name, "%s%04d%d.%s", "cnt", wwww3, dow3, "obx");
		}
	}
	else {
		if (popt->modear == ARMODE_PPPAR_ILS && popt->arprod == AR_PROD_OSB_WHU && !strcmp(popt->ac_name, "obx")) {
			sprintf(file_name, "%s%04d%d.%s", "wum", wwww3, dow3, "obx");
		}
		else {
			//sprintf(file_name, "%s%04d%d.%s", popt->ac_name, wwww3, dow3, "clk");
			if (strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", popt->ac_name, wwww3, dow3, "obx");
			//if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "COM", wwww3, dow3, "CLK");
			if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "com", wwww3, dow3, "obx");

		}
	}
	sprintf(tmp, "%s%c%s", dir, sep, file_name);

	if (access(tmp, 0) != -1) setstr(obxpaths[2], tmp, MAXSTRPATH);
...
extern int match_clkfile(const prcopt_t *popt, gtime_t ts, gtime_t te, const char prc_dir[], char *clkpaths[]) {
	...
	if (popt->aclong) {
		...
		if (popt->arprod == AR_PROD_OSB_WHU && !strcmp(popt->ac_name, "wum")) {
			sprintf(file_name, "%s%04d%d.%s", "wum", wwww1, dow1, "clk");
		}
		else {
			if(strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", popt->ac_name, wwww1, dow1, "clk");
			//if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "COM", wwww1, dow1, "CLK");
			if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "com", wwww1, dow1, "clk");
		}
	}
	...
	if (popt->aclong) {
		...
		if (popt->modear == ARMODE_PPPAR_ILS && popt->arprod == AR_PROD_OSB_WHU && !strcmp(popt->ac_name, "wum")) {
			sprintf(file_name, "%s%04d%d.%s", "wum", wwww2, dow2, "clk");
		}
		else {
			//sprintf(file_name, "%s%04d%d.%s", popt->ac_name, wwww2, dow2, "clk");
			if (strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", popt->ac_name, wwww2, dow2, "clk");
			//if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "COM", wwww2, dow2, "CLK");
			if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "com", wwww2, dow2, "clk");
	...
	if (access(tmp, 0) != -1) setstr(clkpaths[1], tmp, MAXSTRPATH);
		...
		trace(0, "Miss precise clock file: %s\n", tmp);
		return 0;
	}
	...
	if (popt->aclong) {
		...
		if (popt->modear == ARMODE_PPPAR_ILS && popt->arprod == AR_PROD_OSB_WHU && !strcmp(popt->ac_name, "wum")) {
			sprintf(file_name, "%s%04d%d.%s", "wum", wwww3, dow3, "clk");
		}
		else {
			//sprintf(file_name, "%s%04d%d.%s", popt->ac_name, wwww3, dow3, "clk");
			if (strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", popt->ac_name, wwww3, dow3, "clk");
			//if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "COM", wwww3, dow3, "CLK");
			if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "com", wwww3, dow3, "clk");
...
extern int match_sp3file(const prcopt_t *popt, gtime_t ts, gtime_t te, const char prc_dir[], char *sp3paths[]) {
	...
	if (popt->aclong) {
		...
		if (strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", popt->ac_name, wwww1, dow1, "sp3");
		//if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "COM", wwww1, dow1, "EPH");
		if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "com", wwww1, dow1, "sp3");
	}
	sprintf(tmp, "%s%c%s", dir, sep, file_name);
	...
	if (popt->aclong) {
		...
		if (strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", popt->ac_name, wwww2, dow2, "sp3");
		//if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "COM", wwww2, dow2, "EPH");
		if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "com", wwww2, dow2, "sp3");
	}
	sprintf(tmp, "%s%c%s", dir, sep, file_name);

	if (access(tmp, 0) != -1) setstr(sp3paths[1], tmp, MAXSTRPATH);
	else {
		trace(0, "Miss precise orbit: %s\n", tmp);
		return 0;
	}
	...
	if (popt->aclong) {
		...
		if (strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", popt->ac_name, wwww3, dow3, "sp3");
		//if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "COM", wwww3, dow3, "EPH");
		if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "com", wwww3, dow3, "sp3");
	}
	sprintf(tmp, "%s%c%s", dir, sep, file_name);
...
extern int match_eopfile(gtime_t ts, gtime_t te, const char prc_dir[], char eoppath[], const prcopt_t *popt) {
	...
	if (popt->aclong) {
		...
		if (strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d7.erp", popt->ac_name, wwww);
		if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d7.erp", "com", wwww);
	}
	sprintf(tmp, "%s%c%s", dir, sep, file_name);
...
extern int match_cntdcbfile(const prcopt_t* popt, gtime_t ts, gtime_t te, const char prc_dir[], char dcbpath[]) {
	char sep = (char)FILEPATHSEP;
	double ep[6] = { 0 };
	time2epoch(ts, ep);
...
extern int match_snxfile(const prcopt_t *popt, gtime_t ts, gtime_t te, const char prc_dir[], char snxpath[]) {
	char sep = (char)FILEPATHSEP;
	double ep[6] = { 0 };
	...
	char dir[1024], file_name[1024] = { '\0' };
	sprintf(dir, "%s%c%4d%c%03d%cproducts%c%s", prc_dir, sep, yyyy2, sep, doy2, sep, sep, "snx");
	//sprintf(tmp, "%s%cigs.snx", dir, sep);
	if (!strcmp(popt->ac_name, "com"))sprintf(file_name, "%s%04d%d.%s", "COD", wwww2, dow2, "SNX");
	else sprintf(file_name, "%s%04d%d.%s", "igs", wwww2, dow2, "snx");
	sprintf(tmp, "%s%c%s", dir, sep, file_name);
	if (access(tmp, 0) != -1) setstr(snxpath, tmp, MAXSTRPATH);
	else {
		sprintf(file_name, "%s%04d%d.%s", "igs", wwww2, dow2, "snx");
		sprintf(tmp, "%s%c%s", dir, sep, file_name);		
		if (access(tmp, 0) != -1) setstr(snxpath, tmp, MAXSTRPATH);
		else
		{
...
extern int match_ionfile(gtime_t ts, gtime_t te, const char *prc_dir, char *ionpath) {
	...
	char sep = (char)FILEPATHSEP;
	char dir[1024] = { '\0' };
	sprintf(dir, "%s%c%04d%c%03d%cproducts%cigs_ion", prc_dir, sep, yyyy, sep, doy, sep, sep);

	sprintf(tmp, "%s%cigsg%03d0.%02di", dir, sep, doy, yy);
...
extern int match_biafile(const prcopt_t *opt, gtime_t ts, gtime_t te, const char prc_dir[], char biapath[]) {
	...
	if (opt->arprod == AR_PROD_OSB_WHU && !strcmp(opt->ac_name, "wum")) {
		if (opt->aclong) {
			...
			sprintf(file_name, "wum%04d%d.bia", yyyy, wod);
		}
		...
		if (opt->aclong) {
			...
			sprintf(file_name, "COM%04d%d.BIA", week, wod);
		}
		sprintf(tmp, "%s%c%s%c%s", dir, sep, opt->ac_name, sep, file_name);
	}
	...
	else if (opt->arprod == AR_PROD_OSB_SGG && !strcmp(opt->ac_name, "wum")) {
		sprintf(file_name, "SGG%04d%d.BIA", week, wod);
		sprintf(tmp, "%s%c%s%c%s", dir, sep, opt->ac_name, sep, file_name);
	}
...
extern int match_lcfile(const prcopt_t* opt, gtime_t ts, const char* prcdir, char* gpspath) {
	DIR* dir;
	struct dirent* file;
	char* ext;
	int ok = 0, doy, yyyy;
	...
	if (!(dir = opendir(path))) {
		fprintf(stderr, "No imu loose combine file\n");
		fflush(stderr);
		return 0;
	}
	...
	while ((file = readdir(dir)) != nullptr) {
		...
		if (!strcmp(ext + 1, "pos")) {
			sprintf(gpspath, "%s%c%s", path, FILEPATHSEP, file->d_name);
			ok = 1;
			break;
		}
		else continue;
...
extern int match_atxfile(gtime_t ts, gtime_t te, const char prc_dir[], char atxPath[], const prcopt_t *popt) {
	double ct_atx_1[6] = { 2006, 11, 5, 0, 0, 0.00000000 };//05
	double ct_atx_2[6] = { 2011, 4, 17, 0, 0, 0.00000000 };//08
	double ct_atx_3[6] = { 2017, 1, 29, 0, 0, 0.00000000 };//14
	...
	if (access(tmp, 0) != -1) setstr(atxPath, tmp, MAXSTRPATH);
		...
		trace(0, "No found atx file!");
		return 0;
	}

	return 1;
}
extern int match_rcvatxfile(gtime_t ts, gtime_t te, const char prc_dir[], char rcvatxPath[], const prcopt_t* popt) {
	...
	sprintf(dir, "%s%cIGSNetwork", prc_dir, sep);
	sprintf(tmp, "%s%c%s", dir, sep,"IGSNetwork.json");
	if (access(tmp, 0) != -1) setstr(rcvatxPath, tmp, MAXSTRPATH);
	else {
		return 0;
	}
...
extern void matchout(const prcopt_t *popt, const char *prcdir, filopt_t *fopt, const solopt_t *sopt) {
	...
	if (!no_gnss) {
		...
		if (popt->sdopt) {
			sprintf(outname, "%s%s", outname, "_SD");
		}
		...
		if(popt->mode< PMODE_PPP_KINEMA){
			sprintf(fopt->solf, "%s%s.pos", outdir, outname);
		}
		else {
	...
	if (sopt->ambres && (popt->mode >= PMODE_PPP_KINEMA || popt->mode <= PMODE_PPP_FIXED) && (popt->modear == ARMODE_PPPAR || popt->modear == ARMODE_PPPAR_ILS)) {
		sprintf(fopt->wl_amb, "%s%s%c%s.wlamb", outdir, popt->ac_name, sep, outname);
		sprintf(fopt->nl_amb, "%s%s%c%s.nlamb", outdir, popt->ac_name, sep, outname);
		sprintf(fopt->lc_amb, "%s%s%c%s.lcamb", outdir, popt->ac_name, sep, outname);
	}
	if (sopt->ambres && (popt->mode >= PMODE_KINEMA && popt->mode <= PMODE_FIXED) && (popt->modear != ARMODE_OFF)) {
		sprintf(fopt->wl_amb, "%s%s.wlamb", outdir, outname);
		sprintf(fopt->nl_amb, "%s%s.nlamb", outdir, outname);
		sprintf(fopt->lc_amb, "%s%s.lcamb", outdir, outname);
	}
