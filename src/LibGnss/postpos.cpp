	...
	const solopt_t* sopt)
{
	...
	if (sopt->outhead) {
		if (!*sopt->prog) {
			fprintf(fp, "%s program   : iRL LNTU PPP AR ver.%s %s\n", COMMENTH, VER_RTKLIB, PATCH_LEVEL);
		}
		else {
...
static int inputobs(obsd_t* obs, int solq, const prcopt_t* popt)
{
	...
	fprintf(stderr, " processing : %s Q=%d\n\r", time_str(time, 0), solq);
	fflush(stderr);
//    printf(" processing : %s Q=%d\r", time_str(time, 0), solq);
	...
	rtk_t* rtk, int mode)
{
	...
	//	//for (i = 0; i < 3; i++) rtk->sol.rr[i] = /*stas[0].pos*/rtk->opt.rb[i];
	//}
		...
		if (mode == 0) { /* forward/backward */
			if (!solstatic) {
				...
				if (rtk->epoch /*% 10 == 1*/)
				{
					if (popt->mode < PMODE_PPP_KINEMA)
					{
						outsol(fp, &rtk->sol, rtk->opt.ru, sopt, &rtk->opt, NULL);
					}
					else {
						outsol(fp, &rtk->sol, rtk->rb, sopt, &rtk->opt, NULL);
					}
				}
				//outsol_ion(fptm, &rtk->sol, rtk->rb, sopt, &rtk->opt, NULL);
			}
...
{
	if (prcopt->sateph != EPHOPT_PREC)return;
	seph_t seph0 = { 0 };
	int i, ppp = 0;
	char* ext;
	...
	/* read precise ephemeris files */
	for (i = 0; i < 3; i++) {
		//if (strstr(fopt->sp3f[i], "%r") || strstr(fopt->sp3f[i], "%b")) continue;
		readsp3(fopt->sp3f[i], nav, 0);
	}
	/* read precise clock files */
	for (i = 0; i < 3; i++) {
		//if (strstr(fopt->clkf[i], "%r") || strstr(fopt->clkf[i], "%b")) continue;
		readrnxc(prcopt, fopt->clkf[i], nav);
	}
	/* read precise obx files */
	for (i = 0; i < 3; i++) {
		//if (strstr(fopt->clkf[i], "%r") || strstr(fopt->clkf[i], "%b")) continue;
		readobxf( fopt->obxf[i],/*nav->obx,*/ nav);
	}
...
extern int readobsnav_a(const filopt_t* fopt, gtime_t ts, gtime_t te, double ti, char** infile,
	const int* index, int n, prcopt_t* prcopt,
	obs_t* obs, nav_t* nav, sta_t* sta)
{
	int i, j, ind = 0, nobs = 0, rcv = 1;
	...
	//{
	//	if (!fopt->robsf) {
	//		fprintf(stderr, "error : no obs file!\r");
	//		fflush(stderr);
	//		return 0;
	//	}
	//	if (index[0] != ind) {
	...
	//		}
	//		rcv = 0;
	//		ind = index[0]; nobs = obs->n;
	//	}
	//	/* read rinex obs and nav file */
	//	if (readrnxt(prcopt, fopt->robsf, rcv, ts, te, ti, prcopt->rnxopt[rcv <= 1 ? 0 : 1], obs, nav,
	//		rcv <= 2 ? sta + rcv - 1 : NULL) < 0) {
	//		checkbrk("error : insufficient memory");
	//		trace(1, "insufficient memory\n");
	//		return 0;
	//	}
	//	if (prcopt->mode < PMODE_PPP_KINEMA)
	//	{
	//		if ((!fopt->bobsf))
	//		{
	//			fprintf(stderr, "error : no base file!\r");
	//			fflush(stderr);
	//			return 0;
	//		}
	//		if (index[1] != ind) {
	...
	//if (0)
	//{
	//	if (!fopt->navf[1]) { checkbrk("error : no brdc file!"); return 0; }
	//	for (i = 1; i < 4; i++) {
	//		if (checkbrk("")) return 0;
	//		if (!*(fopt->navf[i - 1]))continue;
	//		if (index[i] != ind) {
	//			if (obs->n > nobs) {
	//				rcv++;
	//			}
	...
	//
	//		/* read rinex obs and nav file */
	//		if (readrnxt(prcopt, fopt->navf[i-1], rcv, ts, te, ti, prcopt->rnxopt[rcv <= 1 ? 0 : 1], obs, nav,
	//			rcv <= 2 ? sta + rcv - 1 : NULL) < 0) {
	//			checkbrk("error : insufficient memory");
	//			trace(1, "insufficient memory\n");
	//			return 0;
...
extern int readobs_a(gtime_t ts, gtime_t te, double ti, char* infile,
	const int* index, int n, prcopt_t* prcopt, obs_t* obs, sta_t* sta)
{
	int i, j, ind = 0, nobs = 0, rcv = 1;
	...
	//	}
	//	/* read rinex obs and nav file */
	//	if (readrnxt(prcopt, infile[i], rcv, ts, te, ti, prcopt->rnxopt[rcv <= 1 ? 0 : 1], obs, NULL,
	//		rcv <= 2 ? sta + rcv - 1 : NULL) < 0) {
	//		checkbrk("error : insufficient memory");
	//		trace(1, "insufficient memory\n");
	//		return 0;
	...
	if (1)
	{
		...
		/* read rinex obs and nav file */
		if (readrnxt(prcopt, infile, rcv, ts, te, ti, prcopt->rnxopt[rcv <= 1 ? 0 : 1], obs, NULL,
			rcv <= 2 ? sta + rcv - 1 : NULL) < 0) {
			checkbrk("error : insufficient memory");
			trace(1, "insufficient memory\n");
			return 0;
...
extern void freeobsnav(obs_t* obs, nav_t* nav)
{
	...
	if (nav) {
		...
		if (nav->obx) {
			free(nav->obx); nav->nb = nav->nbmax = 0;
		}
	}
}
...
static int getstapos(const char* file, char* name, double* r)
{
	...
	while (!feof(fp)) {
		...
		if (inregion == 1) {
			strncpy(chtmp, &buff[14], 4); chtmp[4] = '\0';
			if (strcasecmp(chtmp, name) == 0 && strstr(buff, "STAX")) {
				r[0] = str2num(buff, 47, 21);
			}
			if (strcasecmp(chtmp, name) == 0 && strstr(buff, "STAY")) {
				r[1] = str2num(buff, 47, 21);
			}
			if (strcasecmp(chtmp, name) == 0 && strstr(buff, "STAZ")) {
				r[2] = str2num(buff, 47, 21);
				break;
			}
		}
	}
	if (r[0] * r[1] * r[2] != 0.0) r[3] = 2.0;   //来自snx文件

	fclose(fp);
	if (r[0] * r[1] * r[2] != 0.0) { r[3] = 2.0; return 1; }   //来自snx文件

	trace(1, "no station position: %s %s\n", name, file);
	return 0;
}
/* station position from CLK file ------------------------------------------------*/
static int getstapos_clk(const char* file, char* name, double* r)
{
	FILE* fp = NULL;
	char buff[2048], chtmp[5];
	int inregion = -1;
	...
	while (!feof(fp)) {
		fgets(buff, sizeof(buff), fp);
		if (strstr(buff, "# OF SOLN STA / TRF")) {
			inregion = 1;
		}
		if (strstr(buff, "# OF SOLN SATS")) {
			inregion = 0;
			break;
		}
		if (inregion == 1) {
			strncpy(chtmp, &buff[0], 4); chtmp[4] = '\0';
			if (strcasecmp(chtmp, name) == 0) {
				r[0] = str2num(buff, 24, 11) / 100;
				r[1] = str2num(buff, 36, 11) / 100;
				r[2] = str2num(buff, 48, 11) / 100;
				break;
			}
		}
	}
	...
	const sta_t* sta, const char* posfile)
{
	...
	for (i = 0; i < 3; i++)(rcvno == 1 ? opt->ru[i] : opt->rb[i]) = rr[i];
	return 1;
}
	...
	const filopt_t* fopt, nav_t* nav, pcvs_t* pcvs, pcvs_t* pcvr)
{
	...
	//if (*fopt->rcvantp && !(readpcv(fopt->atx, pcvr))) {
	...
	char** infile, const int* index, const int n, const char* outfile)
{
	...
	/* open debug trace */
	if (flag && sopt->trace <= 5 && sopt->trace >= 1) {
		if (*outfile) {
			strcpy(tracefile, outfile);
			strcat(tracefile, ".trace");

			//strcpy(outfiletm, outfile);
			//strcat(outfiletm, ".ion");
		}
		else {
	...
	/* read obs and nav data */
	if (!readobsnav_a(fopt, ts, te, ti, infile, index, n, &popt_, &obss, &navs, stas)) {
		/* free obs and nav data */
	...
	if (popt_.mode == PMODE_FIXED) {
		...
		//if (popt_.refpos == POSOPT_POS)
		//	for (int i = 0; i < 3; i++)
		//	{
		//		popt_.rb[i] = stas[1].pos[i];
		//		popt_.ru[i] = stas[0].pos[i];
		//	}
	}
	if (popt_.refpos == POSOPT_FILE && popt_.mode >= PMODE_FIXED) { /* read from position file */
		char* name = NULL;
		name = stas[0].name;
		//if (strlen(stas[0].name) > 4)for (int i = 4; i < strlen(stas[0].name); i++)stas[0].name[i] = "\0";
		//for (int i = 0; i < 4;i++)name[i] = name[i]+ stas[0].name[i];
		//setstr(name, stas[0].name, 4);
		if (!getstapos(fopt->stapos, name, popt_.rb)) {
			trace(0,"error : no position of %s in %s\nObs file Header position of %s  is used:X :%.4f   Y:%.4f   Z:%.4f\n", name, fopt->stapos, name, stas[0].pos[0], stas[0].pos[1], stas[0].pos[2]);
			for (int i = 0; i < 3; i++)
			{
				popt_.rb[i] = stas[0].pos[i];
			}
			trace(0, "error : no position of %s in %s\n next station process\n", name, fopt->stapos);

			return 0;
			/* read precise clock files */
			//for (int i = 0; i < n; i++) {
			//	if (strstr(infile[i], "%r") || strstr(infile[i], "%b")) continue;
			//	if (getstapos_clk(infile[i], name, popt_.rb))break;
			//}

			//return 0;
		}
		for (int i = 0; i < 3; i++)rtk.rb[i] = popt_.ru[i] = popt_.rb[i];
	}
	...
	if (sopt->ambres && (popt_.modear == ARMODE_PPPAR_ILS || popt_.modear == ARMODE_CONT)) {
	...
	if (popt->modear > ARMODE_OFF) {
		if (rtk.wl_fix_epoch[0]) {
			fprintf(stderr, "Total epoch: %d(100.0%s),GPS WL Fix epoch:>=4  %d(%3.1f%s);  >=5  %d(%3.1f%s);  >=6  %d(%3.1f%s)\n",
				rtk.epoch, "%", rtk.wl_fix_epoch[0], (rtk.wl_fix_epoch[0] + 1e-9) / rtk.epoch * 100.0, "%", rtk.wl_fix_epoch[1], (rtk.wl_fix_epoch[1] + 1e-9) / rtk.epoch * 100.0
				, "%", rtk.wl_fix_epoch[2], (rtk.wl_fix_epoch[2] + 1e-9) / rtk.epoch * 100.0, "%");
			fflush(stderr);
		}
		if (rtk.wl_fix_epoch[0 + 2 * 3]) {
			fprintf(stderr, "Total epoch: %d(100.0%s),Galileo WL Fix epoch:>=4  %d(%3.1f%s);  >=5  %d(%3.1f%s);  >=6  %d(%3.1f%s)\n",
				rtk.epoch, "%", rtk.wl_fix_epoch[0 + 2 * 3], (rtk.wl_fix_epoch[0 + 2 * 3] + 1e-9) / rtk.epoch * 100.0, "%", rtk.wl_fix_epoch[1 + 2 * 3], (rtk.wl_fix_epoch[1 + 2 * 3] + 1e-9) / rtk.epoch * 100.0
				, "%", rtk.wl_fix_epoch[2 + 2 * 3], (rtk.wl_fix_epoch[2 + 2 * 3] + 1e-9) / rtk.epoch * 100.0, "%");
			fflush(stderr);
		}
		if (rtk.wl_fix_epoch[0 + 3 * 3]) {
			fprintf(stderr, "Total epoch: %d(100.0%s),BDS WL Fix epoch:>=4  %d(%3.1f%s);  >=5  %d(%3.1f%s);  >=6  %d(%3.1f%s)\n",
				rtk.epoch, "%", rtk.wl_fix_epoch[0 + 3 * 3], (rtk.wl_fix_epoch[0 + 3 * 3] + 1e-9) / rtk.epoch * 100.0, "%", rtk.wl_fix_epoch[1 + 3 * 3], (rtk.wl_fix_epoch[1 + 3 * 3] + 1e-9) / rtk.epoch * 100.0
				, "%", rtk.wl_fix_epoch[2 + 3 * 3], (rtk.wl_fix_epoch[2 + 3 * 3] + 1e-9) / rtk.epoch * 100.0, "%");
			fflush(stderr);
		}
	}
