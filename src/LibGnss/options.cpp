...
static char snrmask_[(NFREQ + NEXOBS)][1024];
...
#define GLOIFCB "0:off,1:linear-function-of-frequency-number,2:quadratic-polynomial-function-of-frequency-number,3:isb+icb-every-sat,4:isb+icb-every-frq"
...
#define RESQCOPT   "0:off,1:pr_igg,2:cp_igg,3:igg,5:igg_shi"
...
#define QCPRIRES   "0:off,1:qc_pr,2:qc_cp,3:qc_npr,4:qc_ncp,5:qc_pri_res"
...
EXPORT opt_t sysopts[] = {
		...
		{"pos1-snrmask_L1/B1",  2,  (void*)snrmask_[0],         ""     },
		{"pos1-snrmask_L2/B2",  2,  (void*)snrmask_[1],         ""     },
		{"pos1-snrmask_L5/B2a", 2,  (void*)snrmask_[2],         ""     },
		{"pos1-snrmask_B3",  2,  (void*)snrmask_[3],         ""     },
		{"pos1-snrmask_B1C", 2,  (void*)snrmask_[4],         ""     },
		{"pos1-snrmask_B2b", 2,  (void*)snrmask_[5],         ""     },
		...
		{"pos1-gloicb",     3,  (void*)&prcopt_.gloicb,     GLOIFCB },
		...
		{"pos1-obx",        3,  (void*)&prcopt_.posopt[6],  SWTOPT },
		...
		//	    {"pos2-armode",     3,  (void *)&prcopt_.modear,     ARMOPT }, //ÃüÁîÐÐ¿ØÖÆ -A£¬
				...
				{"pos2-pos_res_qc", 3,  (void*)&prcopt_.pos_res_qc,  RESQCOPT },
				{"pos2-igg_pr_k0",  1,  (void*)&prcopt_.igg_pr_k0,  "" },
				{"pos2-igg_pr_k1",  1,  (void*)&prcopt_.igg_pr_k1,  "" },
				{"pos2-igg_cp_k0",  1,  (void*)&prcopt_.igg_cp_k0,  "" },
				{"pos2-igg_cp_k1",  1,  (void*)&prcopt_.igg_cp_k1,  "" },
				...
				{"out-trace",      0,  (void*)&solopt_.trace,      "" },
				...
				{"outstr1-type",    3,  (void*)&filopt_.strtype[6],  OSTOPT },
				{"outstr2-type",    3,  (void*)&filopt_.strtype[7],  OSTOPT },
				{"outstr3-type",    3,  (void*)&filopt_.strtype[8],  OSTOPT },
				{"outstr1-path",    2,  (void*)&filopt_.strpath[6],  ""     },
				{"outstr2-path",    2,  (void*)&filopt_.strpath[7],  "" },
				{"outstr3-path",    2,  (void*)&filopt_.strpath[8],  ""     },
				{"outstr1-format",  3,  (void*)&filopt_.strfmt[6],   SOLOPT },
				{"outstr2-format",  3,  (void*)&filopt_.strfmt[7],   SOLOPT },
				{"outstr3-format",  3,  (void*)&filopt_.strfmt[8],   SOLOPT },
...
EXPORT opt_t insopts[] = {
		...
		{"ins-pri_res_qc",3,  (void*)&prcopt_.insopt.pri_res_qc,   QCPRIRES},
		{"ins-stc_udpr",3,    (void*)&prcopt_.insopt.stc_udpr,   SWTOPT},
...
static void buff2sysopts(void)
{
	...
	if (GPS_frq_[0]) for (i = 0; i < NEXOBS + NFREQ; i++)if (prcopt_.gnss_frq_idx[0][i] != 0)prcopt_.nf_sys[0]++;

#ifdef ENAGLO
	if (GLO_frq_[0]) getobsfrqidx(GLO_frq_, SYS_GLO, prcopt_.nf, prcopt_.gnss_frq_idx[1]);
	if (GLO_frq_[0]) for (i = 0; i < NEXOBS + NFREQ; i++)if (prcopt_.gnss_frq_idx[1][i] != 0)prcopt_.nf_sys[1]++;

#endif
#ifdef ENAGAL
	if (GAL_frq_[0]) getobsfrqidx(GAL_frq_, SYS_GAL, prcopt_.nf, prcopt_.gnss_frq_idx[2]);
	if (GAL_frq_[0]) for (i = 0; i < NEXOBS + NFREQ; i++)if (prcopt_.gnss_frq_idx[2][i] != 0)prcopt_.nf_sys[2]++;
	...
	if (BD2_frq_[0]) for (i = 0; i < NEXOBS + NFREQ; i++)if (prcopt_.gnss_frq_idx[3][i] != 0)prcopt_.nf_sys[3]++;
	if (BD3_frq_[0]) for (i = 0; i < NEXOBS + NFREQ; i++)if (prcopt_.gnss_frq_idx[NSYS][i] != 0)prcopt_.nf_sys[NSYS]++;

#endif
#ifdef ENAQZS
	if (QZS_frq_[0]) getobsfrqidx(QZS_frq_, SYS_QZS, prcopt_.nf, prcopt_.gnss_frq_idx[4]);
	if (QZS_frq_[0]) for (i = 0; i < NEXOBS + NFREQ; i++)if (prcopt_.gnss_frq_idx[4][i] != 0)prcopt_.nf_sys[4]++;
