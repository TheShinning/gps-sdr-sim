...
const double tdistb_0050[30] = {    /*???t???????????0.05*/
...
const double tdistb_0025[30] = {    /*???t???????????0.025*/
		12.706, 4.303, 3.182, 2.776, 2.571, 2.447, 2.365, 2.306, 2.262,2.228,
		2.201, 2.179, 2.160, 2.145, 2.131, 2.120, 2.110, 2.101, 2.093, 2.086,
		2.080, 2.074, 2.069, 2.064, 2.060, 2.056, 2.052, 2.048, 2.045, 2.042
};

const double tdistb_0010[30] = {    /*???t???????????0.01*/
...
const double tdistb_0005[30] = {    /*???t???????????0.005*/
		63.657, 9.925, 5.841, 4.604, 4.032, 3.707, 3.499, 3.355, 3.250, 3.169,
		3.106, 3.055, 3.012, 2.977, 2.947, 2.921, 2.898, 2.878, 2.861, 2.845,
		2.831, 2.819, 2.807, 2.797, 2.787, 2.779, 2.771, 2.763, 2.756, 2.750
};

const double tdistb_0001[30] = {    /*???t???????????0.001*/
...
extern double grubbs[100] = {//0.05
	0,0,0,
	1.1546847100299753, 1.4962499999999703,
...
const prcopt_t prcopt_default = { /* defaults processing options */
	0,"","",0,{0},{0},"",0.0,
	PMODE_KINEMA,0,2,{0},SYS_GPS,   /* mode,soltype,nf,navsys */
...
static char codepris[8][MAXFREQ][16] = {  /* code priority for each freq-index */
		...
		{"PCABX"   ,"PCABX"     ,"IQX"     ,""       ,""       ,"ABX"   ,"ABX"}, /* GLO */
		{"CABXZ"   ,"IQX"       ,"IQX"     ,"ABCXZ"  ,"IQX"    ,""      ,""}, /* GAL */
		{"CLSXZ"   ,"LSX"       ,"IQXDPZ"  ,"LSXEZ"  ,""       ,""      ,""}, /* QZS */
		{"C"       ,"IQX"       ,""        ,""       ,""       ,""      ,""}, /* SBS */
		{"IQX"     ,"IQXDPZ"    ,""        ,"IQXDPZ" ,""       ,""      ,""}, /* BDS */
		{"ABCX"    ,"ABCX"      ,""        ,""       ,""       ,""      ,""} , /* IRN */
		{"IQX"     ,""          ,"DPX"     ,"IQXDPZ" ,"DPX"    ,"DPXSLZ" ,"DPZ"}, /* BDS3 */
};
...
static int obsfrqidx[8][MAXFREQ][1] = {
		...
		{0,1,2,3,4,5,6},                               /*IRN*/
		{0,1,2,3,4,2 + NFREQ,3 + NFREQ},                   /*BDS3*/
};
...
extern char* satno2sys(int sat)
{
	int prn;
	char* satprn = sat_id(sat);
	switch (satsys(sat, &prn)) {
...
static int code2freq_GLO(uint8_t code, int fcn, double* freq)
{
	...
	case '4': *freq = FREQ1a_GLO;              return 5;// 0; /* G1a */
	case '6': *freq = FREQ2a_GLO;              return 6;// 1; /* G2a */
	}
	return -1;
}
...
static int code2freq_BDS3(uint8_t code, double* freq)
{
	char* obs = code2obs(code);

	switch (obs[0]) {
	case '1': if (freq) *freq = FREQ1;     return 5; /* B1C */
	case '2': if (freq) *freq = FREQ1_CMP; return 0; /* B1I */
	case '7': if (freq) *freq = FREQ2_CMP; return 6; /* B2b */
	case '5': if (freq) *freq = FREQ5;     return 2; /* B2a */
	case '6': if (freq) *freq = FREQ3_CMP; return 3; /* B3 */
	case '8': if (freq) *freq = FREQ8;     return 4; /* B2ab */
	}
...
extern int code2idx(int sys, uint8_t code)
{
	...
	case SYS_BD3: return code2freq_BDS3(code, &freq);//BDS3
...
extern double code2freq(int sys, uint8_t code, int fcn)
{
	...
	case SYS_BD3: (void)code2freq_BDS3(code, &freq); break;//BDS3
...
extern double sat2freq(int sat, uint8_t code, const nav_t* nav)
{
	...
	sys = satsys(sat, &prn);
	if (sys == SYS_CMP && prn > 18)sys = SYS_BD3;
	if (sys == SYS_GLO) {
		if (!nav) return 0.0;
		for (i = 0; i < nav->ng; i++) {
...
extern void setcodepri(int sys, int freq, const char* pri)
{
	...
	if (sys & SYS_BD3) strcpy(codepris[7][freq - 1], pri);//BDS3
...
extern int getcodepri(int sys, uint8_t code, const char* opt)
{
	...
	case SYS_BD3: i = 7; optstr = "-CL%2s"; break;//BDS3
...
//extern int scalegetbitu(const unsigned char* buff, int pos, int len, int64_t a, int64_t b, int64_t c)
//{
//	unsigned int bits = getbitu(buff, pos, len);
//
//
...
extern int findmin(const double* d, int n, double* min)
{
	int i, min_idx = 0;
	*min = fabs(d[0]);
...
static int cmpres1(const void* p1, const void* p2)
{
...
extern int median(double* a, int n)
{
	...
	qsort(a, n, sizeof(double), cmpres1);
...
extern void matcpy_int(int* A, const int* B, int n, int m)
{
...
extern void mat_add(const int* int_part, const double* frac_part, double* full, int n)
{
	int i;
	for (i = 0; i < n; i++)
	{
...
extern void mat_block(const double* A, int n, int m, int row_start, int rows, int col_start, int cols, double* C)
{
	int i, j, k = 0, q = 0;
	if (A == NULL || C == NULL) {
		trace(2, "mat_block A==NULL || C==NULL\n");
		return;
	}
...
/* multiply matrix -----------------------------------------------------------*/
//C=alpha*B*A+beta*C；n代表第一个矩阵的行，k代表第二个矩阵的列，m代表第一个矩阵的列和第二个矩阵的行。
extern void matmul(const char* tr, int n, int k, int m, double alpha,
	const double* A, const double* B, double beta, double* C)
{
	double d;
...
extern double detmat(const double* A, int n)
{
	double s = 0, * B = NULL;
	if (n == 1) {
		return A[0];
		...
		for (int i = 0; i < n; i++) {
			...
			matprint(0, B, n - 1, n - 1, 15, 6);

			s = s + detmat(B, n - 1) * pow(-1, i) * A[i * n];
			free(B);
		}
		return s;
	}
...
extern void iimatmul(const char* tr, int n, int k, int m, double alpha,
...
extern int matinv(double* A, int n)
{
	...
	try { indx = imat(n, 1); }
		...
		fprintf(stderr, "matinv indx PROCESS ERROR!\n"); fflush(stderr);
	};
	try { B = mat(n, n); }
	catch (...) {
		fprintf(stderr, "matinv indx PROCESS ERROR!\n"); fflush(stderr);
...
{
	double B = pos[0], L = pos[1], H = pos[2];
	double sB = sin(B), cB = cos(B), sL = sin(L), cL = cos(L);
	double e2 = SQR(wgs84.e);
	double N = wgs84.R0 / sqrt(1.0 - e2 * SQR(sB));
	double NH = N + H;
...
extern int readantex(const char* file, pcvs_t* pcvs)
{
	FILE* fp = NULL;
	static const pcv_t pcv0 = { 0 };
	pcv_t pcv /*= (pcv_t *)calloc(1, sizeof(pcv_t))*/;
	double neu[3], dd;
...
extern double corrDCB(const prcopt_t* popt, const nav_t* nav, const double* cbias, uint8_t code, int frq, int sat)
{
	...
	if (sys == SYS_GPS) { /* broadcast and precise clock base on L1/L2 ionospheric-free combination*/
		...
		if (frq == 0) { /*L1*/
			...
			//if (code == CODE_L2C|| code == CODE_L2X|| code == CODE_L2L|| code == CODE_L2S)dcb += cbias[2];
		}
		else if (frq == 2) { /*L5*/
		...
		if ((popt->mode == PMODE_SINGLE || popt->sateph == EPHOPT_BRDC) && (popt->cbiaopt == CBIAS_OPT_IGG_DCB || popt->cbiaopt == CBIAS_OPT_GBM_DCB || popt->cbiaopt == CBIAS_OPT_MIX_DCB)) { /*broadcast correction */
			...
			if (0 && !strcmp(popt->ac_name, "com") && prn <= 18) { /*B1/B2 correction for BD2*/
				...
				if (prn <= 18) {
					if (frq == 0) { /*B2I 2IQX for BD2*/
						...
						if (popt->cbiaopt == CBIAS_OPT_MIX_DCB)
						{
							dcb = +alpha_12 * cbias[B2I7I];
						}
						else {
					...
					else if (frq == 2 + NFREQ) { /*B1C 1DPAN for BD3*/
					...
					else if (frq == 3 + NFREQ) { /*B2b 7DPZ for BD3*/
...
extern double corr_code_bias(const prcopt_t* popt, const nav_t* nav, const obsd_t* obs, int frq)
{
	...
	if (ppp && (popt->modear == ARMODE_PPPAR_ILS || popt->modear == ARMODE_PPPAR_HOLD) && (popt->arprod == AR_PROD_OSB_GRM || popt->arprod == AR_PROD_OSB_WHU || popt->arprod == AR_PROD_OSB_COM || popt->arprod == AR_PROD_OSB_CNT)) {
		isc = 0.0;
	}
	...
	//if (popt->sateph == EPHOPT_SSRAPC || popt->sateph == EPHOPT_SSRCOM)
	//{
	//	dcb += nav->ssr[obs->sat - 1].cbias[obs->code[frq] - 1];
	//}
	//
	//if (popt->sateph == EPHOPT_SSRAPC || popt->sateph == EPHOPT_SSRCOM) {
	...
	//	if (sys == SYS_GPS)
	//		ix = (frq == 0 ? CODE_L1W - 1 : CODE_L2W - 1);
	//	else if (sys == SYS_GLO)
	//		ix = (frq == 0 ? CODE_L1P - 1 : CODE_L2P - 1);
	//	else if (sys == SYS_GAL)
	//		ix = (frq == 0 ? CODE_L1X - 1 : CODE_L7X - 1);
	//	else if (sys == SYS_CMP)
	//		ix = (frq == 0 ?CODE_L2I  - 1 : CODE_L6I - 1);
	//	/* apply SSR correction */
	//	dcb += (nav->ssr[obs->sat - 1].cbias[obs->code[frq] - 1] - nav->ssr[obs->sat - 1].cbias[ix]);
	//}
	return isc + dcb;
}
...
{
	int sat, prn, f, isys = satsysidx(obs->sat);
	double cbias[NFREQ + NEXOBS] = { 0 }, frqs[NFREQ + NEXOBS] = { 0 }, alpha = 0.0, beta = 0.0/*,c_cbias[NFREQ+NEXOBS]={0}*/;
	...
	for (f = 0; f < popt->nf_sys[isys]; f++) {   /*UC model*/
	...
	if (popt->ionoopt == IONOOPT_IFLC || popt->ionoopt == IONOOPT_IF2) {
		if (popt->nf_sys[isys] == 1) {   /*UofC model*/
			if (Lc && P[frq_idxs[0] - 1] != 0.0 && L[frq_idxs[0] - 1] != 0.0) Lc[0] = 0.5 * P[frq_idxs[0] - 1] + 0.5 * L[frq_idxs[0] - 1];
		}
		else if (popt->nf_sys[isys] == 2) { /*dual-frequency ionospheric-free*/
		...
		else if (popt->nf_sys[isys] == 3) { /*triple-frequency ionospheric-free*/
	...
	double* Lc, double* Pc)
{
	...
	double C1, C2;
	int i, sys;
	for (i = 0; i < NFREQ + NEXOBS; i++)lam[i] = CLIGHT / sat2freq(obs->sat, obs->code[i], nav);
	...
	*Lc = *Pc = 0.0;
	...
	i = rtk->opt.gnss_frq_idx[sys_idx][2] - 1;//(sys & (SYS_GAL | SYS_SBS)) ? 2 : 1; /* L1/L2 or L1/L5 */
//
...
{
	double H[4 * MAXSAT], Q[16], cosel, sinel, al[MAXSAT] = { 0 }, quadrant[4] = { 0 };
	int i, n;
	...
	for (i = n = 0; i < ns && i < MAXSAT; i++) {
		...
		H[3 + 4 * n++] = 1.0;
		al[i] = azel[i * 2] * R2D;
		if (al[i] <= 90)quadrant[0]++;
		else if (al[i] <= 180 && al[i] > 90)quadrant[1]++;
		else if (al[i] <= 270 && al[i] > 180)quadrant[2]++;
		else if (al[i] <= 360 && al[i] > 270)quadrant[3]++;
	}
	if (n < 4) return;
...
static double tropmap_gmf(gtime_t tt, const double* pos, double elev, double* mapw)
{
	...
	if (mapw)*mapw = topcon / (sine + gamma);
...
extern void antmodel_s(const pcv_t* pcv, double nadir, double* dant)
{
	...
	for (i = 0; i < NFREQ; i++) {
		if (sys == SYS_GPS) {
			...
			if (i == 2)dant[i] = interpvar0(18, nadir * R2D * 5.0, pcv->var[1]);
		}
		else if (sys == SYS_GLO) {
			dant[i] = interpvar0(18, nadir * R2D * 5.0, pcv->var[i + NFREQ]);
			if (i == 2)dant[i] = interpvar0(18, nadir * R2D * 5.0, pcv->var[1 + NFREQ]);
		}
		else if (sys == SYS_GAL) {
			dant[i] = interpvar0(18, nadir * R2D * 5.0, pcv->var[i + 2 * NFREQ]);
			if (i == 2)dant[i] = interpvar0(18, nadir * R2D * 5.0, pcv->var[1 + 2 * NFREQ]);
		}
		else if (sys == SYS_CMP) {
			dant[i] = interpvar0(18, nadir * R2D * 5.0, pcv->var[i + 3 * NFREQ]);
			if (i == 3)dant[i] = interpvar0(18, nadir * R2D * 5.0, pcv->var[1 + 3 * NFREQ]);
		}
		else if (sys == SYS_QZS) {
			dant[i] = interpvar0(18, nadir * R2D * 5.0, pcv->var[i + 4 * NFREQ]);
			if (i == 2)dant[i] = interpvar0(18, nadir * R2D * 5.0, pcv->var[1 + 4 * NFREQ]);
		}
	}
...
{
	double Ps[2][MAXSAT][(NFREQ + NEXOBS)] = { {{0}} }, Lp[2][MAXSAT][(NFREQ + NEXOBS)] = { {{0}} }, dcp, lam;
	int i, j, s, r, n[2][MAXSAT][(NFREQ + NEXOBS)] = { {{0}} };
	obsd_t* p;
