...
static void writesol(rtksvr_t* svr, int index)
{
	...
	for (i = 0; i < 2; i++) {
		...
		//svr->solopt[i].posf =svr->solopt[i].sol_frm[i];
		if (svr->solopt[i].posf == SOLF_STAT) {
		...
		strwrite(svr->stream + i + 6, buff, n);
...
static int outssr_code_bias(uint8_t* buff, rtksvr_t* svr, int index, nav_t* nav0, obs_t* obs0, ssr_t* ssr)
{
	const char* msm_sig_gps[32] = {
		/* GPS: ref [17] table 3.5-91 */
		""  ,"1C","1P","1W",""  ,""  ,""  ,"1S","1L","","" ,""  , /*  1-12 */
		"1X"  ,"1Z"  ,"2C","2D","2S","2L"  ,"2X"  ,"2P"  ,"2W"  ,"","","", /* 13-24 */
		"5I"  ,"5Q"  ,"5X"  ,""  ,""  ,"","",""                      /* 25-32 */
	};
	...
	const char* msm_sig_gal[68] = {
		/* BeiDou: ref [17] table 3.5-108 */
		"","1C","","","","","","","","","1A","1B"  ,/*1-12*/
		"","","","","","","","","","","",""  ,/*13-24*/
		"5I","","","7I","7Q","","6A","6B","6C","","",""  ,/*25-36*/
		"5Q","8I","8Q","","","","","","","","",""  ,/*37-48*/
		"","","","","","","","","","","",""  ,/*49-60*/
		"","","","","","","",""               /*61-68*/
	};
	...
	solopt_t solopt = solopt_default;
	int i = 0, j = 0, prn = 0, sys = 0, nbuff = 0, nsat = nav0->nssr[0];
	char* p = (char*)buff;

	char s[64];
	tracet(4, "writessr: index=%d\n", index);
	if (obs0->data[0].P[0])time2str(obs0->data[0].time, s, NULL);
	p += sprintf(p, "> CODE_BIAS %s\n", s);
	//p += sprintf(p, "%s", s);
	for (i = 0; i < MAXSAT; i++)
	{
		if (!ssr[i].ncbias)continue;

		sys = satsys(i + 1, &prn);

		if (sys == SYS_GPS)
		{
			p += sprintf(p, "G%.2d", prn);
			for (j = 0; j < 32; j++)
			{
				if (msm_sig_gps[ssr_sig_gps[j]] == "" || !ssr[i].cbias[ssr_sig_gps[j] - 1])continue;
				p += sprintf(p, "  %4s  %8.4f", msm_sig_gps[ssr_sig_gps[j]], ssr[i].cbias[ssr_sig_gps[j] - 1]);
			}
			p += sprintf(p, "\n");
		}
		if (sys == SYS_GLO)
		{
			p += sprintf(p, "R%.2d", prn);
			for (j = 0; j < 32; j++)
			{
				if (msm_sig_glo[ssr_sig_glo[j]] == "" || !ssr[i].cbias[ssr_sig_glo[j] - 1])continue;
				p += sprintf(p, "  %4s  %8.4f", msm_sig_glo[ssr_sig_glo[j]], ssr[i].cbias[ssr_sig_glo[j] - 1]);
			}
			p += sprintf(p, "\n");
		}
		if (sys == SYS_GAL)
		{
			p += sprintf(p, "E%.2d", prn);
			for (j = 0; j < 32; j++)
			{
				if (msm_sig_gal[ssr_sig_gal[j]] == "" || !ssr[i].cbias[ssr_sig_gal[j] - 1])continue;
				p += sprintf(p, "  %4s  %8.4f", msm_sig_gal[ssr_sig_gal[j]], ssr[i].cbias[ssr_sig_gal[j] - 1]);
			}
			p += sprintf(p, "\n");
		}
		if (sys == SYS_CMP)
		{
			p += sprintf(p, "C%.2d", prn);
			for (j = 0; j < 32; j++)
			{
				if (msm_sig_cmp[ssr_sig_cmp[j]] == "" || !ssr[i].cbias[ssr_sig_cmp[j] - 1])continue;
				p += sprintf(p, "  %4s  %8.4f", msm_sig_cmp[ssr_sig_cmp[j]], ssr[i].cbias[ssr_sig_cmp[j] - 1]);
			}
			p += sprintf(p, "\n");
		}
	}
	//p += sprintf(p, "\n");
	return p - (char*)buff;
	//strwrite(svr->stream + index, buff, nbuff);
	//saveoutbuf(svr, buff, 1024, index);
}
static int outssr_phase_bias(uint8_t* buff, rtksvr_t* svr, int index, nav_t* nav0, obs_t* obs0, ssr_t* ssr)
{
	const char* msm_sig_gps[32] = {
		/* GPS: ref [17] table 3.5-91 */
		""  ,"1C","1P","1W",""  ,""  ,""  ,"1S","1L","","" ,""  , /*  1-12 */
	...
	solopt_t solopt = solopt_default;
	int i = 0, j = 0, prn = 0, sys = 0, nbuff = 0, nsat = nav0->nssr[1];
	char* p = (char*)buff;

	char s[64];
	tracet(4, "writessr: index=%d\n", index);
	time2str(obs0->data[0].time, s, NULL);
	p += sprintf(p, "%s", buff);
	p += sprintf(p, "> PHASE_BIAS %s\n", s);
	//p += sprintf(p, "%s", s);
	for (i = 0; i < MAXSAT; i++)
	{
		if (!ssr[i].npbias)continue;

		sys = satsys(i + 1, &prn);
		if (sys == SYS_GPS)
		{
			p += sprintf(p, "G%2d  %14.4f  %8.4f", prn, ssr[i].yaw_ang, ssr[i].yaw_rate);
			for (j = 0; j < 32; j++)
			{
				if (msm_sig_gps[ssr_sig_gps[j]] == "" || !ssr[i].cbias[ssr_sig_gps[j] - 1])continue;
				p += sprintf(p, "  %4s  %8.4f  %d  %d  %2d", msm_sig_gps[ssr_sig_gps[j]], ssr[i].pbias[ssr_sig_gps[j] - 1],
					ssr[i].sii[ssr_sig_gps[j] - 1], ssr[i].swl[ssr_sig_gps[j] - 1], ssr[i].sdc[ssr_sig_gps[j] - 1]);
			}
			p += sprintf(p, "\n");
		}
		if (sys == SYS_GLO)
		{
			p += sprintf(p, "R%2d  %14.4f  %8.4f", prn, ssr[i].yaw_ang, ssr[i].yaw_rate);
			for (j = 0; j < 32; j++)
			{
				if (msm_sig_glo[ssr_sig_glo[j]] == "" || !ssr[i].cbias[ssr_sig_glo[j] - 1])continue;
				p += sprintf(p, "  %4s  %8.4f  %d  %d  %2d", msm_sig_glo[ssr_sig_glo[j]], ssr[i].pbias[ssr_sig_glo[j] - 1],
					ssr[i].sii[ssr_sig_glo[j] - 1], ssr[i].swl[ssr_sig_glo[j] - 1], ssr[i].sdc[ssr_sig_glo[j] - 1]);
			}
			p += sprintf(p, "\n");
		}
		if (sys == SYS_GAL)
		{
			p += sprintf(p, "E%2d  %14.4f  %8.4f", prn, ssr[i].yaw_ang, ssr[i].yaw_rate);
			for (j = 0; j < 32; j++)
			{
				if (msm_sig_gal[ssr_sig_gal[j]] == "" || !ssr[i].cbias[ssr_sig_gal[j] - 1])continue;
				p += sprintf(p, "  %4s  %8.4f  %d  %d  %2d", msm_sig_gal[ssr_sig_gal[j]], ssr[i].pbias[ssr_sig_gal[j] - 1],
					ssr[i].sii[ssr_sig_gal[j] - 1], ssr[i].swl[ssr_sig_gal[j] - 1], ssr[i].sdc[ssr_sig_gal[j] - 1]);
			}
			p += sprintf(p, "\n");
		}
		if (sys == SYS_CMP)
		{
			p += sprintf(p, "C%2d  %14.4f  %8.4f", prn, ssr[i].yaw_ang, ssr[i].yaw_rate);
			for (j = 0; j < 32; j++)
			{
				if (msm_sig_cmp[ssr_sig_cmp[j]] == "" || !ssr[i].cbias[ssr_sig_cmp[j] - 1])continue;
				p += sprintf(p, "  %4s  %8.4f  %d  %d  %2d", msm_sig_cmp[ssr_sig_cmp[j]], ssr[i].pbias[ssr_sig_cmp[j] - 1],
					ssr[i].sii[ssr_sig_cmp[j] - 1], ssr[i].swl[ssr_sig_cmp[j] - 1], ssr[i].sdc[ssr_sig_cmp[j] - 1]);
			}
			p += sprintf(p, "\n");
		}
	}
...
static int outssr_sp3clk(uint8_t* buff, rtksvr_t* svr, int index, nav_t* nav0, obs_t* obs0, ssr_t* ssr)
{
	solopt_t solopt = solopt_default;
	int i = 0, j = 0, prn = 0, sys = 0, nbuff = 0, nsat = nav0->nssr[1];
	char* p = (char*)buff;
	...
	time2str(obs0->data[0].time, s, NULL);
	//p += sprintf(p, "%s", p);
	p += sprintf(p, "> ORBIT CLOCK %s\n", s);
	//p += sprintf(p, "%s", s);
	for (i = 0; i < MAXSAT; i++)
	{
		if (ssr[i].iode != nav0->eph[i].iode)continue;

		sys = satsys(i + 1, &prn);
		if (sys == SYS_GLO)
			i = i;
		p += sprintf(p, "%s%2d  %4d   ORBIT  %8.4f  %8.4f  %8.4f  %8.4f  %8.4f  %8.4f   CLOCK  %8.4f  %8.4f  %8.4f\n", satno2sys(i + 1), prn, ssr[i].iode,
			ssr[i].deph[0], ssr[i].deph[1], ssr[i].deph[2], ssr[i].ddeph[0], ssr[i].ddeph[1], ssr[i].ddeph[2],
...
static int writessr_t(uint8_t* buff, rtksvr_t* svr, int index, nav_t* nav0, obs_t* obs0, ssr_t* ssr)
{
	int nbuff = 0;
	uint8_t* p = buff;

	p += outssr_code_bias(p, svr, index, nav0, obs0, ssr);
	p += outssr_phase_bias(p, svr, index, nav0, obs0, ssr);
	p += outssr_sp3clk(p, svr, index, nav0, obs0, ssr);

	return p - buff;
	//saveoutbuf(svr, buff, 1024, index);
}

static void writessr(rtksvr_t* svr, int index, nav_t* nav0, obs_t* obs0, ssr_t* ssr)
{
	uint8_t buff[10 * MAXSOLMSG + 1];
	int nbuff = 0;

	nbuff = writessr_t(buff, svr, index, nav0, obs0, ssr);
...
static void corr_phase_bias(const rtk_t* rtk, obsd_t* obs, int n, nav_t* nav/*0,ssr_t *ssr*/)
{
	double freq;
	uint8_t code;
	int i, j;
	int sys_idx = 0, sys = 0, prn = 0;
	nav->nssr[0] = nav->nssr[1] = 0;
	for (i = 0; i < n; i++) for (j = 0; j < NFREQ; j++) {
		sys_idx = satsysidx(obs[i].sat);
		sys = satsys(obs[i].sat, &prn);
		int f = rtk->opt.gnss_frq_idx[sys_idx][j];
		code = obs[i].code[f - 1];
		if ((freq = sat2freq(obs[i].sat, code, nav)) == 0.0) continue;
		if (nav->ssr[obs[i].sat - 1].cbias[code - 1] == 0 && f == 0)
		{
			f = f;//obs[i].L[f-1] = 0;
		}
		/* correct code bias (m) */
		obs[i].P[f - 1] -= nav->ssr[obs[i].sat - 1].cbias[code - 1];
		/* correct phase bias (cyc) */
		if (nav->ssr[obs[i].sat - 1].pbias[code - 1] == 0)continue;
		obs[i].L[f - 1] -= nav->ssr[obs[i].sat - 1].pbias[code - 1] * freq / CLIGHT;
		//obs[i].yaw = nav->ssr[obs[i].sat - 1].yaw_ang * D2R;
	}
}
/* periodic command ----------------------------------------------------------*/
...
#endif
{
	...
	uint8_t* p, * q;
	char msg[128];
	int i, j, n, fobs[3] = { 0 }, cycle, cputime, ep_index = 0;
	...
	for (cycle = 0; svr->state; cycle++) {
		...
		for (i = 0; i < fobs[0]; i++) { /* for each rover observation data */
			...
			if (!strstr(svr->rtk.opt.pppopt, "-DIS_FCB")) {
				corr_phase_bias(&svr->rtk, obs.data, obs.n, &svr->nav);
			}
			/* rtk positioning */
			svr->rtk.epoch++;

			rtksvrlock(svr);
			if (svr->rtk.epoch > 20)rtkpos(&svr->rtk, obs.data, obs.n, &svr->nav);
			rtksvrunlock(svr);
			...
			writessr(svr, 8, &svr->nav, &obs, svr->nav.ssr);
...
extern int rtksvrinit(rtksvr_t* svr)
{
	...
	svr->nav.nssr[0] = svr->nav.nssr[1] = 0;
	...
	solopt_t* solopt, stream_t* moni, char* errmsg)
{
	...
	if (prcopt->mode != PMODE_FIXED || prcopt->mode != PMODE_PPP_FIXED) {
		for (i = 0; i < 3; i++) svr->rtk.sol.rr[i] = /*stas[0].pos*/svr->rtk.opt.rb[i];
	}
	...
	for (i = 0; i < 3; i++) { /* output peek buffer */
	...
	for (i = 6; i < 8; i++) {
		writesolhead(svr->stream + i, svr->solopt + i - 6, &svr->rtk.opt);
	}
	//writesolhead(svr->stream + 8, svr->solopt + 8 - 6, &svr->rtk.opt);
