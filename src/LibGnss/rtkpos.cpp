...
#define NR(opt)     (NP(opt)+NI(opt)+NT(opt)+NL(opt))//#define NR(opt)      (NP(opt)+NC(opt)+NRDCB(opt)+NIFCB(opt)+NT(opt)+NI(opt))
...
extern int rtkoutstat(rtk_t* rtk, char* buff)
{
	...
	if (est && (rtk->opt.tropopt == TROPOPT_EST || rtk->opt.tropopt == TROPOPT_ESTG)) {
		for (i = 0; i < 2; i++) {
			j = rtk->tc ? xiTrp(&rtk->opt.insopt) : IT(i, &rtk->opt);
			xa[0] = j < rtk->na ? rtk->xa[j] : 0.0;
...
static void outsolstat(rtk_t* rtk, const nav_t* nav, const obsd_t* obs, int nobs)
{
	...
	for (i = 0; i < MAXSAT; i++) {
		...
		satno2id(i + 1, id);
		for (j = 0; j < nfreq; j++) {
			k = rtk->tc ? xiAmb(&rtk->opt.insopt, i + 1, j) : (ppp ? iamb_ppp(&rtk->opt, i + 1, j) : IB(i + 1, j, &rtk->opt));

			fprintf(fp_stat, "$SAT,%d,%.3f,%s,%d,%.1f,%.1f,%.4f,%.4f,%d,%.1f,%d,%d,%d,%d,%d,%d,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f\n",
				week, tow, id, j + 1, ssat->azel[0] * R2D, ssat->azel[1] * R2D,
				ssat->vs?ssat->resp[j]:0, ssat->vs?ssat->resc[j]:0, ssat->vsat[j],                  /*post float pseudorange/phase residual*/
				ssat->snr[j] * SNR_UNIT, ssat->fix[j], ssat->slip[j]&3,         /*snr, fix flag, slip flag*/
				ssat->lock[j], ssat->outc[j], ssat->slipc[j], ssat->rejc[j],   /*lock, outc, slipc, rejc*/
				rtk->x[k], ssat->fix_amb[j], SQRT(rtk->P[k + k * rtk->nx]),    /*float amb, fix amb,var amb*/
				ssat->pri_res[0][j], ssat->pri_res[1][j],                    /*pri phase/pseudorange residual*/
				ssat->fix_res[0][j] * 100, ssat->fix_res[1][j],                    /*after fix ambiguity residual*/
				ssat->norm_v[0][j], ssat->norm_v[1][j],                     /*norm phase/pseudorange residual*/
				ssat->norm_pri_v[0][j], ssat->norm_pri_v[1][j],
				ssat->ins_inno[j][2], ssat->ins_inno[j][3],
				ssat->ins_wl_res[2],
				ssat->ins_ewl_res[2]);
		}
	}
...
static int outfcbres(rtk_t* rtk, obsd_t* obs, int n)
{
	...
	for (i = 0; i < n; i++) {
		...
		if (first) {
			...
			fprintf(fp_wl_fcb, "* %s %s   %4.2f   %2d %2d  %5d ratio:%4.2f\n",
				time_str(rtk->sol.time, 3), ref_sat_id, rtk->ssat[amb->ref_sat_no - 1].azel[1] * R2D, rtk->sol.ns, rtk->sol.ar_wl_ns, rtk->epoch, rtk->sol.WL_ratio_m[sys_idx]);
			//rtk->sol.WL_ratio = 0;
	...
	for (i = 0; i < n; i++) {
		...
		if (second) {
			...
			fprintf(fp_nl_fcb, "* %s %s   %4.2f   %2d %2d %5d  ratio:%4.2f\n",
				time_str(rtk->sol.time, 3), ref_sat_id, rtk->ssat[amb->ref_sat_no - 1].azel[1] * R2D, rtk->sol.ns, rtk->sol.ar_nl_ns, rtk->epoch, rtk->sol.ratio_m[sys_idx]);
			ref_sat = amb->ref_sat_no;
			second = 0;
		}
		if (ref_sat == sat) continue;
		...
		fprintf(fp_nl_fcb, "%s %12d %12.3f %12.3f %3s %4.2f\n",
			sat_id, amb->nl_fix, amb->nl, amb->nl_res, amb->fix_nl_flag && rtk->sol.ratio_m[sys_idx] > 2.0 ? "YES" : "NO", rtk->ssat[sat - 1].azel[1] * R2D);
		...
		//for (i = 0; i < n; i++) {
			//sat = obs[i].sat;
			//amb = &rtk->sdamb[sat - 1];
		amb->wl_fix = 0; amb->wl = 0; amb->wl_res = 0;
		amb->nl_fix = 0; amb->nl = 0; amb->nl_res = 0;
		rtk->sol.ar_wl_ns = 0; rtk->sol.ar_nl_ns = 0;
	}
	for (i = 0; i < NSYS + 1; i++)
	{
		if (rtk->sol.ratio_m[i] == 0)continue;
		if (rtk->sol.ratio_m[i] > 3.0)rtk->sol.nl_fix_m[i]++;
		else rtk->sol.nl_fix_m[i] = 0;
		rtk->sol.ratio_m[i] = 0; rtk->sol.WL_ratio_m[i] = 0;
	}
#else
	for (i = 0; i < MAXSAT; i++) {
...
{
	double pi = f < NFREQ + NEXOBS ? obs[i].L[f] : obs[i].P[f - NFREQ - NEXOBS];
	double pj = f < NFREQ + NEXOBS ? obs[j].L[f] : obs[j].P[f - NFREQ - NEXOBS];
	...
	double bl, double dt, int f, const prcopt_t* opt, const obsd_t* obs)
{
	...
	double fact = 1.0;
	double sinel = sin(el);
	int i, nf = NF(opt), frq, code, prn = 0;
	satsys(sat, &prn);
...
static void udion(rtk_t* rtk, double tt, double bl, const int* sat, int ns)
{
	...
	for (i = 0; i < ns; i++) {
		...
		rtk->ssat[sat[i] - 1].ion = rtk->x[j];
	}
}
static void udion_set(rtk_t* rtk, const obsd_t* obs, const nav_t* nav, const int* iu, const int* ir, double tt, double bl, const int* sat, int ns)
{
	double el, fact, freq1, freq2, Ion_init = 0, pr1 = 0, pr2 = 0;
	int i, j, tc = rtk->tc, frq_id1 = 0, frq_id2 = 0, sys_idx = 0;
	insopt_t* opt = &rtk->opt.insopt;
	...
	for (i = 0; i < ns; i++) {
		//j = II(sat[i], &rtk->opt);
		j = tc ? xiIon(opt, i) : II(sat[i], &rtk->opt);
		sys_idx = satsysidx(sat[i]);
		frq_id1 = rtk->opt.gnss_frq_idx[sys_idx][0] - 1;
		frq_id2 = rtk->opt.gnss_frq_idx[sys_idx][1] - 1;
		freq1 = sat2freq(sat[i], obs[iu[i]].code[frq_id1], nav);
		freq2 = sat2freq(sat[i], obs[iu[i]].code[frq_id2], nav);
		pr1 = sdobs(obs, iu[i], ir[i], NFREQ + NEXOBS + frq_id1);
		pr2 = sdobs(obs, iu[i], ir[i], NFREQ + NEXOBS + frq_id2);
		el = rtk->ssat[sat[i] - 1].azel[1];
		fact = cos(el);
		if (rtk->x[j] == 0.0) {
			Ion_init = fabs((pr1 - pr2) / (1 - SQR(freq1 / freq2)));
			if (freq1 == 0 || freq2 == 0 || Ion_init == 0)Ion_init = 1E-6;
			//initx(rtk, 1E-6, SQR(rtk->opt.std[1] * bl / 1E4), j);
			initx(rtk, (Ion_init), SQR(rtk->opt.std[1] * bl / 1E4 * fact), j);
		}
		else {
			/* elevation dependent factor of process noise */
			//el = rtk->ssat[sat[i] - 1].azel[1];
...
static void udtrop(rtk_t* rtk, double tt, double bl)
{
	...
	for (i = 0; i < 2; i++) {
		/// TODO: for tc
		j = tc ? (xiTrp(&rtk->opt.insopt) + i) : IT(i, &rtk->opt);
...
static double mwmeas(const prcopt_t* opt, const obsd_t* obs, int iu, int ir, const nav_t* nav, double* var, double el)
{
	int f1 = 0, f2 = 1, sys, prn, sys_idx = -1, sat = obs[iu].sat;

	double freq1 = 0, freq2 = 0, lam1 = 0, lam2 = 0;
	...
	freq1 = sat2freq(sat, obs[iu].code[f1], nav); /*lam[f1] = CLIGHT / freq1*/;
	freq2 = sat2freq(sat, obs[iu].code[f2], nav); //lam[f2] = CLIGHT / freq2;

	if (obs[ir].L[f1] == 0.0 || obs[iu].L[f1] == 0.0) return 0.0;
	if (obs[ir].P[f2] == 0.0 || obs[iu].P[f2] == 0.0) return 0.0;

	l1 = sdobs(obs, iu, ir, f1);
	l2 = sdobs(obs, iu, ir, f2);
	p1 = sdobs(obs, iu, ir, f1 + NFREQ + NEXOBS);
	p2 = sdobs(obs, iu, ir, f2 + NFREQ + NEXOBS);
	lam_wl = CLIGHT / (freq1 - freq2);
	lam1 = CLIGHT / freq1;
	lam2 = CLIGHT / freq2;

	double mw = 0.0;
	mw = (freq1 * (l1 * lam1) - freq2 * (l2 * lam2)) / (freq1 - freq2)
		- (freq1 * (p1)+freq2 * (p2)) / (freq1 + freq2);
	mw /= lam_wl;//N1-N2
	if (var) {
		*var = SQR(0.7 * opt->eratio[0]) * (SQR(opt->err[1]) + SQR(opt->err[2] / sin(el))) / SQR(lam_wl);
	}

	//return ((l1 - l2) * CLIGHT / (freq1 - freq2) - (freq1 * p1 + freq2 * p2) / (freq1 + freq2)) / lam_wl;
	return mw;
}
...
static void detslp_mw(rtk_t* rtk, const obsd_t* obs, int iu, int ir,
	...
	if ((w1 = mwmeas(&rtk->opt, obs, iu, ir, nav, NULL, 0.0)) == 0.0) {
	...
	const nav_t* nav)
{
	...
	int f1 = 0, f2 = 1, prn = 0, sys = 0, sys_idx = 0;
...
static void saveinfo(rtk_t* rtk, const obsd_t* obs, int iu, int ir, const nav_t* nav)
{
	...
	if ((w1 = mwmeas(&rtk->opt, obs, iu, ir, nav, &var1, rtk->ssat[sat_no - 1].azel[1])) == 0.0) {
		return;
	}
	...
	if (rtk->ssat[sat_no - 1].mw[2] > 0) {
		...
		double K = 0.0;
		K = rtk->ssat[sat_no - 1].mw[3] / (rtk->ssat[sat_no - 1].mw[3] + var1);
		rtk->ssat[sat_no - 1].mw[1] += K * (w1 - w0);
		rtk->ssat[sat_no - 1].mw[3] -= K * rtk->ssat[sat_no - 1].mw[3];
		rtk->ssat[sat_no - 1].mw[2]++;
	}
	else {
	...
	const int* iu, const int* ir, int ns, const nav_t* nav)
{
	...
	int i, j, f, slip, rejc, reset, nf = NF(&rtk->opt), sysi, tc = rtk->tc, iamb = 0, sys = 0, prn = 0, sys_idx = 0, sat0 = 0, frq_id1 = 0, frq_id2 = 1, iion = 0;;
	...
	for (f = 0; f < nf; f++) {
		...
		for (i = 1; i <= MAXSAT; i++) {
			...
			//iamb = IB(i, f, &rtk->opt);
			iamb = tc ? xiAmb(&rtk->opt.insopt, i, f) : IB(i, f, &rtk->opt);
		...
		for (i = 0; i < ns; i++) {
			...
			j = tc ? xiAmb(&rtk->opt.insopt, sat[i], f) : IB(sat[i], f, &rtk->opt);
			...
			if (rejc) {
				trace(3, "%s(%d): reset %s L%d ambiguity value and variance due to outler\n", time_str(obs[0].time, 1), rtk->tc ? rtk->ins_kf->couple_epoch : rtk->epoch, sat_id(sat[i]), f + 1);
			}
			if (slip) {
				trace(3, "%s(%d): reset %s L%d ambiguity value and variance due to slip\n", time_str(obs[0].time, 1), rtk->tc ? rtk->ins_kf->couple_epoch : rtk->epoch, sat_id(sat[i]), f + 1);
			}
		...
		for (i = j = 0, offset = 0.0; i < ns; i++) {
			sat0 = sat[i];
			rtk->ssat[sat0 - 1].amb_from_pr[f] = 0.0;
			sys = satsys(sat0, &prn);
			sys_idx = satsysidx(sat0);
			if (sys_idx == -1) continue;
			frq_id1 = rtk->opt.gnss_frq_idx[sys_idx][0] - 1;
			frq_id2 = rtk->opt.gnss_frq_idx[sys_idx][f] - 1;
			freq1 = sat2freq(sat[i], obs[iu[i]].code[frq_id1], nav);
			freq2 = sat2freq(sat[i], obs[iu[i]].code[frq_id2], nav);
			iion = tc ? xiIon(&rtk->opt.insopt, sat0) : II(sat0, &rtk->opt);

			if (rtk->opt.ionoopt != IONOOPT_IFLC) {
				/* phase diff between rover and base in cycles */
				cp = sdobs(obs, iu[i], ir[i], frq_id2);
				/* pseudorange diff between rover and base in meters */
				pr = sdobs(obs, iu[i], ir[i], frq_id2 + NFREQ + NEXOBS);

				freqi = sat2freq(sat[i], obs[iu[i]].code[frq_id2], nav);

				double p1 = sdobs(obs, iu[i], ir[i], frq_id1 + NFREQ + NEXOBS), p2 = sdobs(obs, iu[i], ir[i], rtk->opt.gnss_frq_idx[sys_idx][1] - 1 + NFREQ + NEXOBS);
				double ion = 0;
				ion = (p1 - p2) / (f ? (1.0 - SQR(freq1 / freq2)) : 1.0);
				//double ion = (P[0] - P[1]) / (1.0 - SQR(frq1 / frq2));

				if (cp == 0.0 || pr == 0.0 || freqi <= 0.0) continue;
				/*if (rtk->opt.ionoopt == IONOOPT_UC)*/ion = rtk->x[iion];

				/* translate cycles diff to meters and subtract pseudorange diff */
				bias[i] = cp * CLIGHT / freqi - pr + 2.0 * ion * SQR(freq1 / freq2);//m
				bias[i] /= CLIGHT / freq2;//cycle
				if (f == 1) {
					double bias0 = rtk->ssat[sat0 - 1].amb_from_pr[0];//N1
					bias[i] = bias0 - bias[i];//N1-N2
					//bias[i] = rtk->ssat[sat - 1].mw[1];
				}
				else if (f == 2) {
					double bias0 = rtk->ssat[sat0 - 1].amb_from_pr[0] - rtk->ssat[sat0 - 1].amb_from_pr[1];//N2
					bias[i] = bias0 - bias[i];//N2-N3
				}
			}
			else {  /* use ionosphere free calc with 2 freqs */
				cp1 = sdobs(obs, iu[i], ir[i], frq_id1);
				cp2 = sdobs(obs, iu[i], ir[i], frq_id2);
				pr1 = sdobs(obs, iu[i], ir[i], NFREQ + NEXOBS + frq_id1);
				pr2 = sdobs(obs, iu[i], ir[i], NFREQ + NEXOBS + frq_id2);
				//freq1 = sat2freq(sat[i], obs[iu[i]].code[frq_id1], nav);
				//freq2 = sat2freq(sat[i], obs[iu[i]].code[frq_id2], nav);
			...
			iamb = tc ? xiAmb(&rtk->opt.insopt, sat[i], f) : IB(sat[i], f, &rtk->opt);
			rtk->ssat[sat0 - 1].amb_from_pr[f] = bias[i];
			if (rtk->x[iamb] != 0.0) {
				freqi = sat2freq(sat[i], obs[iu[i]].code[frq_id2], nav);
		...
		for (i = 0; i < ns; i++) {
			...
			//freqi = sat2freq(sat[i], obs[iu[i]].code[rtk->opt.gnss_frq_idx[sys_idx][f] - 1], nav);
	...
	const int* iu, const int* ir, int ns, const nav_t* nav)
{
		...
		(rtk->opt.ionoopt == IONOOPT_STEC))
	{
		...
		udion_set(rtk, obs, nav, iu, ir, tt, bl, sat, ns);
	}
	/* temporal update of tropospheric parameters */
...
static void zdres_sat(rtk_t* rtk, int base, double r, const obsd_t* obs, const nav_t* nav,
	...
	const prcopt_t* opt, double* y, double* freq)
{
	...
	if (opt->ionoopt == IONOOPT_IFLC) { /* iono-free linear combination */
		...
		for (i = 0; i < nf; i++) {
			...
			if (obs->L[f1] != 0.0)
			{
				y[i] = obs->L[f1] * CLIGHT / freq[i] - r - dant[f1];
				rtk->ssat[obs->sat - 1].obs_phase[f1] = obs->L[f1] * CLIGHT / freq[i];
			}
...
{
	double r, rr_[3], pos[3], dant[(NFREQ + NEXOBS)] = { 0 }, disp[3];
	...
	for (i = 0; i < n; i++) {
		...
		zdres_sat(rtk, base, r, obs + i, nav, azel + i * 2, dant, opt, y + i * nf * 2, freq + i * nf);
	}
	...
	double* dtdx)
{
	...
	//int i = IT(r, opt);
	int i = opt->mode >= PMODE_TC_SPP && opt->mode <= PMODE_TC_PPP ? xiTrp(&opt->insopt) : IT(r, opt);
	...
	double* H, double* R, int* vflg, int* exc, int* prn)
{
	...
	int i, j, ii, jj, k, m, f, frq, code, nv = 0, nb[(NFREQ + NEXOBS) * 4 * 2 + 2] = { 0 }, b = 0, sysi, sysj, nf = NF(opt);
	...
	double ve[MAXOBS * 2 * NFREQ] = { 0 }, vare[MAXOBS * 2 * NFREQ] = { 0 }, wave_le = 0;
	...
	for (m = 0; m < NSYS; m++) {
		...
		for (f = opt->mode > PMODE_DGPS ? 0 : nf; f < nf * 2; f++) {
			...
			for (j = 0; j < ns; j++) {
				...
				rtk->ssat[sat[j] - 1].vs = 1;
				if (exc && exc[sat[j] - 1] == 1) continue;  /*exclude satellite*/
				if (rtk->ssat[sat[j] - 1].slipc[frq] >= 3 && rtk->ssat[sat[j] - 1].lock[frq] <= 0) {
					trace(3, "%s(%d): %s L%d continuous cycle slips, exclude\n", time_str(obs[0].time, 1), rtk->tc ? rtk->ins_kf->couple_epoch : rtk->epoch, sat_id(sat[j]), frq + 1);
					exc[sat[j] - 1] = 1;
					continue;
				}
...
#if 1
							if (tc) {
								jacobi_tc_meas2a_ECEF(&dpda, &rtk->ins_kf->sol->dcm, &rtk->opt.insopt.imup.lever_arm_gps, &e[iu[i] * 3], &e[iu[j] * 3]);
								for (k = xiA(&rtk->opt.insopt); k < xiA(&rtk->opt.insopt) + xnA(&rtk->opt.insopt); k++) {
									Hi[k] = dpda.v[k - xiA(&rtk->opt.insopt)];
								}
							}
#endif
							/*lever arm*/
							if (rtk->opt.insopt.est_armgps) {  // D*Cbe
								double sl[3] = { 0 };
								sl[0] = e[k + iu[i] * 3 + 0] - e[k + iu[j] * 3 + 0];
								sl[1] = e[k + iu[i] * 3 + 1] - e[k + iu[j] * 3 + 1];
								sl[2] = e[k + iu[i] * 3 + 2] - e[k + iu[j] * 3 + 2];
								matmul("NN", 1, 3, 3, 1.0, sl, rtk->ins_kf->insstate->dcm.v, 0.0, dpdl.v);
								for (k = xiArmGps(&rtk->opt.insopt); k < xiArmGps(&rtk->opt.insopt) + xnArmGps(&rtk->opt.insopt); k++) {
									Hi[k] = dpdl.v[k - xiArmGps(&rtk->opt.insopt)];
								}
							}
						}
					}
				...
				if (opt->ionoopt == IONOOPT_UC) {
					iion = tc ? xiIon(&opt->insopt, sat[i]) : II(sat[i], opt);
					jion = tc ? xiIon(&opt->insopt, sat[j]) : II(sat[j], opt);
				...
				if (!code) {
					iamb = tc ? xiAmb(&opt->insopt, sat[i], frq) : IB(sat[i], frq, opt);
					jamb = tc ? xiAmb(&opt->insopt, sat[j], frq) : IB(sat[j], frq, opt);
					...
					if (opt->ionoopt != IONOOPT_IFLC) {
						...
						if (frq == 0) {
							v[nv] -= CLIGHT / freqi * x[iamb] - CLIGHT / freqj * x[jamb];
						...
						{
							double amb00 = 0, amb01 = 0, amb10 = 0, amb11 = 0;
							//iamb = IB(sat[i], 0, opt);jamb = IB(sat[j], 0, opt);
							iamb = tc ? xiAmb(&opt->insopt, sat[i], 0) : IB(sat[i], 0, opt);
							jamb = tc ? xiAmb(&opt->insopt, sat[j], 0) : IB(sat[j], 0, opt);

							if (H) {
								Hi[iamb] = CLIGHT / freqi;
								Hi[jamb] = -CLIGHT / freqj;
							}
							amb00 = x[iamb]; amb01 = x[jamb];//N1
							//iamb = IB(sat[i], frq, opt); jamb = IB(sat[j], frq, opt);
							iamb = tc ? xiAmb(&opt->insopt, sat[i], frq) : IB(sat[i], frq, opt);
							jamb = tc ? xiAmb(&opt->insopt, sat[j], frq) : IB(sat[j], frq, opt);

							amb10 = x[iamb]; amb11 = x[jamb];//NW=N1-N2
							v[nv] -= CLIGHT / freqi * (amb00 - amb10) - CLIGHT / freqj * (amb01 - amb11);
							if (H) {
								Hi[iamb] = -CLIGHT / freqi;
								Hi[jamb] = CLIGHT / freqj;
							}
						...
						else if (frq == 2)
						{
							...
							//iamb = IB(sat[i], 1, opt); jamb = IB(sat[j], 1, opt);
							iamb = tc ? xiAmb(&opt->insopt, sat[i], 1) : IB(sat[i], 1, opt);
							jamb = tc ? xiAmb(&opt->insopt, sat[j], 1) : IB(sat[j], 1, opt);

							if (H) {
								Hi[iamb] = -CLIGHT / freqi;
								Hi[jamb] = CLIGHT / freqj;
							}
							amb00 = amb00 - x[iamb]; amb01 = amb01 - x[jamb];//N2

							//iamb = IB(sat[i], frq, opt); jamb = IB(sat[j], frq, opt);
							iamb = tc ? xiAmb(&opt->insopt, sat[i], frq) : IB(sat[i], frq, opt);
							jamb = tc ? xiAmb(&opt->insopt, sat[j], frq) : IB(sat[j], frq, opt);

							amb10 = x[iamb]; amb11 = x[jamb];//NW=N2-N3
							v[nv] -= CLIGHT / freqi * (amb00 - amb10) - CLIGHT / freqj * (amb01 - amb11);
							if (H) {
								Hi[iamb] = -CLIGHT / freqi;
								Hi[jamb] = CLIGHT / freqj;
				...
				if ((rtk->tc || rtk->stc) && !code && rtk->ssat[sat[j] - 1].slip[frq]) {
					v[nv] -= rtk->ssat[sat[j] - 1].ins_repair_cs[frq] * CLIGHT / freqi;
				}

				/* save residuals */
				/* save residuals */
				if (post) {
					/*after ambiguity fix residual*/
					if (post == 9) {
						if (code) rtk->ssat[sat[j] - 1].fix_res[1][frq] = v[nv]; /* pseudorange */
						else      rtk->ssat[sat[j] - 1].fix_res[0][frq] = v[nv];  /* carrier phase */
					}
					else {
						/*post residual*/
					...
					/*prior residual*/
					if (code) rtk->ssat[sat[j] - 1].pri_res[1][frq] = v[nv]; /* pseudorange */
					else      rtk->ssat[sat[j] - 1].pri_res[0][frq] = v[nv];  /* carrier phase */
				}
				...
				char sat1_id[5] = { '\0' }, sat2_id[5] = { '\0' };
				satno2id(sat[i], sat1_id);
				satno2id(sat[j], sat2_id);
				if (opt->maxinno > 0.0 && fabs(v[nv]) > opt->maxinno) {
					rtk->ssat[sat[j] - 1].vsat[frq] = 0;
					rtk->ssat[sat[j] - 1].rejc[frq]++;
					trace(2, "%s(%d): prior residual rejected (sat=%3s-%3s %s%d v=%.3f)\n",
						time_str(obs[0].time, 1), (rtk->stc || rtk->tc) ? rtk->ins_kf->couple_epoch : rtk->epoch, sat1_id, sat2_id, code ? "P" : "L", frq + 1, v[nv]);
					continue;
				}
				/* save dd measurements and satellite list for ins aid, Li Tuan*/
				if (ins_aid && !code && post < 8) {
					rtk->ssat[sat[j] - 1].ins_inno[frq][1] = v[nv] / (CLIGHT / freqi);  /*current epoch inno*/
					for (int ie = xiP(&rtk->opt.insopt); ie < xiP(&rtk->opt.insopt) + xnP(&rtk->opt.insopt); ie++) rtk->ssat[sat[j] - 1].e[ie] = Hi[ie];
				}
				...
				if (!code && (rtk->tc || rtk->stc) && post < 8) {
					rtk->ssat[sat[i] - 1].obs_phase_var[frq] = Ri[nv];
					rtk->ssat[sat[j] - 1].obs_phase_var[frq] = Rj[nv];
					for (int ie = xiP(&rtk->opt.insopt); ie < xiP(&rtk->opt.insopt) + xnP(&rtk->opt.insopt); ie++) rtk->ssat[sat[j] - 1].e[ie] = Hi[ie];
				}
				...
				if (!0) {
					...
					{
						// 北斗GEO卫星放大5倍？
						if (strstr(nav->pcvs[sat[j] - 1].type, "BEIDOU-2G") ||
							strstr(nav->pcvs[sat[j] - 1].type, "BEIDOU-3G"))
							//if (5 >= prn && prn!= 17 )
						{
							Rj[nv] = 25 * Rj[nv] + SQR(0.15);
						...
						else
						{
							if (2 == frq)
							{
								if (OTYPE_CP_LP == code)		Rj[nv] = 10.0 * Rj[nv] + SQR(0.03);
								if (OTYPE_PR_LP == code)		Rj[nv] = 1.0 * Rj[nv] + SQR(0.01);
							}
						}
						//if (prn == 8)
						//{
					...
					if (code == OTYPE_CP_LP)
					{
						rtk->ssat[sat[j] - 1].azel[0] = azel[1 + iu[j] * 2];
				...
				// Panda 20171104
				if (!0/*&&fq<2*/) {
					if (code == OTYPE_CP_LP)//carrier phase
					{
						rtk->ppp_glo.equS[sat[j] - 1].eS_cp.resi_0[frq] = v[nv];//验前残差
						rtk->ppp_glo.equS[sat[j] - 1].eS_cp.sig_v0[frq] = SQRT(Ri[nv] + Rj[nv]);//验前方差
					}
					else
					{//伪距
						rtk->ppp_glo.equS[sat[j] - 1].eS_pr.resi_0[frq] = v[nv];//验前残差
						rtk->ppp_glo.equS[sat[j] - 1].eS_pr.sig_v0[frq] = SQRT(Ri[nv] + Rj[nv]);//验前方差
					}
					if (code == OTYPE_CP_LP)
					{
						rtk->ssat[sat[j] - 1].vsat[frq] = 1;

						rtk->ppp_glo.equS[sat[j] - 1].eS_cp.bUsed[frq] = true;
						Ri[nv] *= rtk->ppp_glo.equS[sat[j] - 1].eS_cp.eWF_fin[frq];
						Rj[nv] *= rtk->ppp_glo.equS[sat[j] - 1].eS_cp.eWF_fin[frq];
						int prn = 0;
						satsys(sat[j], &prn);
						if (0 == frq)
						{
							...
							if (SYS_GPS == sysj)
								rtk->ppp_glo.nSat[0]++;
							else if (SYS_GLO == sysj)
								rtk->ppp_glo.nSat[1]++;
							else if (SYS_GAL == sysj)
								rtk->ppp_glo.nSat[2]++;
							else if (SYS_CMP == sysj || SYS_BD3 == sysj)
					...
					{
						rtk->ppp_glo.equS[sat[j] - 1].eS_pr.bUsed[frq] = true;
						Ri[nv] *= rtk->ppp_glo.equS[sat[j] - 1].eS_pr.eWF_fin[frq];
						Rj[nv] *= rtk->ppp_glo.equS[sat[j] - 1].eS_pr.eWF_fin[frq];
					}

					// Panda 20170707
					rtk->ppp_glo.oEI[rtk->ppp_glo.nOEI].el_rad = azel[1 + iu[j] * 2];//高度角
					rtk->ppp_glo.oEI[rtk->ppp_glo.nOEI].fq = frq;//频率索引
					rtk->ppp_glo.oEI[rtk->ppp_glo.nOEI].sat = sat[j];
					rtk->ppp_glo.oEI[rtk->ppp_glo.nOEI].sig = SQRT(Ri[nv] + Rj[nv]);//方差
					rtk->ppp_glo.oEI[rtk->ppp_glo.nOEI].ty = code;//观测值类型
				...
				if (rtk->opt.glomodear == GLO_ARMODE_AUTOCAL)
					...
					icb = rtk->ssat[sat[i] - 1].icbias[frq] * (CLIGHT / freqi) - rtk->ssat[sat[j] - 1].icbias[frq] * (CLIGHT / freqj);
				trace(level, "%s(%d) sat=%3s-%3s %s%d v=(%9.5f-%9.5f)-(%9.5f-%9.5f)-(%9.5f-%9.5f)=%9.5f R=%9.6f %9.6f lock=%5d outc=%5d cs=%5s slipc=%4d SD_amb=%9.3f DD_amb=%9.3f amb_stat=%s el=%3.1f dw=%5.2f\n",
					post == 9 ? "FIXAMB" : (H ? "PRIOR " : "POST  "), (rtk->tc || rtk->stc) ? rtk->ins_kf->couple_epoch : rtk->epoch, sat1_id, sat2_id, f < nf ? "L" : "P", f% nf + 1,
					y[f + iu[i] * nf * 2], y[f + ir[i] * nf * 2], y[f + iu[j] * nf * 2], y[f + ir[j] * nf * 2], (code ? 0 : x[rtk->tc ? xiAmb(&rtk->opt.insopt, sat[i], f % nf) : IB(sat[i], f % nf, &rtk->opt)])* (CLIGHT / freqi),
					(code ? 0 : x[rtk->tc ? xiAmb(&rtk->opt.insopt, sat[j], f % nf) : IB(sat[j], f % nf, &rtk->opt)])* (CLIGHT / freqi), v[nv], Ri[nv], Rj[nv], rtk->ssat[sat[j] - 1].lock[f % nf], rtk->ssat[sat[j] - 1].outc[f % nf],
					rtk->ssat[sat[j] - 1].slip[f % nf] & CS_LLI ? "LLI" : (rtk->ssat[sat[j] - 1].slip[f % nf] & CS_MW ? "MW" : (rtk->ssat[sat[j] - 1].slip[f % nf] & CS_GF ? "GF" : (rtk->ssat[sat[j] - 1].slip[f % nf] & CS_INS ? "CS_INS" : "NONE"))),
					code ? 0 : rtk->ssat[sat[j] - 1].slipc[f % nf], (code ? 0 : x[rtk->tc ? xiAmb(&rtk->opt.insopt, sat[j], f % nf) : IB(sat[j], f % nf, &rtk->opt)]),
					code ? 0 : H ? rtk->ssat[sat[j] - 1].ambRes[0][f % nf] : rtk->ssat[sat[j] - 1].ambRes[1][f % nf],
					rtk->ssat[sat[j] - 1].fix[f % nf] <= 1 ? "FLOAT" : (rtk->ssat[sat[j] - 1].fix[f % nf] == 2 ? "FIX  " : "HOLD "),
					rtk->ssat[sat[j] - 1].azel[1] * R2D, rtk->ssat[sat[j] - 1].var_fact);
				vflg[nv++] = (sat[i] << 16) | (sat[j] << 8) | ((code ? 1 : 0) << 4) | (frq);
				nb[b]++;
				rtk->ppp_glo.bUsed_sat[sat[j] - 1] = true;
			}
	...
	if (!H && post != 9 && ne > 0) {
		...
		trace(2, "%s(%d): post-fit residual (%d) rejected %s %s%d res=%9.4f el=%4.1f cs=%5s\n",
			time_str(obs[0].time, 1), (rtk->tc || rtk->stc) ? rtk->ins_kf->couple_epoch : rtk->epoch, post, sat_id(exc_sat), maxfrq / 2 ? "P" : "L", maxfrq % 2 + 1, vmax, azel[1 + maxobs * 2] * R2D,
			rtk->ssat[exc_sat - 1].slip[maxfrq % 2] & CS_LLI ? "LLI" : (rtk->ssat[exc_sat - 1].slip[maxfrq % 2] & CS_MW ? "MW" : (rtk->ssat[exc_sat - 1].slip[maxfrq % 2] & CS_GF ? "GF" : (rtk->ssat[exc_sat - 1].slip[maxfrq % 2] & CS_INS ? "CS_INS" : "NONE"))));
	...
	rtk_t* rtk, double* y)
{
	...
	satposs(&rtk->opt, time, obsb, nb, nav, opt->sateph, rs, dts,NULL, var, svh);

	if (!zdres(rtk, 1, obsb, nb, rs, dts, var, svh, nav, rtk->rb, opt, 1, yb, e, azel, freq)) {
		return tt;
	}
	for (i = 0; i < n; i++) {
...
static int ddmat(rtk_t* rtk, double* D, int gps, int glo, int sbs, double el_mask, int* vs, int* sat_fix_flag)
{
	int i, j, k, m, f, nb = 0, nx = rtk->nx, na = rtk->na, nf = NF(&rtk->opt), nofix, level = 3, slip = CS_NONE;
	...
	for (m = 0; m < 5; m++) { /* m=0:gps/sbs,1:glo,2:gal,3:bds,4:qzs */
		...
		for (f = 0, k = na; f < nf; f++, k += MAXSAT) {
			...
			for (i = k; i < k + MAXSAT; i++) {
				slip = rtk->ssat[i - k].slip[f] != CS_NONE;
				/* skip if sat not active */
				if (rtk->x[i] == 0.0 || !test_sys(rtk->ssat[i - k].sys, m) ||
					!rtk->ssat[i - k].vsat[f] || slip) {
					continue;
				}

				if (m == 3) {
					int prn, sys;
					sys = satsys(i - k + 1, &prn);
					if ((sys & SYS_CMP) && prn >= 1 && prn <= 5) continue;
				}
				...
				if (rtk->ssat[i - k].lock[f] >= 0 &&
					...
					p += sprintf(p, "REF SAT: %s_L%d, ", sat_id(i - k + 1), f + 1);
			...
			for (j = k; j < k + MAXSAT; j++) {
				slip = rtk->ssat[j - k].slip[f] != CS_NONE;
				if (i == j || rtk->x[j] == 0.0 || !test_sys(rtk->ssat[j - k].sys, m) ||
					!rtk->ssat[j - k].vsat[f] || slip) {
					continue;
				}
				...
				if (rtk->ssat[j - k].lock[f] >= 0 &&
					...
					sat_fix_flag[nb] = (j - k) * 10 + f;
					/* inc # of sats used for fix */
					ref[nb] = i - k + 1;
					fix[nb++] = j - k + 1;
					rtk->ssat[j - k].fix[f] = 2; /* fix */
					p += sprintf(p, "%s_L%d ", sat_id(j - k + 1), f + 1);
					if (f == 0) (*vs)++;
				}
				/* else don't use this sat for fixing ambiguity */
	...
	trace(level, "%s", sat_ids);

	if (nb > 0) {
		trace(3, "refSats="); tracemat(2, ref, 1, nb, 7, 2);
		trace(3, "fixSats="); tracemat(2, fix, 1, nb, 7, 2);
	}
	return nb;
}

static int ddidx(rtk_t* rtk, double* ix, int gps, int glo, int sbs)
{
	int i, j, k, m, f, n, nb = 0, na = rtk->na, nf = NF(&rtk->opt), nofix, ref_indx = 0;
	double fix[MAXSAT], ref[MAXSAT], ref_elve = 0;
		...
		for (f = 0, k = na; f < nf; f++, k += MAXSAT) {
			...
			for (i = k; i < k + MAXSAT; i++) {
				...
				if (rtk->ssat[i - k].lock[f] >= 0 && !(rtk->ssat[i - k].slip[f] & 2) &&
					...
					if (ref_elve <= rtk->ssat[i - k].azel[1])ref_elve = rtk->ssat[i - k].azel[1], ref_indx = i - k;
					//                     rtk->ssat[i-k].fix[f]=2; /* fix */
					//                     break;/* break out of loop if find good sat */
				}
				/* else don't use this sat for fixing ambiguity */
			...
			//set ref sat
			if (rtk->ssat[ref_indx].lock[f] >= 0 && !(rtk->ssat[ref_indx].slip[f] & 2) &&
				rtk->ssat[ref_indx].azel[1] >= rtk->opt.elmaskar && !nofix) {
				rtk->ssat[ref_indx].fix[f] = 2; /* fix  find good sat*/
				i = ref_indx + k;
			}
			if (rtk->ssat[i - k].fix[f] != 2) continue;  /* no good sat found */
...
{
	int i, n, m, f, index[MAXSAT], sat_idxs[MAXSAT] = { 0 }, nv = 0, nf = NF(&rtk->opt), rfsat_index = 0;
	int iamb = 0;
	double rfsat_elve = 0;
	trace(3, "restamb :\n");
	...
	for (m = 0; m < 5; m++) for (f = 0; f < nf; f++) {
		for (n = i = 0; i < MAXSAT; i++) {
			...
			sat_idxs[n] = i;
			index[n++] = iamb;
			if (rtk->ssat[i].azel[1] > rfsat_elve && rtk->ssat[i].lock[f] >= 0)rfsat_elve = rtk->ssat[i].azel[1], rfsat_index = n - 1;
		}
		if (n < 2) continue;

		//xa[index[0]] = rtk->x[index[0]];
		xa[index[rfsat_index]] = rtk->x[index[rfsat_index]];
		for (i = 1; i < n; i++) {
			//xa[index[i]] = xa[index[0]] - bias[nv];
			//nv++;
			if (i == rfsat_index)continue;
			xa[index[i]] = xa[index[rfsat_index]] - bias[nv++];
		}
	}
}
...
static void holdamb(rtk_t* rtk, const double* xa, solins_t* solins, gtime_t t)
{
	...
	for (m = 0; m < 5; m++) for (f = 0; f < nf; f++) {
		for (n = i = 0; i < MAXSAT; i++) {
			...
			index[n++] = rtk->tc ? xiAmb(&rtk->opt.insopt, i + 1, f) : IB(i + 1, f, &rtk->opt);;
	...
	if (rtk->tc) {
		inskf_feedback(t, SOLQ_FIX, &rtk->opt.insopt, rtk->x, solins);
	}
		...
		for (j = nv = 0; j < MAXSAT; j++) {
			...
			if (test_sys(rtk->ssat[j].sys, 1) && rtk->ssat[j].vsat[f] && rtk->ssat[j].lock[f] >= 0) {
				if (i < 0) {
					...
					int iambj = rtk->tc ? xiAmb(&rtk->opt.insopt, j + 1, f) : IB(j + 1, f, &rtk->opt);
					int iambi = rtk->tc ? xiAmb(&rtk->opt.insopt, i + 1, f) : IB(i + 1, f, &rtk->opt);
...
static int reset_fix_flag(rtk_t* rtk, const int* sat_fix_flag, int unfix, const int* idx) {
	int i, info, frq, no, nf = NF(&rtk->opt);
	char sat_str[1024] = { '\0' };
	char* p = sat_str;

	for (i = 0; i < unfix; i++) {
		info = sat_fix_flag[idx[i]];
		no = floor(info / 10.0);
		frq = info - no * 10;
		rtk->ssat[no].fix[frq] = 1;
		p += sprintf(p, "%s_L%d ", sat_id(no + 1), frq + 1);
	}

	if (unfix > 0) {
		trace(3, "%s(%d): PAR success, weed out %s \n", time_str(rtk->sol.time, 1), (rtk->stc || rtk->tc) ? rtk->ins_kf->couple_epoch : rtk->epoch, sat_str);
	}

	return 1;
}
static int parfixsol(rtk_t* rtk, const double* y, const double* b, const int nb, const double* Qb, const double* Qab,
	const int nfix, const int* delete_record, const int* record_Z_row, const int* sat_fix_flag, double* bias, double* xa)
{
	int unfix = nb - nfix, na = rtk->na, i, nb_fix = 0;

	double* y_p, * db, * yb, * Qb_p, * Qab_p, * QQ, * QabZT, * ZQb, * ZQbZT;

	y_p = mat(nb, 1); yb = mat(nb, 1); db = mat(nb, 1);
	Qb_p = mat(nfix, nfix); Qab_p = mat(na, nfix); QQ = mat(na, nb);
	QabZT = mat(na, nb); ZQb = mat(nb, nb); ZQbZT = mat(nb, nb);

	//    matprint(1,y,nb,1,15,6);
		/*y_p=Zp*y 将未固定的模糊度置前*/
	ifmatmul("NN", nb, 1, nb, 1.0, record_Z_row, y, 0.0, y_p);

	//    matprint(1,y_p,nb,1,15,6);
	//    matprint(1,b,nb,1,15,6);

		/*QabZT=Qab*Zp' = (Zp*Qab')'*/
	fimatmul("NT", na, nb, nb, 1.0, Qab, record_Z_row, 0.0, QabZT);
	...
	for (i = 0; i < nfix; i++) {
		yb[i] = y_p[i + unfix] - b[i];
	}

	if (rtk->tc && rtk->ins_kf->couple_epoch >= 34760000) {
		matprint(1, yb, nfix, 1, 15, 6);
	}

	mat_block(ZQbZT, nb, nb, unfix, nfix, unfix, nfix, Qb_p);
	mat_block(QabZT, na, nb, 0, na, unfix, nfix, Qab_p);

	if (!matinv(Qb_p, nfix)) {
		/* transform float to fixed solution (xa=xa-Qab*Qb\(b0-b)) */
		matmul("NN", nfix, 1, nfix, 1.0, Qb_p, yb, 0.0, db);
		matmul("NN", na, 1, nfix, -1.0, Qab_p, db, 1.0, rtk->xa);

		/* covariance of fixed solution (Qa=Qa-Qab*Qb^-1*Qab') */
		matmul("NN", na, nfix, nfix, 1.0, Qab_p, Qb_p, 0.0, QQ);
		matmul("NT", na, na, nfix, -1.0, QQ, Qab_p, 1.0, rtk->Pa);

		reset_fix_flag(rtk, sat_fix_flag, unfix, delete_record);
		restamb(rtk, y_p + unfix, nb, xa);

		nb_fix = nb;
		trace(3, "%s(%d): partial AR OK! ratio=%5.2f\n", time_str(rtk->sol.time, 1), (rtk->stc || rtk->tc) ? rtk->ins_kf->couple_epoch : rtk->epoch, rtk->sol.ratio);
	}
	else {
		nb_fix = 0;
	}

	free(y_p); free(yb); free(db);
	free(Qb_p); free(Qab_p); free(QabZT); free(ZQbZT); free(QQ);
...
{
	prcopt_t* opt = &rtk->opt;
	int i, j, ny, nb, info, nx = rtk->nx, na = rtk->na, par = 0, vs = 0, nfix;
	double* D, * DP, * y, * Qy, * b, * db, * Qb, * Qab, * QQ, s[2], ratio_thres = rtk->opt.thresar[0];
	double QQb[MAXSAT];
	double tra_Qbb_float = 0.0, tra_Qbb_fix = 0.0;
	double gain;
	int sat_fix_flag[MAXOBS * 2] = { 0 };

	/* Create single to double-difference transformation matrix (D')
		  used to translate phase biases to double difference */
	D = zeros(nx, nx);

	if ((rtk->sol.ar_nb = nb = ddmat(rtk, D, gps, glo, sbs, rtk->opt.elmaskar, &vs, sat_fix_flag)) <= 0) {  /* nb is sat pairs */
		rtk->cannotfix++;
		trace(3, "%s(%d): not valid DD ambiguities to AR ns=%d nb=0\n", time_str(rtk->sol.time, 1), (rtk->tc || rtk->stc) ? rtk->ins_kf->couple_epoch : rtk->epoch, ns);
		free(D);
		return -1; /* flag abort */
	}
	int* delete_record = imat(nb, 1), * record_Z_row = imat(nb, nb);
	...
	rtk->sol.adop = pow(SQRT(matdet(Qb, nb)), 1.0 / nb) * 1000;
	/* lambda/mlambda integer least-square estimation */
	/* return best integer solutions */
	/* b are best integer solutions, s are residuals */
	if (!(info = lambda_par(&rtk->opt, rtk, nb, 2, y + na, Qb, b, s, &nfix, record_Z_row, delete_record, &ratio_thres))) {
		trace(3, "N(1)=     "); tracemat(3, b, 1, nb, 7, 2);
		trace(3, "N(2)=     "); tracemat(3, b + nb, 1, nb, 7, 2);
		rtk->sol.ar_nb = (float)nfix;
		if (nfix > 0) {
			rtk->sol.thres = (float)ratio_thres;
			/* validation by popular ratio-test of residuals*/
			if (rtk->sol.ratio > ratio_thres) {
				/* init non phase-bias states and covariances with float solution values */
				for (i = 0; i < na; i++) {
					rtk->xa[i] = rtk->x[i];
					for (j = 0; j < na; j++) rtk->Pa[i + j * na] = rtk->P[i + j * nx];
				}
				for (i = 0; i < na; i++) tra_Qbb_float += rtk->Pa[i + i * na];

				if (nb == nfix) { /*update solution by full ambiguity*/
					/* y = differences between float and fixed dd phase-biases bias = fixed dd phase-biases   */
					for (i = 0; i < nb; i++) {
						bias[i] = b[i];
						y[na + i] -= b[i];
					}
					...
					if (!matinv(Qb, nb)) {  /* returns 0 if inverse successful */
						...
						for (i = 0; i < na; i++) {
							tra_Qbb_fix += rtk->Pa[i + i * na];
						}
						gain = SQRT(tra_Qbb_float / tra_Qbb_fix);
						...
						nb = 0;
						trace(2, "%s(%d): Qb error in AR\n", time_str(rtk->sol.time, 1), (rtk->stc || rtk->tc) ? rtk->ins_kf->couple_epoch : rtk->epoch);
					}
				}
				else { /*update solution by partial ambiguities*/
					if (!(nb = parfixsol(rtk, y + na, b, nb, Qb, Qab, nfix, delete_record, record_Z_row, sat_fix_flag, bias, xa))) {
						trace(2, "%s(%d): partial AR failed!\n", time_str(rtk->sol.time, 1), (rtk->stc || rtk->tc) ? rtk->ins_kf->couple_epoch : rtk->epoch);
					}
				}
			}
		}
		else {
			trace(3, "%s(%d): FAR ratio test failed ratio=%5.2f RATIO=%5.2f\n", time_str(rtk->sol.time, 1), (rtk->stc || rtk->tc) ? rtk->ins_kf->couple_epoch : rtk->epoch, rtk->sol.ratio, ratio_thres);
			nb = 0;
		}
	}
	else {
		trace(2, "%s(%d): lambda error(info=%d)\n", time_str(rtk->sol.time, 1), (rtk->stc || rtk->tc) ? rtk->ins_kf->couple_epoch : rtk->epoch, info);
		nb = 0;
	}
	free(D); free(y); free(Qy); free(DP);
	free(b); free(db); free(Qb); free(Qab); free(QQ);
	free(record_Z_row); free(delete_record);
...
static int resamb_LAMBDA(rtk_t* rtk, double* bias, double* xa, double* Pa, int gps, int glo, int sbs, int ns, const int* sat)
{
	...
	int ipos = rtk->tc ? xiP(&rtk->opt.insopt) : 0, npos = rtk->tc ? xnP(&rtk->opt.insopt) : 3;
...
{
	int i, f, lockc[(NFREQ + NEXOBS)], ar = 0, excflag = 0, arsats[MAXOBS] = { 0 };
...
static void rtk_resetamb(rtk_t* rtk, double* xa, const double* Bc, const int* sat1, const int* sat2, int nb)
{
	int i, iamb, jamb;
	for (i = 0; i < nb; i++) {
...
static void rtk_resetamb_uc(rtk_t* rtk, double* xa, const double* Bc, const int* sat1, const int* sat2, int nb)
{
	int i, iamb, jamb;
	for (i = 0; i < nb; i++) {
		iamb = IB(sat1[i], 0, &rtk->opt);
...
extern int rtk_gen_sat_sd(rtk_t* rtk, const nav_t* nav, const obsd_t* obs, int n, const int* exc, int* sat1, int* sat2, const int* satar, const int* sat_idx, int* iu, int* ir, int f, double* el)
{
	...
	for (i = 0; sat_sys[i]; i++) { /*loop each system*/
		...
		for (j = m = 0; j < n; j++) {
			sys = satsys(satar[j], &prn);

			if (!rtk->ssat[satar[j] - 1].vs) continue;
			...
			if ((rtk->ssat[satar[j] - 1].slip[0] || rtk->ssat[satar[j] - 1].slip[1])) {
				rtk->ssat[satar[j] - 1].lock[f] = -rtk->opt.minlock;
				continue;
			}
			if (exc[satar[j] - 1] || !rtk->ssat[satar[j] - 1].vsat[frq] || rtk->ssat[satar[j] - 1].azel[1] < elmask
				|| rtk->ssat[satar[j] - 1].lock[f] < 0) {
				continue;
			}
...
#endif
			sat_no[m] = satar[j];
			el_temp[m] = rtk->ssat[satar[j] - 1].azel[1];
			idxs[m] = sat_idx[j];
...
static int rtk_fix_UC_sol(rtk_t* rtk, const obsd_t* obs, const nav_t* nav, double* H_uc, const double* Bl,
...
//        matprint(1,dx,na,1,15,6);
		...
		/*reset ambiguity*/
		rtk_resetamb_uc(rtk, xa, Bc, sat1, sat2, nb);
...
//        matprint(1,dx,na,1,15,6);
		...
		/*reset ambiguity*/
		rtk_resetamb(rtk, xa, Bc, sat1, sat2, nb);
...
static int rtk_SDmat_UC_wl(int post, const double* xp, rtk_t* rtk, const obsd_t* obs, int ns, const nav_t* nav, double* H_nl, int* sat1,
	int* sat2, int* iu, int* ir, double* el, double* Nw, double* Bw, double* Nl, double* Nc)
{
	...
	//	rtk->ssat[i].fix[j] = 0;
	//}
	trace(3, "rtk SDmat  : epoch=%d, ns=%d na=%d\n", rtk->epoch, ns, rtk->na);
	if (ns > rtk->na)
	{
		i = 0;
	}
	for (i = 0; i < ns; i++) {
		...
		sys_idx = satsysidx(sat);
		if (sys_idx == -1) continue;
		if (rtk->sdamb[sat - 1].fix_wl_flag != 1 && post != -2) continue;//采用MW计算值
...
static int rtk_SDmat_UC(rtk_t* rtk, const obsd_t* obs, int ns, const nav_t* nav, double* H_nl, double* H_uc, int* sat1,
	int* sat2, int* iu, int* ir, double* el, double* Nw, double* Bw, double* Nl, double* Nc, double* sd_nl_fcb)
{
	...
	for (i = 0; i < ns; i++) {
		...
		//H_uc[iamb + (rtk->na + nb) * rtk->nx] = 1.0;
		//H_uc[jamb + (rtk->na + nb) * rtk->nx] = -1.0;
		...
		Nw[nb] = rtk->sdamb[sat - 1].wl;
		Bw[nb] = ROUND(rtk->sdamb[sat - 1].wl);
		if (Nl)Nl[nb] = sd_uc + 0 * rtk->sdamb[sat - 1].nl;
		Nc[nb] = sd_uc;
...
static int rtk_SDmat(rtk_t* rtk, const obsd_t* obs, int ns, const nav_t* nav, double* H_nl, double* H_if, int* sat1,
	int* sat2, int* iu, int* ir, double* el, double* Nw, double* Bw, double* Nl, double* Nc, double* sd_nl_fcb)
{
	prcopt_t opt = rtk->opt;
	int i, j, sat, ref_sat, prn, sys_idx = -1, iamb, jamb, nb = 0;
...
static int rtk_resamb_nl(rtk_t* rtk, double* H_nl, double* nl_amb, int num_nl)
{
...
static int rtk_wl_resamb0(int post, int iter, rtk_t* rtk, const obsd_t* obs, int ns, const nav_t* nav, const  int* sat1,
	const int* sat2, const int* iu, const int* ir, const double* el, double* Nw, double* Bw, double* Nl_t, double* Nc, int* wl_fix_nsat)
{
	...
	//组件单差宽巷方差
	if (!post) {
		nb = rtk_SDmat_UC_wl(0, rtk->xs, rtk, obs, ns, nav, H_wl, sat10, sat20, iu0, ir0, el0, Nw, Bw, N10, Nc);
		if (nb < MIN_AMB_RES && iter == 0 && rtk->fix_epoch != 0)//only used for first
		{
			...
			nb = rtk_SDmat_UC_wl(0, rtk->xs, rtk, obs, ns, nav, H_wl, sat10, sat20, iu0, ir0, el0, Nw, Bw, N10, Nc);
			R = zeros(nb, nb);
			for (i = 0; i < nb; i++) R[i + i * nb] = rtk->opt.varholdamb / 100;
			...
			free(R);
			nb = rtk_SDmat_UC_wl(-1, rtk->xs, rtk, obs, nb, nav, H_wl, sat10, sat20, iu0, ir0, el0, Nw, Bw, N10, Nc);
		}
		else {
			R = zeros(nb, nb);
...
static int rtk_resamb_wl(rtk_t* rtk, double* H_nl, double* nl_amb, int num_nl)
{
	trace(3, "resamb_nl  : epoch=%d\n", rtk->epoch);
	int nb = num_nl, stat = 0;
	double* Qnl, * HP, s[2] = { 0 }, * b, * pb;
...
static int rtk_wl_resamb_lambda(int post, int iter, rtk_t* rtk, const obsd_t* obs, int ns, const nav_t* nav, const  int* sat1,
	const int* sat2, const int* iu, const int* ir, const double* el, double* Nw, double* Bw, double* Nl_t, double* Nc, int* wl_fix_nsat, double* xa)
{
	trace(3, "pppar_UC_ILS  :wl_resamb_lambda epoch=%d\n", rtk->epoch);
	double* H_wl, * H_uc, el0[MAXOBS] = { 0 }, * N10, * R, * xp, * Pp, BN[MAXOBS] = { 0 };
	int i, j, sat, prn, ref_sat, sys, sys_idx = -1, sat10[MAXOBS] = { 0 }, sat20[MAXOBS] = { 0 }, iu0[MAXOBS] = { 0 }, ir0[MAXOBS] = { 0 }, na = rtk->na, stat = 0;
	matcpy_int(sat10, sat1, ns, 1);
	matcpy_int(sat20, sat2, ns, 1);
	matcpy_int(iu0, iu, ns, 1);
	...
	//double* Nw, * Nl, * Nc;   /*float ambiguity*/
	//double* Bw, * Bl, * Bc;   /*inter ambiguity*/
	H_wl = zeros(rtk->nx, ns); H_uc = zeros(rtk->nx, rtk->nx);
	N10 = zeros(ns, 1);
	...
	if (!post) {
		...
		for (i = 0; i < ns; i++)
		{
			...
			sys_idx = satsysidx(sat10[i]);
			wl_amb = rtk->sdamb[sat10[i] - 1].wl, wl_var = rtk->sdamb[sat10[i] - 1].wl_var;
			if (conf_func(ROUND(wl_amb), wl_amb, wl_var) < rtk->opt.thresar[1] || fabs(newround(wl_amb) - wl_amb) > 0.25) {//降低阈值，增加可固定卫星数
				rtk->sdamb[sat10[i] - 1].fix_wl_flag = 0;
		...
		if (nb < MIN_AMB_RES)
		{
			nb = rtk_SDmat_UC_wl(-2, rtk->xs, rtk, obs, ns, nav, H_wl, sat10, sat20, iu0, ir0, el0, Nw, Bw, N10, Nc);
			//nb = rtk_resamb_wl(rtk, H_wl, N10, ns);
			if (rtk_resamb_wl(rtk, H_wl, N10, nb) >= MIN_AMB_RES) {
				rtk->sol.WL_ratio;
				matcpy(BN, N10, nb, 1);
				for (i = 0; i < NSYS + 1; i++)wl_fix_nsat[i] = 0;
				for (i = 0; i < nb; i++)
				{
					if (rtk->sdamb[sat10[i] - 1].wl_fix == BN[i]) {
						Bw[i] = -N10[i] + Bw[i];//宽巷Nw v
						rtk->sdamb[sat10[i] - 1].nl = Bw[i];
						rtk->sdamb[sat10[i] - 1].fix_wl_flag = 1;
						sys_idx = satsysidx(sat10[i]);
						/*float wl ambiguity*/
						int iamb = IB(sat10[i], 0, &rtk->opt);
						int jamb = IB(sat20[i], 0, &rtk->opt);
						double sd_n1 = rtk->xs[iamb] - rtk->xs[jamb];
				...
				free(R);
				nb = rtk_SDmat_UC_wl(-1, rtk->xs, rtk, obs, nb, nav, H_wl, sat10, sat20, iu0, ir0, el0, Nw, Bw, N10, Nc);
				free(H_wl); free(N10); /*free(R);*/ free(xp); free(Pp); free(H_uc);
				return nb;
			}
			else {
				free(H_wl); free(H_uc); 	free(N10); free(xp); free(Pp);
				return nb;
			}
		}
		nb = rtk_SDmat_UC_wl(-2, rtk->xs, rtk, obs, nb, nav, H_wl, sat10, sat20, iu0, ir0, el0, Nw, Bw, N10, Nc);

		//matcpy(Bw, Nw, ns, 1);
		//nb = resamb_wl(rtk, H_wl, Bw, nb);
		if (rtk_resamb_wl(rtk, H_wl, Bw, nb) >= MIN_AMB_RES) {
			matcpy(BN, Bw, nb, 1);
			for (i = 0; i < nb; i++)
			{
			...
			//nb = rtk_SDmat_UC(rtk, obs, ns, nav, NULL, H_uc, sat10, sat20, iu0, ir0, el0, Nw, Bw, NULL, Nc, NULL);
			//rtk_fix_UC_sol(rtk, obs, nav, H_uc, BN, Bw, nb, sat10, sat20, iu0, xa);
...
static int rtkar_IF_ILS(int iter, rtk_t* rtk, double* xa, double* bias, const obsd_t* obs, int n, const int* satar, const int* exc, const nav_t* nav)
{
	prcopt_t opt = rtk->opt;
	...
	trace(3, "rtkar_IF_ILS  start : epoch=%d\n", rtk->epoch);
	...
	nb_t = nb = rtk_SDmat(rtk, obs, ns, nav, H_nl, H_if, sat1, sat2, iu, ir, el, Nw, Bw, Nl, Nc, sd_nl_fcb);
	rtk->nb_ar = nb;
	if (!iter) {
		for (i = 0; i < NSYS; i++) {
	...
	if (nb >= MIN_AMB_RES) {
		nb = rtk_resamb_nl(rtk, H_nl, Nl, nb);
	...
	if (!nb && !stat && nb_t > MIN_AMB_RES) {
		...
		exc0[max_idx - 1] = 1;
		nb = rtkar_IF_ILS(++iter, rtk, xa, bias, obs, n, satar, exc0, nav);
		if (nb) stat = 1;
	}

	return stat ? nb : 0;
}
static int rtkar_UC_ILS0(int iter, rtk_t* rtk, double* xa, double* bias, const obsd_t* obs, int n, const int* satar, const int* sat_idx, const int* exc, const nav_t* nav)
{
	rtk->sol.WL_ratio = 0;
	...
	trace(3, "rtkar_UC_ILS  start : epoch=%d\n", rtk->epoch);
	if (rtk->epoch == 426) {
		ns = ns;
		//return 0;
	}
	...
	if (rtk->opt.gpsmodear == ARMODE_CONT && rtk->opt.bdsmodear == ARMODE_CONT)//多系统模糊度固定
	{
		...
		sys_n[0] = rtkar_UC_ILS0(0, rtk, xa0, bias, obs, n, satar, sat_idx, exc, nav);
		rtk->sol.ratio_m[0] = rtk->sol.ratio;
		//BDS second
		rtk->opt.gpsmodear = ARMODE_OFF; rtk->opt.bdsmodear = ARMODE_CONT;
		sys_n[1] = rtkar_UC_ILS0(0, rtk, xa1, N1, obs, n, satar, sat_idx, exc, nav);
	...
	/*generate satellite SD*/
	if (!(ns = rtk_gen_sat_sd(rtk, nav, obs, n, exc, sat1, sat2, satar, sat_idx, iu, ir, 0, el))) {
		return 0;
	}
	for (i = 0; i < MAXSAT; i++) {
		rtk->sdamb[i].nl = 0.0;
	...
	//搜索WL模糊度
	nb = rtk_wl_resamb0(0, iter, rtk, obs, ns, nav, sat1, sat2, iu, ir, el, Nw, Bw, Nl_t, Nc, wl_fix_nsat);
	if (nb < MIN_AMB_RES)
	{
		nb = rtk_wl_resamb_lambda(0, iter, rtk, obs, ns, nav, sat1, sat2, iu, ir, el, Nw, Bw, Nl_t, Nc, wl_fix_nsat, xa);
		if (sys_n_indx != 2) {
	...
	nb_t = nb = rtk_SDmat_UC(rtk, obs, ns, nav, H_nl, H_uc, sat1, sat2, iu, ir, el, Nw, Bw, Nl, Nc, sd_nl_fcb);
	//matcpy(Nl, Nl_t, ns, 1);
	...
	if (nb >= MIN_AMB_RES) {
		...
		if (!sys_copy) {
			if (nb && rtk_fix_UC_sol(rtk, obs, nav, H_uc, Nl, Bw, nb, sat1, sat2, iu, xa)) {
				rtk->sol.ar_nl_ns = nb;
				stat = 1;
				//if (/*sys_copy == 0&& */sys_n_indx == 0) {
				if (rtk->sol.ratio_m[sys_idx] >= 4.0) {
					//matcpy(rtk->x, rtk->xs, rtk->nx, 1);
					//matcpy(rtk->P, rtk->Ps, rtk->nx, rtk->nx);
				}
			}
			else {
				trace(2, "%s(%d): rtk AR failed ratio=%5.2f\n", time_str(obs[0].time, 1),
					rtk->tc ? rtk->ins_kf->couple_epoch : rtk->epoch, rtk->sol.ratio);
			}
		}
	}
	...
	free(Bw); free(Bl); free(Bc);
	free(sd_nl_fcb); free(Qnl);
	trace(3, "rtkar_UC_ILS end  : epoch=%d\n", rtk->epoch);
	if (!nb && !stat && nb_t > MIN_AMB_RES) {
		trace(3, "rtkar_UC_ILS  :fix failed epoch=%d\n", rtk->epoch);
		int exc0[MAXSAT] = { 0 };
		matcpy_int(exc0, exc, MAXSAT, 1);
		//组合系统，优先剔除bds
		...
		for (i = 0; i < NSYS; i++) {
			...
			//exc0[max_idx /*- 1*/] = 1;
			exc0[sat1[sys_wl_sat - 1] - 1] = 1;
			trace(3, "rtkar_IF_ILS exclude  : epoch=%d sat=%d\n", rtk->epoch, max_idx);
		}

		if (iter != -1)nb = rtkar_UC_ILS0(++iter, rtk, xa, bias, obs, n, satar, sat_idx, exc0, nav);
...
static int rtkar(rtk_t* rtk, double* bias, double* xa, double* Pa, int nf, const obsd_t* obs, int ns, const nav_t* nav, const int* satar, const int* sat_idx, int* exc)
{
	int i, nb = 0;
	double var = 0.0;
	prcopt_t opt = rtk->opt;
	...
	if (rtk->opt.ionoopt == IONOOPT_IFLC) {
		nb = rtkar_IF_ILS(0, rtk, xa, bias, obs, ns, satar, exc, nav);
	}
	else if (rtk->opt.modear > ARMODE_OFF) {
		nb = rtkar_UC_ILS0(0, rtk, xa, bias, obs, ns, satar, sat_idx, exc, nav);
	}
	if (!0) {
		for (i = 0; i < NSYS; i++) {
...
static int manage_rtkar(rtk_t* rtk, double* bias, double* xa, double* Pa, int nf, const obsd_t* obs, int ns, const nav_t* nav, const int* satar, const int* sat_idx, int* exc)
{
	...
	if (rtk->opt.glomodear != GLO_ARMODE_FIXHOLD || rtk->holdamb) {
		nb = rtkar(rtk, bias, xa, Pa, nf, obs, ns, nav, satar, sat_idx, exc);
	}
	else nb = 0;
...
static int relpos(rtk_t* rtk, const obsd_t* obs, int nu, int nr, const nav_t* nav) {
	...
	double* rs = NULL, * dts = NULL, * var = NULL, * y = NULL, * e = NULL, * azel = NULL, * freq = NULL, * v = NULL, * H = NULL, * R = NULL,
		* xp = NULL, * Pp = NULL, * xa = NULL, * Pa = NULL, * bias = NULL, dt, rr[3], * post_v = NULL;
	double ba_std, bg_std;
	int i = 0, j, f, n = nu + nr, ns, ny, nv = 0, sat[MAXSAT], iu[MAXSAT], ir[MAXSAT], niter, exc[MAXSAT] = { 0 }, * prn = NULL;
	...
	if (rtk->pfix && rtk->tc) {
#define GETSTD(x)   rtk->P[(x)+(x)*rtk->nx]
		ba_std = sqrt(GETSTD(xiBa(&opt->insopt)) + GETSTD(xiBa(&opt->insopt) + 1) + GETSTD(xiBa(&opt->insopt) + 1));
		bg_std = sqrt(GETSTD(xiBg(&opt->insopt)) + GETSTD(xiBg(&opt->insopt) + 1) + GETSTD(xiBg(&opt->insopt) + 1));
		ba_std *= MPS22MG;
		bg_std *= RPS2DPH;
		if (fabs(ba_std) < 1.0 || fabs(bg_std) < 1.0) rtk->ins_repair = 1;
#undef GETSTD
	}
	...
	rs = zeros(6, n);            /* range to satellites */
	dts = zeros(2, n);           /* satellite clock biases */
	var = zeros(1, n);
	y = zeros(nf * 2, n);
	e = zeros(3, n);
	...
	for (i = 0; i < MAXSAT; i++) {
		...
		rtk->ssat[i].cal_pr = 0.0;/* gps system */
	...
	satposs(&rtk->opt, time, obs, n, nav, opt->sateph, rs, dts,NULL, var, svh);
	...
	if (!zdres(rtk, 1, obs + nu, nr, rs + nu * 6, dts + nu * 2, var + nu, svh + nu, nav, rtk->rb, opt, 1,
		y + nu * nf * 2, e + nu * 3, azel + nu * 2, freq)) {
		errmsg(rtk, "initial base station position error\n");

		free(rs); free(dts); free(var); free(y); free(e); free(azel); free(freq); free(prn);
		return 0;
	}
	...
	ny = ns * nf * 2 + 2;
	v = zeros(ny, 1); H = zeros(rtk->nx, ny);
	R = zeros(ny, ny); bias = zeros(rtk->nx, 1);
	post_v = zeros(ny, 1);
	...
	for (i = 0; i < rtk->opt.robust ? 5 : niter; i++) {
		...
		/* calculate zero diff residuals [range - measured pseudorange] for rover (phase and code)
			output is in y[0:nu-1], only shared input with base is nav  */
		if (!zdres(rtk, 0, obs, nu, rs, dts, var, svh, nav, rr, opt, 0, y, e, azel, freq)) {
			errmsg(rtk, "rover initial position error\n");
			stat = SOLQ_NONE;
			break;
		}
		/* calculate double-differenced residuals and create state matrix from sat angles  */
		if ((nv = ddres(0, rtk, nav, obs, dt, xp, Pp, sat, y, e, azel, freq, iu, ir, ns, v, H, R, vflg, exc, prn)) < 1) {
		...
		rtk->ins_repair = 0;
		if (rtk->ins_rerun) continue;  // Ambiguity repair done and rerun, only once

		if ((info = filter0(rtk, xp, Pp, H, v, R, rtk->nx, nv, rtk->opt.robust, rtk->opt.kfopt, &res, tra))) {
			errmsg(rtk, "filter error (info=%d)\n", info);
			stat = SOLQ_NONE;
			break;
		}
		if (rtk->tc) {
			//matcpy(xp_copy, xp, rtk->nx, 1);
			inskf_feedback(obs->time, SOLQ_IGTC, &opt->insopt, xp, &insstate_copy);
			ins2gnss(&insstate_copy, &insstate_copy.arm_gps, NULL, rr, NULL, opt->insopt.mech_coord);
			insstate_copy.g_status = GNSS_STATUS_PPK;
		}
		else {
		...
		/*post residual test*/
		if (stat != SOLQ_NONE && zdres(rtk, 0, obs, nu, rs, dts, var, svh, nav, rr, opt, 0, y, e, azel, freq)) {
			//if ((qualityControl_v1(rtk, nav, i, vflg, nv, exc)))//	j = IB(sat, f, &rtk->opt);			rtk->ssat[sat - 1].amb_from_pr[f] = bias[i];
			//{
			//	continue;
			//}
			if (!ddres(-1, rtk, nav, obs, dt, xp, Pp, sat, y, e, azel, freq, iu, ir, ns, v, NULL, R, vflg, exc, NULL)) {
				continue;
			}
			else {
	...
	if (stat != SOLQ_NONE && rtk->opt.modear) {
		int nb;
		solins_t sol_copy = insstate_copy;
		matcpy(xa, xp, rtk->nx, 1);
		/* if valid fixed solution, process it */
		if ((nb = manage_amb_LAMBDA(rtk, bias, xa, Pa, sat, nf, ns)) > 0) {
			//if ((nb = manage_rtkar(rtk, bias, xa, Pa, 1, obs, ns, nav,sat,iu, exc)) > 0) {
			if (rtk->tc) {
				inskf_feedback(obs[0].time, SOLQ_IGTC, &opt->insopt, xa, &insstate_copy);
				ins2gnss(&insstate_copy, &insstate_copy.arm_gps, NULL, rr, NULL, opt->insopt.mech_coord);
				insstate_copy.g_status = GNSS_STATUS_PPK_AR;
			}
			else {
			...
			if (zdres(rtk, 0, obs, nu, rs, dts, var, svh, nav, rr, opt, 0, y, e, azel, freq)) {
				...
				if (fix_hold) {
					holdamb(rtk, xa, &sol_copy, obs[0].time);
					sol_copy.g_status = GNSS_STATUS_PPK_AR;
					if (rtk->tc) {
	...
	if (rtk->stc) {
		if (stat == SOLQ_FIX) {
			ins2gnss(rtk->ins_kf->insstate, &rtk->ins_kf->insstate->arm_gps, NULL, rr, NULL, opt->insopt.mech_coord); /*浮点解*/
			zdres(rtk, 0, obs, nu, rs, dts, var, svh, nav, rr, opt, 0, y, e, azel, freq);
			ddres(9, rtk, nav, obs, dt, xa, NULL, sat, y, e, azel, freq, iu, ir, ns, v, NULL, R, vflg, exc, NULL);
			ig_stc(rtk, v, nv, H, vflg, R, 1);
			matcpy(rtk->x, xa, rtk->nx, 1);
		}
		else {
			ins2gnss(rtk->ins_kf->insstate, &rtk->ins_kf->insstate->arm_gps, NULL, rr, NULL, opt->insopt.mech_coord); /*浮点解*/
			zdres(rtk, 0, obs, nu, rs, dts, var, svh, nav, rr, opt, 0, y, e, azel, freq);
			ddres(9, rtk, nav, obs, dt, xp, NULL, sat, y, e, azel, freq, iu, ir, ns, v, NULL, R, vflg, exc, NULL);
			ig_stc(rtk, v, nv, H, vflg, R, 0);
		}
	}
#endif
	...
	for (i = 0; i < NSYS + 1; i++)
	{
		rtk->sol.ns_sys[i] = rtk->ppp_glo.nSat[i] + (rtk->ppp_glo.nSat[i] ? 1 : 0);//add ref sat
	}
	...
	free(rs); free(dts); free(var); free(y); free(e); free(azel); free(post_v); free(prn);
	free(xp); free(Pp);  free(xa);  free(v); free(H); free(R); free(bias); free(freq);
	free(Pa);
	//rs = NULL; dts = NULL; var = NULL; y = NULL; e = NULL; azel = NULL; freq = NULL; v = NULL; H = NULL; R = NULL;
	//xp = NULL; Pp = NULL; xa = NULL; Pa = NULL; bias = NULL; post_v = NULL;

	if (stat != SOLQ_NONE) rtk->sol.stat = stat;

	return stat != SOLQ_NONE;
}
void init_kf_ion(const ion_base_t* ib, kf_ion_t* kf)
{
	int i, j;

	kf->gf_x[0] = ib->gf;
	kf->gf_x[1] = 0.01;
	kf->ion_x[0] = ib->ion_val;
	kf->ion_x[1] = 0.01;
	...
	for (i = 0; i < 2; i++)
	{
		for (j = 0; j < 2; j++)
		{
			kf->gf_P[i * 2 + j] = i == j ? 999.9 : 0.0;
			kf->ion_P[i * 2 + j] = i == j ? 999.9 : 0.0;
		}
	}
	// jxy 20171230 kf_ion
	/*
...
void initppp_global(rtk_t* rtk)
{
	int i, f;
	gtime_t t0 = { 0 };

	for (i = 0; i < 4; i++)
		rtk->ppp_glo.coordTrue[i] = 0.0;

	rtk->ppp_glo.sample = 10.0;

	//for (i = 0; i <= MAXPRNGLO; i++)
	//    rtk->ppp_glo.glo_frq[i] = 0;

	rtk->ppp_glo.errRatio_BDS = 300;
	rtk->ppp_glo.errRatio_GLO = 600;
	rtk->ppp_glo.errRatio_GAL = 300;

	for (i = 0; i < MAXSAT; i++)
	{
		for (f = 0; f < NFREQ; f++)
			rtk->ppp_glo.dCsAdd[i][f] = 0.0;

		rtk->ppp_glo.b_unhealthy[i] = false;
		rtk->ppp_glo.gt_unh_0[i].time = 0;
		rtk->ppp_glo.gt_unh_0[i].sec = 0.0;
		rtk->ppp_glo.gt_unh_9[i].time = 0;
		rtk->ppp_glo.gt_unh_9[i].sec = 0.0;
	}

	//rtk->ppp_glo.nMaxEpLoss_csDetect = 3;

	for (f = 0; f < NFREQ; f++)
	{
		rtk->ppp_glo.bFqUsed_GPS[f] = false;
		rtk->ppp_glo.bFqUsed_GLO[f] = false;
		rtk->ppp_glo.bFqUsed_BDS[f] = false;
		rtk->ppp_glo.bFqUsed_GAL[f] = false;

		rtk->ppp_glo.clkJump[f] = 0.0;
	}
	rtk->ppp_glo.bFqUsed_GPS[0] = rtk->ppp_glo.bFqUsed_GPS[1] = rtk->ppp_glo.bFqUsed_GPS[2] = true;
	rtk->ppp_glo.bFqUsed_GLO[0] = rtk->ppp_glo.bFqUsed_GLO[1] = true;
	rtk->ppp_glo.bFqUsed_BDS[0] = rtk->ppp_glo.bFqUsed_BDS[1] = rtk->ppp_glo.bFqUsed_BDS[2] = true;
	rtk->ppp_glo.bFqUsed_GAL[0] = rtk->ppp_glo.bFqUsed_GAL[1] = true;

	//rtk->ppp_glo.nEp_block;
	//double  dMin_trop;

	rtk->ppp_glo.sr = 0.0;
	rtk->ppp_glo.ratio = 0.0;
	rtk->ppp_glo.stat = SOLQ_NONE;
	rtk->ppp_glo.nFixed = 0;

	obsEquInfo_t obsEquInfo0 = { 0 };
	equStat_t equStat0 = { 0 };
	rtk->ppp_glo.oEI = (obsEquInfo_t*)calloc(sizeof(obsEquInfo_t), MAXSAT * NFREQ);
	rtk->ppp_glo.equS = (equStat_t*)calloc(sizeof(equStat_t), MAXSAT * NFREQ);
	for (i = 0; i < MAXSAT * NFREQ; i++) {
		rtk->ppp_glo.oEI[i] = obsEquInfo0;
		rtk->ppp_glo.equS[i] = equStat0;
		if (i < MAXSAT)rtk->ppp_glo.bUsed_sat[i] = false;
	}

	rtk->ppp_glo.soltime_ppp = t0;

	rtk->ppp_glo.nmFmt[0] = '\0';
	rtk->ppp_glo.antType[0] = '\0';

	//rtk->ppp_glo.bPrcDir = false;

	for (i = 0; i < 10; i++)
	{
		rtk->ppp_glo.bSatSysObserved[i] = false;
	}

	// added by jxy
	rtk->ppp_glo.delEp = 1;

	rtk->ppp_glo.obsfreq[0][0] = 1;
	rtk->ppp_glo.obsfreq[0][1] = 2;
	rtk->ppp_glo.obsfreq[0][2] = 5;

	rtk->ppp_glo.obsfreq[1][0] = 1;
	rtk->ppp_glo.obsfreq[1][1] = 2;
	rtk->ppp_glo.obsfreq[1][2] = 3;

	rtk->ppp_glo.obsfreq[2][0] = 1;
	rtk->ppp_glo.obsfreq[2][1] = 5;
	rtk->ppp_glo.obsfreq[2][2] = 7;

	rtk->ppp_glo.obsfreq[3][0] = 2;
	rtk->ppp_glo.obsfreq[3][1] = 6;
	rtk->ppp_glo.obsfreq[3][2] = 7;
}
extern void psd_ion_init(rtk_t* rtk) {
	int i = 0;
	psd_ion_t psd0 = { 0 };
	rtk->psd = (psd_ion_t*)calloc(sizeof(psd_ion_t), MAXSAT + 1);
	for (i = 0; i < MAXSAT + 1; i++)rtk->psd[i] = psd0;
}
...
extern void rtkinit(rtk_t* rtk, const prcopt_t* opt, const imup_t* imup)
{
	...
	rtk->tc = 0, rtk->stc = 0;
	if (opt->mode == PMODE_STC_PPK || opt->mode == PMODE_STC_PPP) {/*semi-tightly coupled*/
		rtk->stc = 1;
		if (opt->mode == PMODE_STC_PPK) gnss_opt.mode = PMODE_KINEMA;
		else if (opt->mode == PMODE_STC_PPP) gnss_opt.mode = PMODE_PPP_KINEMA;
		inskf_init(rtk->ins_kf, imup, &insopt);
	}
	else if (opt->mode >= PMODE_TC_SPP && opt->mode <= PMODE_TC_PPP) { /*tightly coupled*/
		rtk->tc = 1;
		inskf_init(rtk->ins_kf, imup, &insopt);
	}
	else if (opt->mode >= PMODE_INS_MECH && opt->mode <= PMODE_LC_PPP) { /*loosely coupled*/
		if (opt->mode == PMODE_LC_SPP) gnss_opt.mode = PMODE_SINGLE;
		else if (opt->mode == PMODE_LC_DGPS) gnss_opt.mode = PMODE_DGPS;
		else if (opt->mode == PMODE_LC_PPK) gnss_opt.mode = PMODE_KINEMA;
		else if (opt->mode == PMODE_LC_PPP) gnss_opt.mode = PMODE_PPP_KINEMA;
		inskf_init(rtk->ins_kf, imup, &insopt);
	}
	if (opt->mode == PMODE_LC_POS) return;

	rtk->opt.pcvr[0] = rtk->opt.pcvr[1] = pcv0;
	rtk->filter_start.time = 0.0; rtk->filter_start.sec = 0.0;
	rtk->epoch = 0;
	rtk->fix_epoch = 0;
	rtk->cannotfix = 0;
	rtk->abnormal_epoch = 0;
	for (i = 0; i < 2 * NSYS; i++) rtk->wl_fix_epoch[i] = 0.0;
	rtk->del_ep = 0;
	rtk->sol = sol0;
	rtk->sol.ns = 0;
	...
	for (i = 0; i < 6; i++) rtk->rb[i] = rtk->opt.rb[i] = opt->rb[i], rtk->opt.ru[i] = opt->ru[i];
	rtk->tt = 0.0;
	rtk->nfix = rtk->neb = 0;
	if (opt->mode >= PMODE_TC_SPP && opt->mode <= PMODE_TC_PPP) { /*tightly coupled*/
		...
		//rtk->na = gnss_opt.mode <= PMODE_FIXED ? NX(&gnss_opt) : pppnx(&gnss_opt);
		rtk->x = rtk->xu = zeros(rtk->nx, 1);
		rtk->P = rtk->Pu = zeros(rtk->nx, rtk->nx);
		rtk->xs = zeros(rtk->nx, 1);
		rtk->Ps = zeros(rtk->nx, rtk->nx);

		//rtk->x  = zeros(rtk->nx, 1);
		//rtk->P  = zeros(rtk->nx, rtk->nx);
		//rtk->xu = zeros(rtk->nx, 1);
		//rtk->Pu = zeros(rtk->nx, rtk->nx);
		rtk->xa = zeros(rtk->na, 1);
		rtk->Pa = zeros(rtk->na, rtk->na);
		//rtk->Psd = zeros(rtk->na, rtk->na);
	}
	for (i = 0; i < MAXSAT; i++) {
		rtk->ambc[i] = ambc0;
		rtk->ssat[i] = ssat0;
		rtk->sdamb[i].fix_epoch[0] = 0;
		rtk->sdamb[i].fix_epoch[1] = 0;
	}
	rtk->holdamb = 0;
	rtk->excsat = 0;
	rtk->nb_ar = 0;
	...
	rtk->opt = gnss_opt;
	for (i = 0; i < (NFREQ + NEXOBS); i++)rtk->clk_jump[i] = 0;

	initppp_global(rtk);
	rtk->ppp_glo.sample = opt->sample;

	//rtk->ppp_glo.navsys = cfg.iSys;
	rtk->ppp_glo.navsys = opt->navsys;
	//rtk->ppp_glo.bEpeSet = cfg.bEpeSet;
	//rtk->ppp_glo.bEpsSet = cfg.bEpsSet;
	//rtk->ppp_glo.ep_end = cfg.ep_end;
	//rtk->ppp_glo.ep_start = cfg.ep_start;
	//rtk->ppp_glo.bTsSet = cfg.bTsSet;
	//rtk->ppp_glo.bTeSet = cfg.bTeSet;
	//rtk->ppp_glo.gt_end = cfg.gt_end;
	//rtk->ppp_glo.gt_start = cfg.gt_start;
	//rtk->opt_ex.tPrcUnit = cfg.dSamplePrc;
	rtk->ppp_glo.bCsEst = opt->ionoopt == 6 ? 1 : 0;// cfg.bCsEst;
	//rtk->ppp_glo.nMaxEpLoss_csDetect = cfg.nMaxEpLoss_csDetect;
	//prcopt.maxout = cfg.nMaxEpLoss_csDetect;

	rtk->ppp_glo.errRatio_GPS = 100;// cfg.errRatio[0];
	rtk->ppp_glo.errRatio_GLO = 500;//cfg.errRatio[1];
	rtk->ppp_glo.errRatio_GAL = 300;//cfg.errRatio[2];
	rtk->ppp_glo.errRatio_BDS = 600;//cfg.errRatio[3];

	rtk->ppp_glo.err_cp[0] = 10.0 / 1000;//G
	rtk->ppp_glo.err_cp[1] = 30.0 / 1000;//R
	rtk->ppp_glo.err_cp[2] = 40.0 / 1000;//E
	rtk->ppp_glo.err_cp[3] = 50.0 / 1000;//C

	psd_ion_init(rtk);
	ssat_ex_t   ssat_Ex0 = { 0 };
	arcInfo_t  arcInf0 = { 0 };
	gtime_t t0 = { 0 };
	...
	for (f = 0; f < MAXSAT; f++)
	{
		// jxy 20171230 kf_ion
		/*
		if (rtk->ssat_ex[f].kf_ion.vec.size())
			rtk->ssat_ex[f].kf_ion.vec.clear();
		*/
		rtk->ssat_ex[f] = ssat_Ex0;
		rtk->ssat_ex[f].arcInf = arcInf0;
	}

	for (i = 0; i < MAXSAT; i++)
	{
		rtk->ssat_ex[i].eWl = 0.0;

		//Panda 20180301
		rtk->ssat_ex[i].mwAve = 0.0;
		rtk->ssat_ex[i].mwVar_c = 0.25;
		rtk->ssat_ex[i].nC = 0;

		rtk->ssat_ex[i].beta_deg = 0.0;
		rtk->ssat_ex[i].mu_deg = 0.0;
		rtk->ssat_ex[i].gt_last = t0;
	}
	/*init ssat_ex*/
	int nmax = 600.0 / (double)opt->sample + 1;
	for (int f = 0; f < MAXSAT; f++)
	{
		rtk->ssat_ex[f].kf_ion.vec = (ion_base_t*)calloc(sizeof(ion_base_t), nmax + 1);
		rtk->ssat_ex[f].kf_ion.nmax = nmax;
		rtk->ssat_ex[f].kf_ion.n = 0;
	}

	prcopt_ex_t prcopt_ex_0 = { 0 };
	rtk->opt_ex = prcopt_ex_0;
	rtk->opt_ex.bUsed_llCs = true;
	rtk->opt_ex.bUsed_gfCs = true;
	rtk->opt_ex.bUsed_mwCs = true;
	rtk->opt_ex.bUsed_resiQc = true;
	rtk->opt_ex.csThresGF = 0.0;
	rtk->opt_ex.csThresMW = 0.0;
	rtk->opt_ex.elMin = opt->elmin;
	if (gnss_opt.sample == 0.0)gnss_opt.sample = 1.0;
	if (fabs(rtk->opt_ex.csThresGF) < 0.01 || fabs(rtk->opt_ex.csThresMW) < 0.01)calCsThres(rtk, 1.0);
	rtk->opt_ex.bres = true;
...
extern void rtkfree(rtk_t* rtk)
{
	...
	free(rtk->xs); rtk->xs = NULL;
	free(rtk->Ps); rtk->Ps = NULL;

	free(rtk->ppp_glo.oEI); rtk->ppp_glo.oEI = NULL;
	free(rtk->ppp_glo.equS); rtk->ppp_glo.equS = NULL;
	free(rtk->psd); rtk->psd = NULL;

	for (int i = 0; i < MAXSAT; i++)
	{
		free(rtk->ssat_ex[i].kf_ion.vec);
		rtk->ssat_ex[i].kf_ion.vec = NULL;
		rtk->ssat_ex[i].kf_ion.nmax = 0;
		rtk->ssat_ex[i].kf_ion.n = 0;
	}
}
/* precise positioning ---------------------------------------------------------
...
extern int rtkpos(rtk_t* rtk, obsd_t* obs, int n, const nav_t* nav)
{
	...
	//obsd_t *obsd = (obsd_t *)calloc(sizeof(obsd_t)*MAXOBS);
	//if (opt->adjobs) {
