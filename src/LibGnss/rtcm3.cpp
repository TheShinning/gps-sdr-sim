...
static int decode_ssr1(rtcm_t* rtcm, int sys, int subtype)
{
	...
	for (j = 0; j < nsat && i + 121 + np + ni + nj <= rtcm->len * 8; j++) {
		...
		if (sys == SYS_CMP)
		{
			rtcm->ssr[sat - 1].iode = iodcrc != 0 ? iodcrc : iode;
		}
		for (k = 0; k < 3; k++) {
...
static int decode_ssr3(rtcm_t* rtcm, int sys, int subtype)
{
	...
	for (j = 0; j < nsat && i + 5 + np <= rtcm->len * 8; j++) {
		...
		sprintf(rtcm->ssr[sat - 1].prn, "%s%2d", satno2sys(sat), prn);
		for (k = 0; k < MAXCODE; k++) {
			rtcm->ssr[sat - 1].cbias[k] = (float)cbias[k];
		}
		rtcm->ssr[sat - 1].update = 1;
		rtcm->ssr[sat - 1].ncbias = nbias;
	}
	return sync ? 0 : 10;
}
...
static int decode_ssr7(rtcm_t* rtcm, int sys, int subtype)
{
	...
	int ph_swl[MAXCODE], ph_sii[MAXCODE], ph_sdc[MAXCODE];
	...
	for (j = 0; j < nsat && i + 5 + 17 + np <= rtcm->len * 8; j++) {
		...
		for (k = 0; k < MAXCODE; k++)ph_swl[k] = ph_sii[k] = ph_sdc[k] = pbias[k] = stdpb[k] = 0.0;
		for (k = 0; k < nbias && i + ((subtype == 0) ? 49 : 32) <= rtcm->len * 8; k++) {
			...
			if (sigs[mode]) {
				...
				ph_swl[sigs[mode] - 1] = swl;
				ph_sii[sigs[mode] - 1] = sii;
				ph_sdc[sigs[mode] - 1] = sdc;
			}
			else {
				trace(2, "rtcm3 %d not supported mode: mode=%d\n", type, mode);
			}
		...
		rtcm->ssr[sat - 1].yaw_ang = yaw_ang / 256.0 * 180.0; /* (deg) *//* the computation of phase wind-up correction*/
		rtcm->ssr[sat - 1].yaw_rate = yaw_rate / 8192.0 * 180.0; /* (deg/s) */

		for (k = 0; k < MAXCODE; k++) {
			rtcm->ssr[sat - 1].pbias[k] = pbias[k];
			rtcm->ssr[sat - 1].stdpb[k] = (float)stdpb[k];
			rtcm->ssr[sat - 1].sii[k] = ph_sii[k];
			rtcm->ssr[sat - 1].swl[k] = ph_swl[k];
			rtcm->ssr[sat - 1].sdc[k] = ph_sdc[k];
		}
		rtcm->ssr[sat - 1].npbias = nbias;
	}
	return 20;
}
/* decode SSR 8 VTEC -----------------------------------------------*/
static int decode_ssr8_head(rtcm_t* rtcm, int subtype, int* sync,
	int* iod, double* udint, double* vtecqc, int* num_vteclay,
	int* hsize)
{
	char* msg, tstr[64];
	int i = 24 + 12, nsat = 0, udi, provid = 0, solid = 0;
	...
	//	if (i + 3 + 8 + 51 + ns > rtcm->len * 8) return -1;
	//}
	i = decode_ssr_epoch(rtcm, 0, subtype);//20
	udi = getbitu(rtcm->buff, i, 4); i += 4;//SSR Update Interval
	*sync = getbitu(rtcm->buff, i, 1); i += 1;//SSR Multiple Message Indicator
	*iod = getbitu(rtcm->buff, i, 4); i += 4;//IOD SSR
	provid = getbitu(rtcm->buff, i, 16); i += 16; /* provider ID */
	solid = getbitu(rtcm->buff, i, 4); i += 4; /* solution ID */
	*vtecqc = getbitu(rtcm->buff, i, 9); i += 9; /* VTEC Quality Indicator */
	*num_vteclay = getbitu(rtcm->buff, i, 2); i += 2; /* MW consistency indicator */
	//nsat = getbitu(rtcm->buff, i, ns); i += ns;
	*udint = ssrudint[udi];

	time2str(rtcm->time, tstr, 2);
	trace(4, "decode_ssr8_head: time=%s subtype=%d  sync=%d iod=%d"
		" provid=%d solid=%d\n", tstr, subtype, *sync, *iod, provid, solid);

	if (rtcm->outtype) {
		msg = rtcm->msgtype + strlen(rtcm->msgtype);
		sprintf(msg, " %s iod=%2d udi=%2d sync=%d", tstr, *iod, udi,
			*sync);
	}
	*hsize = i;
	return *num_vteclay;
...
static int decode_ssr8(rtcm_t* rtcm, int sys, int subtype)
{
	...
	double udint, bias, std = 0.0, pbias[MAXCODE], stdpb[MAXCODE];
	int ph_swl[MAXCODE], ph_sii[MAXCODE], ph_sdc[MAXCODE];
	double he = 0.0, vtecqc = 0.0;
	int deg = 0, ord = 0;
	int i, j, k, m, type, mode, sync, iod, nsat, prn, sat, nbias, np, mw, offp, sii, swl, num_vteclay;
	int dispe, sdc, yaw_ang, yaw_rate;

	type = getbitu(rtcm->buff, 24, 12);
	double a[30] = { 0 }; int inx = 24 + 12;
	a[0] = getbitu(rtcm->buff, inx, 3); inx += 3;
	a[1] = getbitu(rtcm->buff, inx, 8); inx += 8;
	a[2] = getbitu(rtcm->buff, inx, 20); inx += 20;
	a[3] = getbitu(rtcm->buff, inx, 4); inx += 4;
	a[4] = getbitu(rtcm->buff, inx, 1); inx += 1;
	a[5] = getbitu(rtcm->buff, inx, 4); inx += 4;
	a[6] = getbitu(rtcm->buff, inx, 16); inx += 16;
	a[7] = getbitu(rtcm->buff, inx, 4); inx += 4;
	a[8] = getbitu(rtcm->buff, inx, 9); inx += 9;
	num_vteclay = a[9] = getbitu(rtcm->buff, inx, 2); inx += 2;
	//if ((nsat = decode_ssr8_head(rtcm, subtype, &sync, &iod, &udint, &vtecqc, &num_vteclay,
	//	&i)) < 0) {
	//	trace(2, "rtcm3 %d length error: len=%d\n", type, rtcm->len);
	//	return -1;
	//}
	...
	double cosms[16][16] = { {0} }, sinms[16][16] = { {0} };
	i = inx;
	for (m = 0; m < num_vteclay; m++)
	{
		he = getbitu(rtcm->buff, i, 8) * 10000; i += 8;
		deg = getbitu(rtcm->buff, i, 4); i += 4;
		ord = getbitu(rtcm->buff, i, 4); i += 4;
		for (j = 0; j < ord; j++)
		{
			for (k = 0; k < deg; k++)
			{
				cosms[k][j] = getbitu(rtcm->buff, i, 16) * 0.005; i += 16;
			}
		}
		for (j = 0; j < ord; j++)
		{
			for (k = 0; k < deg; k++)
			{
				sinms[k][j] = getbitu(rtcm->buff, i, 16) * 0.005; i += 16;
			}
		}
	}

	rtcm->ssr[0].rt_vtec.n = num_vteclay;
	rtcm->ssr[0].rt_vtec.update = 1;
...
static int decode_type4076(rtcm_t* rtcm)
{
	...
	case 127: return decode_ssr5(rtcm, SYS_SBS, subtype);
		//case 202: return decode_ssr8(rtcm, 0, subtype);//VTEC not supported
	}
...
extern int decode_rtcm3(rtcm_t* rtcm)
{
	...
	if (type >= 1265 && type <= 1270)
	{
		type = type;
	}
	if (type == 1265)type = 11;//GPS
	if (type == 1267)type = 12;//GAL
	if (type == 1270)type = 14;//BDS
	...
	//case 1264: ret = decode_ssr8(rtcm, 0, 0); break;//VTEC
