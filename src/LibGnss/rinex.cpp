	...
	char tobs[][MAXOBSTYPE][4], nav_t *nav, sta_t *sta)
{
	...
	if (strstr(label, "MARKER NAME")) {
		if (sta) setstr(sta->name, buff, 4/*60*/);
	}
	...
	sigind_t *index, obsd_t *obs)
{
	...
	if (ver > 2.99) { /* ver.3 */
		...
		if (sys == SYS_CMP && prn > 18)sys = SYS_BD3;//BDS3
	...
	case SYS_BD3: ind = index + 7; break;
	...
	char tobs[MAXOBSTYPE][4], sigind_t *ind)
{
	...
	for (i = n = 0; *tobs[i]; i++, n++) {
		...
		ind->idx[i] = code2idx(sys, ind->code[i]);//BDS3
		ind->pri[i] = getcodepri(sys, ind->code[i], opt);//BDS3
		ind->pos[i] = -1;
	}
	...
	case SYS_BD3: optstr = "-CL%2s=%lf"; break;
	...
	for (i = 0; i < NFREQ+(sys == SYS_CMP?1: sys == SYS_BD3 ? NEXOBS :0 ); i++) {
		for (j = 0, k = -1; j < n; j++) {
	...
	for (i = 0; i < NEXOBS; i++) {
		if(sys==SYS_CMP|| sys == SYS_BD3)break;
		for (j = 0; j < n; j++) {
		...
		for (k = 0; k < n; k++) {
			...
			{				
				if (0&&sys == SYS_BD3) {
					if (tobs[j][1] == '1')ind->pos[k] = NFREQ + 2;//B1C
					else if (tobs[j][1] == '6')ind->pos[k] = NFREQ ;//B3
					else if (tobs[j][1] == '7'&&(tobs[j][2] == 'D'|| tobs[j][2] == 'P'|| tobs[j][2] == 'Z'))ind->pos[k] = NFREQ + 3;//B2b
				}
				else
				{
					ind->pos[k] = NFREQ + i/* - 1*/;
...
//    mask=set_sysmask(opt);
	...
	if (nsys >= 6) set_index(ver, SYS_BD3, opt, tobs[5], index + 7);//BDS3
	...
	char tobs[][MAXOBSTYPE][4], obs_t *obs, sta_t *sta)
{
	...
	{
		i = i;
		trace(2, "readrnxobs error: nobs=%d stat=%d\n", obs->n, stat);
	}
	free(data);
	...
	nav_t *nav)
{
	...
	{
		stat = stat;
		trace(2, "readrnxnavb error: nobs=%d stat=%d\n", nav->n, stat);
	}
	...
	sta_t *sta)
{
	...
	for (i = 0; i < n&&stat >= 0; i++) {
		stat = readrnxfile(popt,file /*files[i]*/, ts, te, tint, opt, 0, rcv, &type, obs, nav, sta);
	}
...
extern int readrnxc(const prcopt_t *popt, const char *file, nav_t *nav)
{
	...
	/* read rinex clock files */
	for (i = 0; i < n; i++) {
		if (readrnxfile(popt, file/*files[i]*/, t, t, 0.0, "", 1, index++, &type, NULL, nav, NULL)) {
			continue;
		}
		stat = 0;
		break;
