	...
	int* svh)
{
	...
	*svh = 0;
	...
	case EPHOPT_PREC:
		if (!peph2pos(popt, time, sat, nav, 1, rs, dts,atts, var)) break; else return 1;
	}
	*svh = -1;
	return 0;
}
...
extern void satposs(const prcopt_t* popt, gtime_t teph, const obsd_t* obs, int n, const nav_t* nav,
	int ephopt, double* rs, double* dts, double* atts, double* var, int* svh)
{
	...
	for (i = 0; i < n && i < 2 * MAXOBS; i++) {
		...
		var[i] = 0.0; svh[i] = 0;
		if (SYS_CMP == satsys(obs[i].sat, &prn0)) {
			i = i;
		}
		...
		if (!satpos(popt, time[i], teph, obs[i].sat, ephopt, nav, rs + i * 6, dts + i * 2,atts+i*9, var + i, svh + i)) {
