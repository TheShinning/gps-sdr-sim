...
static int fixsol(rtk_t* rtk, const double* v, const double* H, int nv)
{
    ...
    if (!(info = filter(rtk->x, rtk->P, H, v, R, rtk->nx, nv,rtk->opt.robust, rtk->opt.kfopt, 0, 0))) {
        /* set solution */
        for (i = 0; i < rtk->na; i++) {
...
extern int resamb_WLNL0(rtk_t* rtk, const obsd_t* obs, const int* sat,
    const int* iu, const int* ir, int ns, const nav_t* nav,
    const double* azel)
{
    ambc_t* ambi, * ambj;
    ...
    trace(3, "resamb_WLNL0: time=%s ns=%d\n", time_str(obs[0].time, 0), ns);

    lam_N = lam_LC(1, 1, 0);//Õ­Ïï N1+N2
    lam_W = lam_LC(1, -1, 0);//¿íÏï N1-N2
...
//IF model
extern int resamb_TCAR0(rtk_t* rtk, const obsd_t* obs, const int* sat,
    const int* iu, const int* ir, int ns, const nav_t* nav,
    const double* azel)
{
    ambc_t* ambi, * ambj;
