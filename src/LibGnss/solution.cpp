...
static int decode_solstat(char* buff, solstat_t* stat)
{
	...
	double tow, az, el, resp, resc, snr, amb, fix_amb, var_amb, pri_cp, pri_pr, fix_cp, fix_pr, nv_cp, nv_pr, nv_pri_cp, nv_pri_pr, ins_inno, cs_thres, ins_wl_res, ins_ewl_res;
	...
	n = sscanf(buff, "$SAT%d%lf%s%d%lf%lf%lf%lf%d%lf%d%d%d%d%d%d%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf",
		&week, &tow, id, &frq, &az, &el, &resp, &resc, &vsat, &snr, &fix, &slip,
		&lock, &outc, &slipc, &rejc, &amb, &fix_amb, &var_amb, &pri_cp, &pri_pr, &fix_cp, &fix_pr, &nv_cp, &nv_pr, &nv_pri_cp, &nv_pri_pr, &ins_inno, &cs_thres, &ins_wl_res, &ins_ewl_res);
	...
	*stat = stat0;
	...
	stat->amb = (float)amb;
	stat->fix_amb = (float)fix_amb;
	stat->var_amb = (float)var_amb;
	stat->pri_cp = (float)pri_cp;
	stat->pri_pr = (float)pri_pr;
	stat->fix_cp = (float)fix_cp;
	stat->fix_pr = (float)fix_pr;
	stat->nv_cp = (float)nv_cp;
	stat->nv_pr = (float)nv_pr;
	stat->nv_pri_cp = (float)nv_pri_cp;
	stat->nv_pri_pr = (float)nv_pri_pr;
	stat->ins_inno = (float)ins_inno;
	stat->cs_thres = (float)cs_thres;
	stat->ins_wl_res = (float)ins_wl_res;
	stat->ins_ewl_res = (float)ins_ewl_res;
	return 1;
}
...
static void readref_ygm_avp(const char* file, solbuf_t* solbuff, int week, int type, double sow)
{
	int i = 0, n;
	double sec, pos[3] = { 0 }, vn[3] = { 0 };
	FILE* fp;
	...
	while (fgets(buff, MAXSOLMSG, fp)) {
		...
		if (i + 3 < n + 1) {
			...
			pos[2] = v[i++];
			v3_t rn;
			for (int k = 0; k < 3; k++) rn.v[k] = pos[k];
			//            pos2ecef(pos,sol.rr);
			llh2ecef(&rn, NULL, NULL, INSLOCAL_ENU);
			for (int k = 0; k < 3; k++) sol.rr[k] = rn.v[k];
		}
		enu2ecef(pos, vn, sol.rr + 3);

		if (type == REF_SOL_FMT_YGM_AVPED) {
			if (i + 3 < n) {
				sol.bg[0] = v[i++] * RPS2DPH;
				sol.bg[1] = v[i++] * RPS2DPH;
				sol.bg[2] = v[i++] * RPS2DPH;
			}
			if (i + 3 < n + 1) {
				sol.ba[0] = v[i++] * MPS22MG;
				sol.ba[1] = v[i++] * MPS22MG;
...
extern int readref(const char* file, solbuf_t* sol, const solopt_t* sopt, int week, double sow)
{
	...
	else if (sopt->ref_type == REF_SOL_FMT_YGM_AVP) {
		readref_ygm_avp(file, sol, week, REF_SOL_FMT_YGM_AVP, sow);
	}
	else if (sopt->ref_type == REF_SOL_FMT_YGM_AVPED) {
		readref_ygm_avp(file, sol, week, REF_SOL_FMT_YGM_AVPED, sow);
	}
	return sol->n;
}
	...
	const solopt_t* opt, const prcopt_t* popt)
{
	...
	p += sprintf(p, "%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d"
		"%s%6.2f%s%6.1f%s%6.1f",
		s, sep, sol->rr[0], sep, sol->rr[1], sep, sol->rr[2], sep, sol->stat, sep,
		sol->ns, sep, sol->age, sep, sol->ratio, sep, sol->dop[1]);
	p += sprintf(p, "%s%6d%s%6d%s%6d%s%6d%s%6d",
		sep, sol->ns_sys[0], sep, sol->ns_sys[1], sep, sol->ns_sys[2], sep, sol->ns_sys[3], sep, sol->ns_sys[4]);
	...
	if (0)
	{
		for (int at = 0; at < 32; at++) {
			p += sprintf(p, "%s%10.5f", sep, sol->ion[at]);
		}
	}
	...
	const solopt_t* opt, const prcopt_t* popt)
{
	...
	//	"%s%6.2f%s%6.1f%s%4.1f",
	//	sep, pos[2], sep, sol->stat, sep, sol->ns, sep, SQRT(Q[4]), sep,
	//	SQRT(Q[0]), sep, SQRT(Q[8]), sep, sqvar(Q[1]), sep, sqvar(Q[2]),
	//	sep, sqvar(Q[5]), sep, sol->age, sep, sol->ratio, sep, sol->dop[1]);
	p += sprintf(p, "%s%10.4f%s%3d%s%3d"
		"%s%6.2f%s%6.1f%s%6.1f",
		sep, pos[2], sep, sol->stat, sep, sol->ns, sep, SQRT(Q[4]),
		sep, sol->age, sep, sol->ratio, sep, sol->dop[1]);
	p += sprintf(p, "%s%6d%s%6d%s%6d%s%6d%s%6d",
		sep, sol->ns_sys[0], sep, sol->ns_sys[1], sep, sol->ns_sys[2], sep, sol->ns_sys[3], sep, sol->ns_sys[4]);
...
static int outenu(uint8_t* buff, const char* s, const sol_t* sol,
	const double* rb, const solopt_t* opt, const prcopt_t* popt)
{
	...
	covenu(pos, P, Q);
	ecef2enu(pos, rr, enu);
	//p += sprintf(p, "%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%4.1f%s%8.4f%s%8.4f%s%8.4f%s"
	//	"%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f%3d%s",
	//	s, sep, enu[0], sep, enu[1], sep, enu[2], sep, sol->stat, sep, sol->ns, sep,
	//	SQRT(Q[0]), sep, SQRT(Q[4]), sep, SQRT(Q[8]), sep, sqvar(Q[1]),
	//	sep, sqvar(Q[5]), sep, sqvar(Q[2]), sep, sol->age, sep, sol->ratio, sol->dop[1], sep);
	p += sprintf(p, "%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s"
		"%6.2f%s%6.1f%s%6.1f",
		s, sep, enu[0], sep, enu[1], sep, enu[2], sep, sol->stat, sep, sol->ns, sep,
		sol->age, sep, sol->ratio, sep, sol->dop[1]);
	p += sprintf(p, "%s%6d%s%6d%s%6d%s%6d%s%6d",
		sep, sol->ns_sys[0], sep, sol->ns_sys[1], sep, sol->ns_sys[2], sep, sol->ns_sys[3], sep, sol->ns_sys[4]);
	...
	if (opt->outtrp) {
		...
		{
			p += sprintf(p, "%s%12.2f", sep, sol->ztrp[2]);
		}
	}
	if (popt->nf >= 3 && (popt->ionoopt == IONOOPT_UC || popt->ionoopt == IONOOPT_UC_CONS)) {
		if (popt->navsys & SYS_GPS) p += sprintf(p, "%s%12.2f", sep, sol->ifcb[0] * 1E9);
	...
	{
		int It = 0;
		for (It = 0; It < MAXSAT; It++)
		{
			p += sprintf(p, "%s%12.2f%s%12.2f%s%12.2f", sep, sol->ion[It], sep, sol->qv_ion[It], sep, sol->obs_ion[It]);/* IRN clock offset ns*/
			sol->ion[It];
			sol->qv_ion[It];
			sol->obs_ion[It];
		}
...
static int outenu_ion(uint8_t* buff, const char* s, sol_t* sol,
	const double* rb, const solopt_t* opt, const prcopt_t* popt)
{
	double pos[3], rr[3], enu[3], P[9], Q[9];
	int i;
	...
	if (1)
	{
		...
		{
			satsys(It, &prn);
			p += sprintf(p, "%s   PRN%d%s%12.5f%s%12.5f", sep, prn, sep, sol->ion[It], /*sep, sol->qv_ion[It], sep, sol->obs_ion[It],*/sep, sol->elve_ion[It]);/* IRN clock offset ns*/
			sol->ion[It] = 0;
			sol->qv_ion[It] = 0;
			sol->obs_ion[It] = 0;
			sol->elve_ion[It] = 0;
...
extern int outinssol(uint8_t* buff, const char* s, const sol_t* sol,
	const solopt_t* opt, const prcopt_t* popt, const solins_t* ins_sol)
{
	...
	char* p = (char*)buff;
	int ins = 0;
	solins_t outins = *ins_sol;
	attsync(&outins.pos, &outins.rpy, &outins.dcm, &outins.quat, popt->insopt.imu_coord, popt->insopt.local_coord, popt->insopt.mech_coord, 1);
	...
	//todo check
/*    v3_t llh; llh.x=sol->rr[0];llh.y=sol->rr[1];llh.z=sol->rr[2];
	ecef2llh(&llh,NULL,NULL,opt);
	pos[0] = llh.x; pos[1] = llh.y; pos[2] = llh.z;*/
	...
	if (opt->posf == SOLF_INS_LLH) {
		...
		p += sprintf(p, "%s%10.4f%s%3d%s%3d%s%4.1f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f"
			"%s%6.2f%s%6.1f%s%8.4f%s%4d%s%6.3f%s%6.1f",
			sep, pos[2], sep, sol->stat, sep, sol->ns, sep, sol->dop[1], sep, SQRT(Q[4]), sep,
			SQRT(Q[0]), sep, SQRT(Q[8]), sep, sqvar(Q[1]), sep, sqvar(Q[2]),
			sep, sqvar(Q[5]), sep, sol->age, sep, sol->ratio, sep, sol->adop, sep, sol->ar_nb, sep, sol->ps, sep, sol->thres);

		if (opt->outvel) { /* output velocity */
			soltocov_vel(sol, P);
			ecef2enu(pos, sol->rr + 3, vel);
			covenu(pos, P, Q);/*NED*/
			if (popt->insopt.local_coord == INSLOCAL_NED) {
				p += sprintf(p, "%s%10.5f%s%10.5f%s%10.5f%s%9.5f%s%8.5f%s%8.5f%s%8.5f%s"
					"%8.5f%s%8.5f",
					sep, vel[1], sep, vel[0], sep, -vel[2], sep, SQRT(Q[4]), sep,
					SQRT(Q[0]), sep, SQRT(Q[8]), sep, sqvar(Q[1]), sep, sqvar(Q[2]),
				...
				p += sprintf(p, "%s%10.5f%s%10.5f%s%10.5f%s%9.5f%s%8.5f%s%8.5f%s%8.5f%s"
					"%8.5f%s%8.5f",
					sep, vel[0], sep, vel[1], sep, vel[2], sep, SQRT(Q[4]), sep,
					SQRT(Q[0]), sep, SQRT(Q[8]), sep, sqvar(Q[1]), sep, sqvar(Q[2]),
					sep, sqvar(Q[5]));
			}
		}
		...
		p += sprintf(p, "%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%4.1f%s%8.4f%s%8.4f%s%8.4f%s"
			"%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f%s%8.4f%s%4d%s%6.3f%s%6.1f",
			s, sep, sol->rr[0], sep, sol->rr[1], sep, sol->rr[2], sep, sol->stat, sep,
			sol->ns, sep, sol->dop[1], sep, SQRT(sol->qr[0]), sep, SQRT(sol->qr[1]), sep,
			SQRT(sol->qr[2]), sep, sqvar(sol->qr[3]), sep, sqvar(sol->qr[4]), sep,
			sqvar(sol->qr[5]), sep, sol->age, sep, sol->ratio, sep, sol->adop, sep, sol->ar_nb, sep, sol->ps, sep, sol->thres);
	...
	if (ins) {
		p += sprintf(p, "%s%3d%s%14.3f%s%14.3f%s%14.3f%s%14.3f%s%14.3f%s%14.3f",
			sep, outins.g_status, sep, outins.rpy.x * R2D, sep, outins.rpy.y * R2D, sep, outins.rpy.z * R2D, sep,
			SQRT(ins_sol->Qatt.m11) * R2D, sep, SQRT(ins_sol->Qatt.m22) * R2D, sep, SQRT(ins_sol->Qatt.m33) * R2D);

		if (popt->insopt.est_ba && opt->outba) {
			p += sprintf(p, "%s%14.3f%s%14.3f%s%14.3f%s%14.3f%s%14.3f%s%14.3f",
				sep, outins.ba.x * MPS22MG, sep, outins.ba.y * MPS22MG, sep, outins.ba.z * MPS22MG, sep,
				outins.ba_std.x * MPS22MG, sep, outins.ba_std.y * MPS22MG, sep, outins.ba_std.z * MPS22MG);
		}
		if (popt->insopt.est_bg && opt->outbg) {
			p += sprintf(p, "%s%14.3f%s%14.3f%s%14.3f%s%14.3f%s%14.3f%s%14.3f",
				sep, outins.bg.x * RPS2DPH, sep, outins.bg.y * RPS2DPH, sep, outins.bg.z * RPS2DPH, sep,
				outins.bg_std.x * RPS2DPH, sep, outins.bg_std.y * RPS2DPH, sep, outins.bg_std.z * RPS2DPH);
		}
		if (popt->insopt.est_sa && opt->outsa) {
			p += sprintf(p, "%s%14.3f%s%14.3f%s%14.3f%s%14.3f%s%14.3f%s%14.3f",
				sep, outins.sa.x, sep, outins.sa.y, sep, outins.sa.z, sep,
				outins.sa_std.x, sep, outins.sa_std.y, sep, outins.sa_std.z);
		}
		if (popt->insopt.est_sg && opt->outsg) {
			p += sprintf(p, "%s%14.3f%s%14.3f%s%14.3f%s%14.3f%s%14.3f%s%14.3f",
				sep, outins.sg.x, sep, outins.sg.y, sep, outins.sg.z, sep,
				outins.sg_std.x, sep, outins.sg_std.y, sep, outins.sg_std.z);
		}
		if (popt->insopt.est_armgps && opt->outarm) {
			p += sprintf(p, "%s%14.3f%s%14.3f%s%14.3f%s%14.3f%s%14.3f%s%14.3f",
				sep, outins.arm_gps.x, sep, outins.arm_gps.y, sep, outins.arm_gps.z, sep,
				outins.arm_gps_std.x, sep, outins.arm_gps_std.y, sep, outins.arm_gps_std.z);
		}
		if (popt->insopt.est_igdt && opt->outigdt) {
			p += sprintf(p, "%s%14.3f%s%14.3f",
				sep, outins.t_delay, sep, 0.0);
		}
	}
...
extern int outinsygm(uint8_t* buff, const char* s, const sol_t* sol,
	const solopt_t* opt, const prcopt_t* popt, const solins_t* ins_sol)
{
	double sow, sow0 = 108143.0, dt = 0.0;
	const char* sep = opt2sep(opt);
	char* p = (char*)buff;
	int ins = 0;
	solins_t outins = *ins_sol;
	...
	sow = time2gpst(ins_sol->time, NULL);
	dt = sow;
	p += sprintf(p, "%20.10G %20.10G %20.10G %20.10G %20.10G %20.10G %20.10G %20.10G %20.10G %20.10G\n",
		outins.rpy.x, outins.rpy.y, outins.rpy.z, outins.vel.x, outins.vel.y, outins.vel.z, outins.pos.x, outins.pos.y, outins.pos.z, dt);
...
extern int outprcopts(uint8_t* buff, const prcopt_t* opt)
{
	static char obsfrqstr[8][MAXFREQ][5] = {
			...
			{"L5", "S",  "",   "",   "",    "", ""},       /*IRN*/
			{"B1I","B2I","B2a","B3I","B2ab","B1C", "B2b"}, /*BDS3*/
	};
	const int sys[] = {
		SYS_GPS,SYS_GLO,SYS_GAL,SYS_QZS,SYS_SBS,SYS_CMP,SYS_IRN,SYS_BD3,0
	};
	const char* s1[] = {
	...
	//"0:off,1:brdc,2:sbas,3:tec,4:IF,5:IF2,6:UC,7:UC-tec,8:qzs-brdc,9:qzs-lex,10:stec"
	const char* s51[] = {
	"OFF","Brdc","Sbas","Tec","IF","IF2","Estimate","Estimate-tec","Qzs-brdc","Qzs-lex","Stec"
	};
	const char* s6[] = {
	...
	const char* s7[] = {
		"GPS","GLONASS","Galileo","QZSS","SBAS","BDS","NavIC","BDS3",""
	};
	const char* s8[] = {
	...
	p += sprintf(p, "%s iono  opt : %s\n", COMMENTH, s51[opt->ionoopt]);
	...
	for (i = 0; sys[i]; i++) {
		if (opt->navsys & sys[i]) {
			...
			else if (sys[i] == SYS_BD3) {
				if (opt->nf == 1) {
					p += sprintf(p, " B3(%s)", obsfrqstr[i][opt->gnss_frq_idx[NSYS][0] - 1]);
				}
				else if (opt->nf == 2) {
					p += sprintf(p, " B3(%s+%s)", obsfrqstr[i][opt->gnss_frq_idx[NSYS][0] - 1], obsfrqstr[i][opt->gnss_frq_idx[NSYS][1] - 1]);
				}
				else if (opt->nf == 3) {
					p += sprintf(p, " B2(%s+%s+%s)", obsfrqstr[i][opt->gnss_frq_idx[NSYS][0] - 1], obsfrqstr[i][opt->gnss_frq_idx[NSYS][1] - 1], obsfrqstr[i][opt->gnss_frq_idx[NSYS][2] - 1]);
				}
			}
		}
	}
...
extern int outsolheads(uint8_t* buff, const solopt_t* opt, const prcopt_t* popt)
{
	...
	if (opt->posf == SOLF_LLH || opt->posf == SOLF_INS_LLH) { /* lat/lon/hgt */
		if (opt->degf) {
			...
			//	sep, "ratio");
			p += sprintf(p, "%16s%s %16s%s %10s%s %3s%s %3s%s",
				"%6s%s%6s%s%6s",
				"latitude(d'\")", sep, "longitude(d'\")", sep, "height(m)",
				sep, "Q", sep, "ns", sep, "sdn(m)", sep, "sde(m)", sep, "sdu(m)",
				sep, "sdne(m)", sep, "sdeu(m)", sep, "sdue(m)", sep, "age(s)",
				sep, "ratio", sep, "pdop");
			p += sprintf(p, "%s%6s%s%6s%s%6s%s%6s%s%6s",
				sep, "ngps", sep, "nglo", sep, "ngal", sep, "nbds2", sep, "nbds3");
		}
		else {
		...
		p += sprintf(p, "%14s%s%14s%s%14s%s%3s%s%3s"
			"%s%6s%s%6s%s%6s",
			"x-ecef(m)", sep, "y-ecef(m)", sep, "z-ecef(m)", sep, "Q", sep, "ns",
			sep, "age(s)", sep, "ratio", sep, "pdop");
		p += sprintf(p, "%s%6s%s%6s%s%6s%s%6s%s%6s",
			sep, "ngps", sep, "nglo", sep, "ngal", sep, "nbds2", sep, "nbds3");
		...
		p += sprintf(p, "%14s%s%14s%s%14s%s%2s%s%3s"
			"%s%6s%s%6s%s%6s",
			"e-baseline(m)", sep, "n-baseline(m)", sep, "u-baseline(m)", sep,
			"Q", sep, "ns", sep, "age(s)", sep,
			"ratio", sep, "pdop");
		p += sprintf(p, "%s%6s%s%6s%s%6s%s%6s%s%6s",
			sep, "ngps", sep, "nglo", sep, "ngal", sep, "nbds2", sep, "nbds3");
	...
	const solopt_t* opt, const prcopt_t* popt, const solins_t* ins_sol)
{
	...
	case SOLF_ENU:  p += outenu(p, s, sol, rb, opt, popt); break;
	case SOLF_NMEA: p += outnmea_rmc(p, sol);
					p += outnmea_gga(p, sol); break;
	case SOLF_INS_LLH:  p += outinssol(p, s, sol, opt, popt, ins_sol); break;
	case SOLF_INS_XYZ:  p += outinssol(p, s, sol, opt, popt, ins_sol); break;
	case SOLF_INS_YGM:  p += outinsygm(p, s, sol, opt, popt, ins_sol); break;

	}
	return p - buff;
}
extern int outsols_ion(uint8_t* buff, sol_t* sol, const double* rb,
	const solopt_t* opt, const prcopt_t* popt, const solins_t* ins_sol)
{
	gtime_t time, ts = { 0 };
	double gpst;
	...
	case SOLF_LLH:  p += outpos(p, s, sol, opt, popt);   break;
	case SOLF_XYZ:  p += outecef(p, s, sol, opt,popt);   break;
	case SOLF_ENU:  p += outenu_ion(p, s, sol, rb, opt, popt); break;
	case SOLF_NMEA: p += outnmea_rmc(p, sol);
		p += outnmea_gga(p, sol); break;
	}
	return p - buff;
	...
	const solopt_t* opt, const prcopt_t* popt, const solins_t* ins_sol)
{
	...
	if ((n = outsols_ion(buff, sol, rb, opt, popt, ins_sol)) > 0) {
		fwrite(buff, n, 1, fp);
	}
}
