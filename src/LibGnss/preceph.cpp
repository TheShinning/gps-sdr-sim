...
{
	peph_t peph/**peph= (peph_t *)calloc(1, sizeof(peph_t))*/;
	gtime_t time;
	double val, std, base;
	...
	while (fgets(buff, sizeof(buff), fp)) {
		...
		if (v) {
			if (!addpeph(nav, &peph)) { /*free(peph);*/ return; }
		}
	}
	/*free(peph);*/
}
...
static int readfcbf(const char* file, nav_t* nav)
{
	...
	nav->fcbs = (fcbs_t*)malloc(sizeof(fcbs_t));
...
extern int readobxf(const char* file, /*obx_t* sat_obx,*/ nav_t* nav)
{
	FILE* fp;
	obx_t* obx_temp;
	char buff[200], satid[8] = "";
	int i,j, code, sat, sys,nsat,nq=4;

	if (!(fp = fopen(file, "r"))) {
		trace(2, "obx parameters file open error: %s\n", file);
		return 0;
	}
	//nav->obx = (obx_t*)malloc(sizeof(obx_t));
	//memset(nav->obx, 0, sizeof(obx_t));
	while (fgets(buff, sizeof(buff), fp)) {
		if (!strncmp(buff, "##", 2)) {
			gtime_t time;
			if ((nsat = (int)str2num(buff, 35, 5)) <= 0) continue;;
			if (str2time(buff, 3, 26, &time)) {
				trace(2, "rinex obx invalid epoch: epoch=%26.26s\n", buff);
				continue;
			}
			if (nav->nb >= nav->nbmax) {
				nav->nbmax += 1024;
				if (!(obx_temp = (obx_t*)realloc(nav->obx, sizeof(obx_t) * (nav->nbmax)))) {
					free(nav->obx);
					nav->obx = NULL;
					nav->nb = nav->nbmax = 0;
					return -1;
			...
			memset(&nav->obx[nav->nb], 0, sizeof(obx_t));
			for ( i = 0; i < nsat; i++)
			{
				fgets(buff, sizeof(buff), fp);
				if (strncmp(buff + 1, "ATT", 3))continue;
				nav->obx[nav->nb ].time = time;
				nq= nav->obx[nav->nb ].nq = (int)str2num(buff, 21, 2);
				
				strncpy(satid, buff + 5, 3);
				sat =  (uint8_t)satid2no(satid);
				if (sat <= 0) continue;
				nav->obx[nav->nb].obx_data[sat - 1].sat = sat;
				sprintf(nav->obx[nav->nb].obx_data[sat - 1].prn, "%s", satid);
								
				for ( j = 0; j < nq; j++)
				{
					nav->obx[nav->nb].obx_data[sat-1].q[j] = str2num(buff, 23+j*20, 20);
				}			
			}
			nav->nb++;
...
static int readupdf_nl(const char* file, nl_upds_t* nls)
{
	...
	//nl_upds_t nl_upds0 = { {0} };
	if (!nls) return 0;
	if (!(fp = fopen(file, "r"))) {
		trace(2, "nl upd parameters file open error: %s\n", file);
		return 0;
	}
	nls->n = nls->nmax = 0; nls->data = NULL;
...
{
	nav->upds = (upds_t*)malloc(sizeof(upds_t));
...
/* satellite position by precise ephemeris -----------------------------------*/
static int pephatt(gtime_t time, int sat, const nav_t* nav, double* att,
	 double* vare)
{

	double t[NMAX + 1], p[4][NMAX + 1], c[2], * pos, std = 0.0, s[3], sinl, cosl,atts[9]={0};
	int i, j, k, index,nmax0= NMAX;

	trace(4, "pephpos : time=%s sat=%2d\n", time_str(time, 3), sat);
	//if (!att)return 0;
	//att[0] = att[1] = att[2] = 0.0;
	//for (i = 0; i < 9; i++)att[i] = 0;
	if (nav->nb < 1 + nmax0 ||
		timediff(time, nav->obx[0].time) < -MAXDTE ||
		timediff(time, nav->obx[nav->nb - 1].time) > MAXDTE) {
		trace(3, "no prec obx %s sat=%2d\n", time_str(time, 0), sat);
		return 0;
	}
	/* binary search */
	for (i = 0, j = nav->nb - 1; i < j;) {
		k = (i + j) / 2;
		if (timediff(nav->obx[k].time, time) < 0.0) i = k + 1; else j = k;
	}
	index = i <= 0 ? 0 : i - 1;

	/* polynomial interpolation for obx */
	i = index - (nmax0 + 1) / 2;
	if (i < 0) i = 0; else if (i + nmax0 >= nav->nb) i = nav->nb - nmax0 - 1;

	for (j = 0; j <= nmax0; j++) {
		t[j] = timediff(nav->obx[i + j].time, time);
		if (norm(nav->obx[i + j].obx_data[sat - 1].q, 4) <= 0.0) {
			trace(3, "prec obx outage %s sat=%2d\n", time_str(time, 0), sat);
			return 0;
		}
	}

	for (j = 0; j <= nmax0; j++) {
		pos = nav->obx[i + j].obx_data[sat - 1].q;
		p[0][j] = pos[0];
		p[1][j] = pos[1];
		p[2][j] = pos[2];
		p[3][j] = pos[3];
	...
	for (i = 0; i < 4; i++) {
		atts[i] = interppol(t, p[i], nmax0 + 1);
	}
	//ex
	att[0] = SQR(atts[0]) + SQR(atts[1]) - SQR(atts[2]) - SQR(atts[3]);
	att[1] = 2 * (atts[1] * atts[2] - atts[0] * atts[3]);
	att[2] = 2 * (atts[1] * atts[3] + atts[0] * atts[2]);
	//ey			 
	att[3] = 2 * (atts[1] * atts[2] + atts[0] * atts[3]);
	att[4] = SQR(atts[0]) - SQR(atts[1]) + SQR(atts[2]) - SQR(atts[3]);
	att[5] = 2 * (atts[2] * atts[3] + att[0] * atts[1]);
	//ez			 
	att[6] = 2 * (atts[1] * atts[3] - atts[0] * atts[2]);
	att[7] = 2 * (atts[2] * atts[3] + atts[0] * atts[1]);
	att[8] = SQR(atts[0]) - SQR(atts[1]) - SQR(atts[2]) + SQR(atts[3]);
	...
	double* dant)
{
	...
	cross3(ey, ez, ex);//the unit vector from the satellite to the sun
	...
	if (sys == SYS_GPS || sys == SYS_QZS) { /* L1-L2 */
		...
		if (0 && !strcmp(popt->ac_name, "com")) {/* B1I-B2I */
			freq[0] = FREQ1_CMP;
			freq[1] = FREQ2_CMP;
			j = 0 + 3 * NFREQ;
			k = 1 + 3 * NFREQ;
...
extern void satantoff_t(const prcopt_t* popt, gtime_t time, const double* rs, int sat, const nav_t* nav,
	double* dant, double *att)
{
	const pcv_t* pcv = nav->pcvs + sat - 1;
	double ex[3], ey[3], ez[3], es[3], r[3], rsun[3], gmst, erpv[5] = { 0 }, freq[2];
	double C1, C2, dant1, dant2;
...
{
	gtime_t time_tt;
	double rss[3], rst[3], dtss[1], dtst[1], dant[3] = { 0 }, att[9] = { 0 }, vare = 0.0, varc = 0.0, tt = 1E-3;
	int i;
	...
	if (opt) {
		if (popt->posopt[6])
		{
			if (!pephatt(time_tt, sat, nav, att, NULL)) {
				trace(2, "\nNo obx file found\n");
			}
		}			
		satantoff_t(popt, time, rss, sat, nav, dant, att);
