...
#define TRACE
#define OTYPE_CP_LP			0 //carrier phase obs
#define OTYPE_PR_LP			1 //pseudo-range  obs
	/* constants -----------------------------------------------------------------*/

#define VER_RTKLIB  "PPP AR"             /* library version */

#define PATCH_LEVEL "a0"               /* patch level */

	extern double grubbs[100];
#define COPYRIGHT_RTKLIB \
            "Copyright (C) 2007-2019 T.Takasu\nAll rights reserved."
#define PI          3.1415926535897932  /* pi */
#define D2R         (PI/180.0)          /* deg to rad */
#define R2D         (180.0/PI)          /* rad to deg *//*57.29578*/
...
#define OBSSTATE_GOOD			0     //正常
#define OBSSTATE_CS			    1     //确切周跳
#define OBSSTATE_CS_PB			2     //疑似周跳
...
#define EFACT_QZS   3.0                 /* error factor: QZSS */
#define EFACT_CMP   1.0                 /* error factor: BeiDou2 */
#define EFACT_BDS3  1.0                 /* error factor: BeiDou3 */
...
#define SYS_CMP     0x20                /* navigation system: BeiDou-2 */
#define SYS_BD3     0x40                /* navigation system: BeiDou-3 */
...
#ifndef NEXOBS
#define NEXOBS      4                   /* number of extended obs codes */
#endif
...
#define MAXEXFILE   512                /* max number of expanded files */
...
#define MAXSTRRTK   9                   /* max number of stream in RTK server */
...
#define ARMODE_PPPAR_HOLD 8

#define PAR_OFF              0x00
#define PAR_MODEL            0x01      /* AR test: model dirven*/
#define PAR_DATA_RATIO       0x02      /* AR test: data driven*/
#define PAR_DATA_FFRT        0x04
#define PAR_MODEL_DATA_RATIO 0x08
#define PAR_MODEL_DATA_FFRT  0x16

#define ROBUST_QC_PRI_OFF     0
#define ROBUST_QC_PRI_PR      1
#define ROBUST_QC_PRI_CP      2
#define ROBUST_QC_NORM_PRI_PR 3
#define ROBUST_QC_NORM_PRI_CP 4
#define ROBUST_QC_PRI_RES     5
...
#define CS_NONE     0x00                /*cycle slip: none*/
#define CS_LLI      0x01                /*cycle slip: by LLI*/
#define CS_MW       0x02                /*cycle slip: by MW*/
#define CS_GF       0x04                /*cycle slip: by GF*/
#define CS_INS      0x08                /*cycle slip: by INS*/
...
/* type definitions ----------------------------------------------------------*/
	...
	typedef struct {        /* precise clock type */
		int sat;            /* satellite number */
		char prn[8];
		double q[4]; /* Attitude quaternions */
	} obx_date_t;
	typedef struct {        /* precise clock type */
		gtime_t time;       /* time (GPST) */
		int nq;             /*四元数个数*/
		obx_date_t obx_data[MAXSAT]; /* Attitude quaternions */
	} obx_t;
	...
	typedef struct {        /* TEC grid type */
		double h;           /*Height of Ionospheric Layer*/
		int    d;           /*Spherical Harmonics Degree*/
		int    o;           /*Spherical Harmonics Order*/
		double c[16 + 1];     /* Spherical Harmonic Coefficient C */
		double s[16 + 1];     /* Spherical Harmonic Coefficient S */
		int    update;
	} coeffi_t;//SSR Ionosphere VTEC Spherical Harmonics coefficient

	typedef struct {        /* TEC grid type */
		gtime_t time;       /* epoch time (GPST) */
		int    n;           /*Number of Ionospheric Layers*/
		coeffi_t harm_coeff[16];//SSR Ionosphere VTEC Spherical Harmonics
		int    update;
	} harm_vtec_t;//SSR Ionosphere VTEC Spherical Harmonics
	...
	typedef struct {        /* SSR correction type */
		...
		int    swl[MAXCODE];  /* WL integer-indicator */
		int    sii[MAXCODE];  /* integer-indicator */
		int    sdc[MAXCODE];  /* discontinuity counter */
		...
		unsigned char ncbias; /* update flag (0:no update,1:update) */
		unsigned char npbias; /* update flag (0:no update,1:update) */
		harm_vtec_t rt_vtec;
	} ssr_t;
	...
	typedef struct {        /* navigation data type */
		...
		int nb, nbmax;       /* number of satllite obx */
		int na, namax;       /* number of almanac data */
		int nt, ntmax;       /* number of tec grid data */
		int nf, nfmax;       /* number of satellite fcb data */
		int nssr[2];       /* number of satellite ssr data *///(GREC)
	...
	typedef struct {        /* solution type */
		...
		double rx[6];       /* position/velocity (m|m/s) */
					/* {x,y,z,vx,vy,vz} or {e,n,u,ve,vn,vu} */
		double rpy[3];
		quat_t quat;
		double ba[3];
		...
		double rdcb[6];
		double ifcb[6];
		double ztrp[3];     /* tropospheric delay (m) */
		double ion[MAXSAT];     /* tropospheric delay (m) */
		double qv_ion[MAXSAT];     /* tropospheric delay (m) */
		double obs_ion[MAXSAT];     /* tropospheric delay (m) */
		double elve_ion[MAXSAT];     /* tropospheric delay (m) */
		int wl_fix_nsat[NSYS + 1];
		double dop[4];
		unsigned char type; /* type (0:xyz-ecef,1:enu-baseline) */
		unsigned char stat; /* solution status (SOLQ_???) */
		unsigned char g_stat;
		unsigned char ns;   /* number of valid satellites */
		unsigned char ns_sys[NSYS + 1];   /* number of valid satellites */
		unsigned char ar_wl_ns;
		unsigned char ar_nl_ns;
		int ar_nb;
		float age;          /* age of differential (s) */
		float ratio;        /* AR ratio factor for valiation */
		float ratio_m[NSYS + 1];        /* AR ratio factor for valiation */
		float nl_fix_m[NSYS + 1];        /* continue AR fixed epoch */
		float WL_ratio_m[NSYS + 1];        /* AR ratio factor for valiation */
		float WL_ratio;        /* AR ratio factor for valiation */
	...
	typedef struct {        /* extended receiver error model */
		...
		double cerr[4][(NFREQ + NEXOBS) * 2]; /* code errors (m) */
		double perr[4][(NFREQ + NEXOBS) * 2]; /* carrier-phase errors (m) */
		double gpsglob[(NFREQ + NEXOBS)]; /* gps-glonass h/w bias (m) */
		double gloicb[(NFREQ + NEXOBS)]; /* glonass interchannel bias (m/fn) */
	} exterr_t;

	typedef struct {        /* SNR mask type */
		int ena[2];         /* enable flag {rover,base} */
		double mask[(NFREQ + NEXOBS)][9]; /* mask (dBHz) at 5,10,...85 deg */
	} snrmask_t;
	...
	typedef struct {        /* processing options type */
		...
		double eratio[(NFREQ + NEXOBS)]; /* code/phase error ratio */
		...
		double ru[6];       /* rover position for fixed mode {x,y,z} (ecef) (m) */
		double rb[6];       /* base position for relative mode {x,y,z} (ecef) (m) */
		...
		double cs_mw;
		double cs_gf;
		int gnss_frq_idx[NSYS + 1][(NFREQ + NEXOBS)];
		int tropmap;
		int robust;
		int pos_res_qc;
		double igg_pr_k0;
	...
	//    char satantp[MAXSTRPATH]; /* satellite antenna parameters file */
		...
		int  nsite;
		char site_name[MAXFILE][MAXSTRPATH];
		char site_path[MAXFILE][MAXSTRPATH];
		...
		char obxfile[MAXSTRPATH]; /* Attitude quaternions file  */
	...
	typedef struct {        /* satellite status type */
		...
		double resp[(NFREQ + NEXOBS)]; /* residuals of pseudorange (m) */
		double resc[(NFREQ + NEXOBS)]; /* residuals of carrier-phase (m) */
		double icbias[(NFREQ + NEXOBS)];  /* glonass IC bias (cycles) */
		uint8_t vsat[(NFREQ + NEXOBS)]; /* valid satellite flag */
		uint16_t snr[(NFREQ + NEXOBS)]; /* signal strength (*SNR_UNIT dBHz) */
		uint16_t snr_rover[(NFREQ + NEXOBS)]; /* rover signal strength (0.25 dBHz) */
		uint16_t snr_base[(NFREQ + NEXOBS)]; /* base signal strength (0.25 dBHz) */
		uint8_t fix[(NFREQ + NEXOBS)]; /* ambiguity fix flag (1:float,2:fix,3:hold) */
		uint8_t slip[(NFREQ + NEXOBS)]; /* cycle-slip flag */
		uint8_t half[(NFREQ + NEXOBS)]; /* half-cycle valid flag */
		int lock[(NFREQ + NEXOBS)];   /* lock counter of phase */
		uint32_t outc[(NFREQ + NEXOBS)]; /* obs outage counter of phase */
		uint32_t slipc[(NFREQ + NEXOBS)]; /* cycle-slip counter */
		uint32_t rejc[(NFREQ + NEXOBS)]; /* reject counter */
		double eclipse;
		double  gf[(NFREQ + NEXOBS) - 1];   /* geometry-free phase L1-L2 (m) */
		double  mw[4 * 2];         /* MW-LC (m) mw,smw,mw_idx,mw_var*/
		double  phw;           /* phase windup (cycle) */
		double delta_mw[2];
		double delta_gf[2];
		double mtrp[2];         /* mtrph mtrpw*/
		double ion;         /* mtrph mtrpw*/
		double amb[(NFREQ + NEXOBS)];
		double fix_amb[(NFREQ + NEXOBS)];
		gtime_t ct;
		gtime_t pt[2][(NFREQ + NEXOBS)]; /* previous carrier-phase time */
		double  ph[2][(NFREQ + NEXOBS)]; /* previous carrier-phase observable (cycle) */
		double tec;
		double L[(NFREQ + NEXOBS)];
		double P[(NFREQ + NEXOBS)];
		double cor_L[(NFREQ + NEXOBS)];
		double cor_P[(NFREQ + NEXOBS)];
		double lam[(NFREQ + NEXOBS)];
		double norm_v[2][(NFREQ + NEXOBS)];
		double norm_pri_v[2][(NFREQ + NEXOBS)];
		double pri_res[2][(NFREQ + NEXOBS)];
		double fix_res[2][(NFREQ + NEXOBS)];
		double ambRes[2][(NFREQ + NEXOBS)];
		double ins_inno[(NFREQ + NEXOBS)][4];      /* previous,current, delta, thres, using in ins aid cycle slip detection */
		double e[3];
		int ins_repair_cs[(NFREQ + NEXOBS)];
		double cal_pr;
		double obs_phase[(NFREQ + NEXOBS)];
		double obs_phase_var[(NFREQ + NEXOBS)];
		double ins_wl_res[5];          /* previous, current, delta, variance，thres*/
		double ins_ewl_res[5];
		double ins_omc[5][2];
		double detect[(NFREQ + NEXOBS)][2];      /* previous and now omc, using in ins aid cycle slip detection */
		double var_fact[2][(NFREQ + NEXOBS)];
		int init_amb[(NFREQ + NEXOBS)];
		int new_sat;
		double  mu_deg;
		double  beta_deg;
		double amb_from_pr[(NFREQ + NEXOBS)];
	} ssat_t;
	...
	typedef struct {
		...
		double nl_res;
		double nl_var;
		int   fix_epoch[2];//sat continue fix epoch's number
		double count_fix;
	}sdamb_t;
	...
	typedef struct {
		gtime_t t;
		double raw_P[(NFREQ + NEXOBS)];
		double raw_L[(NFREQ + NEXOBS)];
		double cor_P[(NFREQ + NEXOBS)];
		double cor_L[(NFREQ + NEXOBS)];
		double LC_P[(NFREQ + NEXOBS)];
		double LC_L[(NFREQ + NEXOBS)];
		double code[(NFREQ + NEXOBS)];
		double range;
		double dtr;
		double shaprio;
		double tide;
		...
		double dts;
		double rs[3];
		double dcb[(NFREQ + NEXOBS)];
		double trp[4];
		double ion[2];
		double phw;
		double amb[(NFREQ + NEXOBS)];
		double LC_amb[(NFREQ + NEXOBS)];
	}sat_model_t;
	/*ppp_glo 来源于JXY ppprt 程序*/
	//Panda 20141208
	typedef struct
	{
		int iStResi_PR[(NFREQ + NEXOBS)];		//伪距标准化残差指标	0：正常		越大表示越严重
		int iResi[(NFREQ + NEXOBS)];			//相位残差指标			0：正常		越大表示越严重
		int iStResi[(NFREQ + NEXOBS)];			//相位标准化残差指标	0：正常		越大表示越严重
		int iMErr[(NFREQ + NEXOBS)];			//模型化误差指标	    0：正常
		int iter;
	} statIndicator_t;
	//Panda 20170707
	typedef struct
...
//NFREQ;
	typedef struct
	{
		bool   bUsed[(NFREQ + NEXOBS)];//使用判断
		bool   bGross[(NFREQ + NEXOBS)];

		double resi_0[(NFREQ + NEXOBS)]; //验前残差
		double resi_1[(NFREQ + NEXOBS)]; //验后残差

		double sig_v0[(NFREQ + NEXOBS)]; //验前方差
		double sig_vp[(NFREQ + NEXOBS)]; //验后方差

		double stResi[(NFREQ + NEXOBS)];

		double eWF_cur[(NFREQ + NEXOBS)];	 //降权因子
		double eWF_fin[(NFREQ + NEXOBS)];	 //降权因子
	} equStat_0_t;
	//Panda 20170707
	typedef struct
	{
		double ecliF;
		equStat_0_t eS_pr;//伪距残差方差信息
		equStat_0_t eS_cp;//载波残差方差信息
	} equStat_t;
	// Panda 20171107
	typedef struct
	{
	...
	typedef struct
	{
		...
		double gf_x[2];
		double gf_P[4];
		//vector<ion_base_t> vec;	// mod jxy 20171230 because vector is not suitable for memset	// jxy 20171230 kf_ion
		int nmax;
		int n;
		ion_base_t* vec;
	} kf_ion_t;
	...
	typedef struct
	{
		...
		double  eWlVar_c;
		double  gf_23;
		double  if_12;		//panda 20170501 add
		double  if_13;		//panda 20170501 add
		double  gf_pr12;	//panda 20170523 add
		int iCycle_wl;			//0		1		2
		int iCycle_gf;			//0		1		2
		int iCycle;				//0		1		2
		double  amb_from_pr[NFREQ];		//panda 20170729 add
		double  elev_max;
		double  mu_deg;
		double  beta_deg;
		double  ang_sunearsat_deg;		//Panda 20171106 add
		kf_ion_t kf_ion;
		arcInfo_t arcInf;
		cycleSlipInfo_t csInf;
	} ssat_ex_t;
	...
	typedef struct
	{
		...
		//bool   bElevLmtEx;
		//int    nEpMin_csOut;
		//int    nMaxEpLoss_csDetect;
		bool   bQc;
		//double lam[MAXSAT][NFREQ];		// jxy
		bool   bFqUsed_GPS[(NFREQ + NEXOBS)];
		bool   bFqUsed_GLO[(NFREQ + NEXOBS)];
		bool   bFqUsed_BDS[(NFREQ + NEXOBS)];
		bool   bFqUsed_GAL[(NFREQ + NEXOBS)];
		double	 coordTrue[4];		//测站真值坐标.PPP_Glo.coordtrue[3]表示真值来源(clk文件或snx文件)
		double dCsAdd[MAXSAT][(NFREQ + NEXOBS)];
		//////////////////////////////////////////////////////////////////////////
		int    nSat[8];
		int    nSat_BDS[3];
		int    iArMode;
		...
		//errCor_t eCor;
		bool bCsOut;
		int bCsEst;		// 0.NONE 1:UDUC 2.LC
		double	 clkJump[(NFREQ + NEXOBS)];
		double   obsLast[MAXSAT][6];
		double   varFac[MAXSAT];
		bool    b_unhealthy[MAXSAT];
		gtime_t gt_unh_0[MAXSAT];
		gtime_t gt_unh_9[MAXSAT];
		int    iCrdCst;
		double sigCrdCst;
		int  nOEI;
		obsEquInfo_t* oEI;
		equStat_t* equS;
		bool bQcNewCsFound;
		int   iWorstSat_Resi_C;
		int   iWorstSat_stResi[2];
		double resiAve_C;//验后相位残差ave （不包括本历元观测值残差最大值）
		double resiStd_C;//验后相位残差std （不包括本历元观测值残差最大值）
		double resiMax_C;
		double stResiAve[2];//标准化验后残差ave
		double stResiStd[2];//标准化验后残差std
		double stResiMax[2];
		// Panda 20170802
		gtime_t soltime_ppp;
		char  nmFmt[MAXCHARS];
		char  antType[MAXCHARS];
		bool bSatSysObserved[10];
		bool bUsed_sat[MAXSAT];
		int obsfreq[4][(NFREQ + NEXOBS)];	// added by jxy 每个系统使用的频率号 GREC
	} ppp_global_t;
	typedef struct
	{
		double	csThresGF;		/* slip threshold of geometry-free phase (m) */
		double	csThresMW;		/* Panda 2011-01-12 */
		bool bUsed_llCs;
		bool bUsed_gfCs;
		bool bUsed_mwCs;
		bool bUsed_resiQc;
		bool bres;	// 在滤波器中是否输出残差信息
	...
	typedef struct {        /* RTK control/result type */
		...
		double* xu, * Pu;      /*save float states and their covariance */
		double* xs, * Ps;      /* pppar wl constraint n1 states and their covariance */
		double* xa, * Pa;     /* fixed states and their covariance */
		//double *Psd;        /* Ion Psd by smooth Apsd*/
		psd_ion_t* psd;     /* Ion Psd by smooth Apsd*/
		...
		int stc;
		kf_t* ins_kf;
		int clk_jump[(NFREQ + NEXOBS)];
		int fix_epoch;
		int wl_fix_epoch[NSYS * 3];
		//int wl_fix_nsat[NSYS+1];
		...
		int ins_rerun;
		double e[MAXSAT][3];
		int refsat[NSYS][(NFREQ + NEXOBS)][2]; /* reference satellite of double-difference residuals* (0:gps/qzs/sbs,1:glo,2:gal,3:bds) */
		gtime_t filter_start;
		int exist_sys[NSYS + 1];
		double dtrr;

		ppp_global_t  ppp_glo;	// pppGlo
		ssat_ex_t    ssat_ex[MAXSAT];
		prcopt_ex_t	  opt_ex;
	} rtk_t;
	typedef struct {
		...
		double* norm_pri_pr; /* normalized prior pseudorange phase residual  */
		double* norm_pri_cp; /* normalized prior carrier phase residual  */
	}res_t;
	...
	//	double c;
	//	size_t comp_time;
	//	std::vector<std::vector<std::vector<double>>> A_L;

	//	std::vector<Eigen::MatrixXd> P;		//权阵
	//	std::vector<Eigen::MatrixXd> A;		//A阵
	//	std::vector<Eigen::MatrixXd> L;		//L阵
	//	std::vector<Eigen::MatrixXd> Q;		//L阵
	//	Eigen::MatrixXd S;		//S阵
	//	Eigen::MatrixXd N;		//N阵
	//	std::vector<Eigen::MatrixXd> W;		//W阵
	//	std::vector<Eigen::MatrixXd> X;		//X阵
	//	std::vector<Eigen::MatrixXd> V;		//V阵
	//	Eigen::MatrixXd WV;		//WV阵
	//	Eigen::MatrixXd o;		//o阵
	//	Eigen::MatrixXd dx;		//o阵
	//}Helmert_para;
	...
	EXPORT int findmin(const double* d, int n, double* min);
	...
	EXPORT void matcpy_int(int* A, const int* B, int n, int m);
	EXPORT void asi_blk_mat(double* A, int m, int n, const double* B, int p, int q,
		int isr, int isc);
	EXPORT void mat_add(const int* int_part, const double* frac_part, double* full, int n);
	EXPORT void mat_block(const double* A, int n, int m, int row_start, int rows, int col_start, int cols, double* C);

	EXPORT void matmul(const char* tr, int n, int k, int m, double alpha,
		const double* A, const double* B, double beta, double* C);
	EXPORT double detmat(const double* A, int n);
	EXPORT double matdet(double* Mat, int n);
	EXPORT void imatmul(const char* tr, int n, int k, int m, int alpha,
		const int* A, const int* B, int beta, int* C);
	EXPORT void fimatmul(const char* tr, int n, int k, int m, double alpha,
		const double* A, const int* B, double beta, double* C);
	EXPORT void ifmatmul(const char* tr, int n, int k, int m, double alpha,
		const int* A, const double* B, double beta, double* C);
	EXPORT void iimatmul(const char* tr, int n, int k, int m, double alpha,
		const int* A, const int* B, double beta, int* C);
	...
	EXPORT int readobsnav_a(const filopt_t* fopt, gtime_t ts, gtime_t te, double ti, char** infile,
		const int* index, int n, prcopt_t* prcopt,
		obs_t* obs, nav_t* nav, sta_t* sta);
	...
	EXPORT int  peph2pos(const prcopt_t* popt, gtime_t time, int sat, const nav_t* nav, int opt,
		double* rs, double* dts, double* atts, double* var);
	...
	EXPORT int  satpos(const prcopt_t* popt, gtime_t time, gtime_t teph, int sat, int ephopt,
		const nav_t* nav, double* rs, double* dts, double *atts,double* var,
		int* svh);
	EXPORT void satposs(const prcopt_t* popt, gtime_t time, const obsd_t* obs, int n, const nav_t* nav,
		int sateph, double* rs, double* dts, double* atts, double* var, int* svh);
	...
	EXPORT int readobxf(const char* file, /*obx_t* sat_obx,*/ nav_t* nav);
	...
	EXPORT uint32_t getbitu(const unsigned char* buff, int pos, int len);
	...
	EXPORT int input_gw10(raw_t* raw, uint8_t data);
	...
	EXPORT int input_cmr(raw_t* raw, uint8_t data);
	EXPORT int input_tersus(raw_t* raw, uint8_t data);
	EXPORT int input_lexr(raw_t* raw, uint8_t data);
	...
	EXPORT int outsolheads(uint8_t* buff, const solopt_t* opt, const prcopt_t* popt);
	...
	EXPORT int lambda_search(int n, int m, const double* a, const double* Q,
		double* F, double* s);
	EXPORT int parlambda(rtk_t* rtk, int n, int m, const double* a, double* Q, double* F, double* s, double* bias);
	EXPORT int plambda(rtk_t* rtk, int n, int m, const double* a, double* Q, double* F, double* s, double* ps);
	EXPORT int lambda_par(const prcopt_t* opt, rtk_t* rtk, int n, int m, const double* a, const double* Q, double* F,
		double* s, int* fixed, int* record_Z_row, int* delete_record, double* ratio_thres);

	/* observation model */
	EXPORT void matchcposb(int type, const obsd_t* obs, const nav_t* nav, int f, double* cbias, double* pbias);
	EXPORT int match_obxfile(const prcopt_t* popt, gtime_t ts, gtime_t te, const char prc_dir[], char* obxpaths[]);
...
#define FREE(pointer) {free(pointer);(pointer) = NULL;}
	...
	typedef struct {
		...
		int is_cov;            /**< if contain variance-covrainace matrix or not*/
		...
		int is_ext;            /**< if contain extension variables or not */
	...
	/* match files  --------------------------------------------------------*/
	EXPORT int match_imufile(const prcopt_t* popt, gtime_t ts, const char* prcdir, char* imupath, char* imuppath);
	EXPORT int match_lcfile(const prcopt_t* opt, gtime_t ts, const char* prcdir, char* gpspath);
	...
	EXPORT int match_snxfile(const prcopt_t* popt, gtime_t ts, gtime_t te, const char prc_dir[], char snxpath[]);
	EXPORT int match_rcvatxfile(gtime_t ts, gtime_t te, const char prc_dir[], char rcvatxPath[], const prcopt_t* popt);
	/* preprocess function  -------------------------------------------------*/
	...
	EXPORT double earth_RN(const earth_t* eth, double lat);
	EXPORT double earth_RE(const earth_t* eth, double lat);
	EXPORT double updearth(earth_t* eth, const v3_t pos, const v3_t vel, int opt, int init);

	/* attitude transformation */
	EXPORT int rv2quat(const v3_t* dtheta, quat_t* quat);
	EXPORT int rv2dcm(const v3_t* dtheta, m3_t* dcm);
	EXPORT int dcm2rv(const m3_t* dcm, v3_t* dtheta);
	EXPORT int quat2rv(const quat_t* dcm, v3_t* dtheta);
	EXPORT int euler2quat(const v3_t* euler, quat_t* quat, int opt);
	EXPORT int quat2euler(const quat_t* quat, v3_t* euler, int imu_opt);
	EXPORT int dcm2quat(const m3_t* dcm, quat_t* quat);
	EXPORT int quat2dcm(const quat_t* quat, m3_t* dcm, int opt);
	EXPORT int dcm2euler(const m3_t* dcm, v3_t* euler, int imu_opt);
	EXPORT int euler2dcm(const v3_t* euler, m3_t* dcm, int opt);
	EXPORT int att2dcm(const v3_t* att, m3_t* dcm, int opt);
	EXPORT int dcm2att(const m3_t* dcm, v3_t* att, int imu_opt);
	EXPORT int att2quat(const v3_t* att, quat_t* quat, int opt);
	EXPORT int quat2att(const quat_t* quat, v3_t* att, int opt);
	EXPORT int rotvec(const v3_t* rv, v3_t* vi);
	EXPORT int rvupdquat(const v3_t rv_ib, const v3_t rv_in, quat_t* q);
	EXPORT void quatdelphi(quat_t* q, const v3_t* phi);
	EXPORT void attsync(const v3_t* rr, v3_t* rpy, m3_t* dcm, quat_t* quat, int imu_opt, int local_opt, int mech_opt, int att_fmt);

	/* 3D vector operation */
	EXPORT int asymmetric_mat(const v3_t* v3, m3_t* mat);
	EXPORT m3_t v3_askew(v3_t v3);
	EXPORT v3_t v3_cross(v3_t v1, v3_t v2);
	EXPORT v3_t v3_add(v3_t v1, v3_t v2);
	EXPORT v3_t v3_del(v3_t v1, v3_t v2);
	EXPORT v3_t v3_scalar(double s, v3_t v);
	EXPORT double v3_norm(v3_t v3);
	EXPORT int v3_normalize(v3_t* v3);
	EXPORT double v3_mul_rxc(v3_t v1, v3_t v2); /* row vector x column vector */
	EXPORT m3_t v3_mul_cxr(v3_t v1, v3_t v2);   /* column vector x row vector */
	EXPORT v3_t v3_mul_cxc(v3_t v1, v3_t v2);   /* multiply corresponding elements */
	EXPORT m3_t v3_diag(v3_t diag);
	EXPORT v3_t v3_pow(v3_t v, double order);
	EXPORT v3_t v3_sqrt(v3_t v);
	EXPORT int v3_equal(const v3_t* v1, const v3_t* v2, double eps);
	EXPORT v3_t v3_sum(const v3_t* v3_list, int n);
	EXPORT v3_t v3_mean(const v3_t* v3_list, int n);
	EXPORT v3_t v3_rms(const v3_t* v3_list, int n);
	EXPORT v3_t v3_std(const v3_t* v3_list, int n);
	EXPORT void v3_2_array(const v3_t v, double* a);
	/* 3D matrix operator */
	EXPORT v3_t m3_iaskew(m3_t m3);
	EXPORT m3_t m3_T(m3_t A);
	EXPORT int m3_inv(m3_t* A);
	EXPORT double m3_det(const m3_t* A);
	EXPORT m3_t m3_add(m3_t A, m3_t B);
	EXPORT m3_t m3_del(m3_t A, m3_t B);
	EXPORT m3_t m3_scalar(double alpha, m3_t A);
	EXPORT m3_t m3_mul(m3_t A, m3_t B);
	EXPORT v3_t m3_mul_v3(m3_t A, v3_t B);
	EXPORT v3_t m3_diag(m3_t diag);
	EXPORT m3_t m3_diag_add(m3_t A, double a);
	EXPORT m3_t m3_pow(m3_t A, double order);
	EXPORT int m3_equal(const m3_t* A, const m3_t* B, double eps);
	EXPORT void m3_swap_row(m3_t* A, int r1, int r2);
	EXPORT void m3_swap_clm(m3_t* A, int c1, int c2);
	EXPORT int m3_SVD(const m3_t* A, m3_t* U, v3_t* D, m3_t* V);
	EXPORT int m3_LU(const m3_t* A, m3_t* L, m3_t* U, m3_t* P);
	EXPORT void m3_diag_v3(m3_t m3, v3_t* v3);
	extern const m3_t I33;
	extern const m3_t O33;
	extern const v3_t V0;
	extern const v3_t V1;

	/* quaternion operation */
	EXPORT int quat_normalize(quat_t* quat);
	EXPORT int quat_conj(quat_t* quat);
	EXPORT int quat_inv(quat_t* quat);
	EXPORT double quat_dot(quat_t P, quat_t Q);
	EXPORT v3_t quat_cross(quat_t P, quat_t Q);
	EXPORT double quat_norm(quat_t P);
	EXPORT quat_t quat_mul(quat_t P, quat_t Q);
	EXPORT v3_t quat_mul_v3(quat_t quat, v3_t vec);
	EXPORT int quat_equal(const quat_t* P, const quat_t* Q, double eps);
	EXPORT quat_t qaddphi(const quat_t* quat, v3_t phi);

	/* so3 operation */
	EXPORT void so3_log(const double* C, double* omega, double* theta);
	EXPORT void so3log(const double* C, double* phi);

	/* Attitude add and del operation */
	EXPORT v3_t euler_add(v3_t Eab, v3_t Ebc, int opt);
	EXPORT v3_t euler_del(v3_t Eab, v3_t Eac, int opt);
	EXPORT double yaw_del(double yaw1, double yaw2);
	EXPORT int euler_addphi(v3_t* Eab, const v3_t* phi_bc, int opt);
	EXPORT int euler_delphi(v3_t* Eab, const v3_t* phi_bc, int opt);

	/* coordinate transformation */
	EXPORT m3_t formCen_llh(double lat, double lon, int opt);
	EXPORT m3_t att2Cbe(const v3_t* pos, const v3_t* Enb, int imu_coord_opt, int local_opt);
	EXPORT quat_t att2Qbe(const v3_t* pos, const v3_t* Enb, int imu_coord_opt, int local_opt);
	EXPORT v3_t att2Ebe(const v3_t* pos, const v3_t* Enb, int imu_coord_opt, int local_opt);
	EXPORT v3_t Cbe2att(const v3_t* xyz, const m3_t* Cbe, int imu_opt, int local_opt);
	EXPORT v3_t Qbe2att(const v3_t* xyz, const quat_t* Qbe, int imu_opt, int local_opt);
	EXPORT v3_t Ebe2att(const v3_t* xyz, const v3_t* Ebe, int imu_opt, int local_opt);
	//EXPORT int is_blh(const v3_t *pos);

	/* Gravity model*/
	EXPORT int gravity_ecef(const v3_t* r, v3_t* ge);
	EXPORT int gravity_ned(double lat, double hgt, v3_t* gn);

	/* INS detect static imu measurement */
	EXPORT int detstatic_MEAS(const insopt_t* opt, const imud_t* imus, int ws, const double* pos);
	EXPORT int detstatic_GLRT(const imud_t* imu, int n, const insopt_t* opt, const double* pos, double dt, double* z);
	EXPORT int detstatic_MV(const imud_t* imu, int n, const insopt_t* opt, double dt, double* z);
	EXPORT int detstatic_MAG(const imud_t* imu, int n, const insopt_t* opt, const double* pos, double dt, double* z);
	EXPORT int detstatic_ARE(const imud_t* imu, int n, const insopt_t* opt, double dt, double* z);
	EXPORT int detimustatic(const insopt_t* opt, const imud_t* data, const v3_t pos, double dt);
	EXPORT int detimustatic_test(const insopt_t* opt, const imud_t* data, const double* pos, double dt, int det, int ws);
	EXPORT int test_zero_threshold(kf_t* inskf, FILE* fp_sol, const insopt_t* opt, imud_t* imuz);  //

	/* INS Align */
	EXPORT int checkpva(const pva_t* pva, int idx);
	EXPORT int align_coarse_static_base(const imu_t* imu, double lat, m3_t* Cnb);
	EXPORT int dblvec2att(const v3_t* vn1, const v3_t* vn2, const v3_t* vb1, const v3_t* vb2, m3_t* Cnb);
	EXPORT int align_coarse_inertial(const imu_t* imu, double lat, m3_t* Cnb);
	EXPORT int align_coarse_wuhba(const imu_t* imu, double lat, const v3_t* veb_n,
		unsigned int Nveb_n, m3_t* Cnb);
	EXPORT int ins_align(const prcopt_t* popt, const obs_t* obs, const nav_t* nav, const pva_t* pva, imu_t* imus, int* imu_idx,
		int* rover_idx, int* base_idx, int* pva_idx, rtk_t* rtk);

	/* INS navigation */
	EXPORT int multisample(const v3_t* dtheta_list, const v3_t* dv_list, int N, v3_t* dtheta, v3_t* dv);
	EXPORT int ins_nav_ecef(double dt, const v3_t* dtheta, const v3_t* dv, v3_t* r, v3_t* v, quat_t* q, int opt);
	EXPORT int ins_nav_llh(double dt, const v3_t* dtheta, const v3_t* dv, solins_t* sol, int opt);
	EXPORT int ins_nav_ecef_back(double dt, const v3_t* dtheta, const v3_t* dv, v3_t* r, v3_t* v, quat_t* q, int opt);
	EXPORT int ins_nav_llh_back(double dt, const v3_t* dtheta, const v3_t* dv, solins_t* sol, int opt);
	//EXPORT int dr_nav_ecef(double dt, const v3_t* dtheta, double dS, v3_t *r, quat_t *q);

	/* utilities */
	EXPORT double ins_randn(void);
	EXPORT v3_t v3_randn(double mean, double sigma);
	EXPORT void v3_paste(double* dest, const v3_t* v3);
	EXPORT void m3_paste(double* dest, int n, const m3_t* m3);
	EXPORT void v3_copy(v3_t* v3_dest, const double* src);
	EXPORT void m3_copy(m3_t* m3_dest, const double* src, int n);
	EXPORT void imu_orientation_adjust(imud_t* imud, const char* or1, const char* or2);
	EXPORT unsigned int soltype_add(unsigned int status, unsigned int SOL_TYPE);
	EXPORT unsigned int soltype_remove(unsigned int status, unsigned int SOL_TYPE);
	EXPORT int is_soltype(unsigned int status, unsigned int SOL_TYPE);
	EXPORT double angle_to180(double angle);
	EXPORT double angle_to360(double angle);
	EXPORT double markov_std2rw(double std, double T);
	EXPORT double markov_rw2std(double rw, double T);

	EXPORT int ins_readf(const char* fname, int ft, imu_t* imu, pva_t* pva, od_t* od, const insopt_t* opt);
	EXPORT int inputimu(int ws, int* imu_idx, const imu_t* imus, imud_t* imud, imud_t* imuz);
	EXPORT int readimu_m39(const char* file, imu_t* imu, FILE* fp);
	EXPORT int matchgnsssol(gtime_t imu_time, int* sol_idx, const pva_t* pva, double dt, int mode);
	EXPORT int matchgnsssol1(gtime_t imu_time1, gtime_t imu_time2, int* sol_idx, const pva_t* pva, double dt, int mode);
	EXPORT int matchgnssobs(gtime_t imu_time, int* obs_idx, const obs_t* obs, double dt);
	EXPORT int imu_init(imu_t* imu);
	EXPORT void imu_free(imu_t* imu);
	EXPORT int imu_add(imu_t* imu, const imud_t* data);
	EXPORT void od_init(od_t* od);
	EXPORT void od_add(od_t* od, const gtime_t* time, const double* dS);
	EXPORT void od_free(od_t* od);
	EXPORT void pva_init(pva_t* pva, int n);
	EXPORT void pva_resize(pva_t* pva, unsigned int n);
	EXPORT void pva_free(pva_t* pva);

	/* kalman filter operation for ins-gnss fusion */
	...
	EXPORT int xnArmGps(const insopt_t* opt);
	EXPORT int xnDt(const insopt_t* opt);
	EXPORT int xnKod(const insopt_t* opt);
	EXPORT int xnFrq(const insopt_t* opt);
	EXPORT int xnClk(const insopt_t* opt);
	EXPORT int xnRr(const insopt_t* opt);
	EXPORT int xnTrp(const insopt_t* opt);
	EXPORT int xnIon(const insopt_t* opt);
	EXPORT int xnAmb(const insopt_t* opt);
	EXPORT int xnIns(const insopt_t* opt);
	EXPORT int xnGnss(const insopt_t* opt);
	EXPORT int xnR(const insopt_t* opt);
	EXPORT int xnX(const insopt_t* opt);
	EXPORT int xnClp(const insopt_t* opt);
	...
	EXPORT int xiArmGps(const insopt_t* opt);
	EXPORT int xiDt(const insopt_t* opt);
	EXPORT int xiKod(const insopt_t* opt);
	EXPORT int xiClk(const insopt_t* opt);
	EXPORT int xiTrp(const insopt_t* opt);
	EXPORT int xiIon(const insopt_t* opt, int sat);
	EXPORT int xiRr(const insopt_t* opt);
	EXPORT int xiAmb(const insopt_t* opt, int sat, int f);

	/* jacobi matrix */
	EXPORT int jacobi_trans_a2a_ECEF(m3_t* F, double dt);
	EXPORT int jacobi_trans_a2bg_ECEF(m3_t* F, double dt, const m3_t* Cbe);
	EXPORT int jacobi_trans_a2sg_ECEF(m3_t* F, const m3_t* Cbe, const v3_t* dtheta);
	EXPORT int jacobi_trans_v2a_ECEF(m3_t* F, const m3_t* Cbe, const v3_t* dtheta);
	EXPORT int jacobi_trans_v2v_ECEF(m3_t* F, double dt);
	EXPORT int jacobi_trans_v2p_ECEF(m3_t* F, double dt, const v3_t* reb_e, int opt);
	EXPORT int jacobi_trans_v2ba_ECEF(m3_t* F, double dt, const m3_t* Cbe);
	EXPORT int jacobi_trans_v2sa_ECEF(m3_t* F, const m3_t* Cbe, const v3_t* dv);
	EXPORT int jacobi_trans_p2v_ECEF(m3_t* F, double dt);

	EXPORT void jacobi_trans_p2p_NED(m3_t* F, solins_t* sol, double dt);
	EXPORT void jacobi_trans_p2v_NED(m3_t* F, double dt);
	EXPORT void jacobi_trans_v2p_NED(m3_t* F, solins_t* sol, double dt);
	EXPORT void jacobi_trans_v2v_NED(m3_t* F, solins_t* sol, double dt);
	EXPORT void jacobi_trans_v2a_NED(m3_t* F, const m3_t* Cbn, const v3_t* dv);
	EXPORT void jacobi_trans_a2p_NED(m3_t* F, solins_t* sol, double dt);
	EXPORT void jacobi_trans_a2v_NED(m3_t* F, solins_t* sol, double dt);
	EXPORT void jacobi_trans_v2ba_NED(m3_t* F, const m3_t* Cbn, double dt);
	EXPORT void jacobi_trans_a2bg_NED(m3_t* F, const m3_t* Cbn, double dt);
	EXPORT void jacobi_trans_v2sa_NED(m3_t* F, const m3_t* Cbn, const v3_t* dv);
	EXPORT void jacobi_trans_a2sg_NED(m3_t* F, const m3_t* Cbn, const v3_t* dtheta);

	EXPORT int jacobi_trans_markov(m3_t* F, double dt, const v3_t* T);
	EXPORT int jacobi_lc_meas_p2a_ECEF(m3_t* H, const m3_t* Cbe, const v3_t* lever_arm);
	EXPORT int jacobi_lc_meas_v2a_ECEF(m3_t* H, const m3_t* Cbe, const v3_t* wib_b, const v3_t* lever_arm);
	EXPORT int jacobi_lc_meas_v2bg_ECEF(m3_t* H, const m3_t* Cbe, const v3_t* lever_arm);
	EXPORT int jacobi_tc_meas2a_ECEF(v3_t* dpda, const m3_t* Cbe, const v3_t* lever_arm, double* ei, double* ej);
	...
	EXPORT void etm_ECEF(kf_t* inskf, const imup_t* imup, const insopt_t* opt, double dt, const v3_t dv, const v3_t dtheta);
	EXPORT void etm_LLH_NED(kf_t* inskf, const imup_t* imup, const insopt_t* opt, double dt, const v3_t dv, const v3_t dtheta);
	EXPORT void etm_LLH_ENU(kf_t* inskf, const imup_t* imup, const insopt_t* opt, double dt, const v3_t dv, const v3_t dtheta);

	EXPORT int vel2yaw(const v3_t* veb_n, double* yaw, const m3_t* Qveb_n, double* Qyaw);
	EXPORT void pos2vel(const v3_t* pos1, const v3_t* pos2, v3_t* v, double dt);
	EXPORT int gnsspva2ins(const v3_t re, const v3_t ve, const imud_t* imu_data, imup_t* imup, int imu_coord_opt, int local_opt);

	EXPORT void ins2gnss(const solins_t* ins_sol, const v3_t* arm, const v3_t* wib_b, double* gr, double* gv, int mech_coord);
	EXPORT void gnss2ins(const v3_t* gr, const v3_t* gv, const v3_t* arm, const v3_t* wib_b, solins_t* ins_sol);
	EXPORT int ins_use_rtk_init(rtk_t* rtk, prcopt_t* popt, const imup_t* imup);
	EXPORT int ins_use_rtk(const prcopt_t* popt, imup_t* imup, const imu_t* imu, const int kimu_idx, int* rover_idx, int* base_idx,
		const obs_t* obs, const nav_t* nav, rtk_t* rtk);
	EXPORT int inskf_init(kf_t* inskf, const imup_t* imup, insopt_t* opt);
	EXPORT int inskf_free(kf_t* inskf, insopt_t* opt);
	EXPORT int inskf_udstate(kf_t* inskf, const imu_t* imus, int* imu_idx, const imup_t* imup, const insopt_t* opt);
	EXPORT int inskf_NHC(kf_t* inskf, const v3_t* wib_b, const insopt_t* opt);
	EXPORT int inskf_ZST_pos(kf_t* inskf, const solins_t* last_sol);
	EXPORT int inskf_ZST_imu(kf_t* inskf, double mean_dv, double std_dv, const insopt_t* opt);
	EXPORT int inskf_ZARU(kf_t* inskf, const insopt_t* opt);
	EXPORT int inskf_ZUPT(kf_t* inskf, const insopt_t* opt);
	EXPORT int inskf_ZAU(kf_t* inskf, double yaw, double Qyaw, const insopt_t* opt);
	EXPORT int inskf_udmeas_yaw(kf_t* inskf, double yaw, double Qyaw, const insopt_t* opt);
	EXPORT int inskf_constraint_yaw(kf_t* inskf, const v3_t* wib_b, const imup_t* imup, const insopt_t* opt);
	EXPORT int inskf_feedback(gtime_t t, unsigned int SOL_TYPE, const insopt_t* opt, double* x, solins_t* sol_ins);
	EXPORT int inskf_norm_innov(kf_t* inskf, const double* dz, const double* R, double* ndz);
	EXPORT int inskf_close(kf_t* inskf);

	EXPORT int lcfilter(const prcopt_t* popt, int kpva_idx, const pva_t* pva, const imu_t* imu, int kimu_idx, kf_t* inskf);

	EXPORT int kf_formQ(double* Q, const m3_t* Cbe, double dt_zoom, const imup_t* imup, const insopt_t* opt);
	EXPORT void inskf_proP(double* x, double* P, kf_t* inskf, int all_nx, int upd_nx);

	/*rts ------------------------------------------*/
	EXPORT void rts_save_forward(FILE* fp, const solins_t* sol, const insopt_t* opt);
	EXPORT int rts_ins_gnss(const prcopt_t* popt, const filopt_t* fopt, const solopt_t* solopt, rtk_t* ins_rtk, pva_t* pva,
		const imu_t* imu, int* id, const obs_t* obss, const nav_t* navs);

	EXPORT void sol2pva(const insopt_t* opt, int pva_idx, sol_t* sol, pva_t* pva);
	EXPORT void pva2sol(const pva_t* pva, sol_t* sol, int pva_idx);
	EXPORT void sol2solins(const sol_t* sol, solins_t* solins);
	EXPORT void solins2sol(const insopt_t* opt, const solins_t* solins, sol_t* sol, int att_fmt);
	EXPORT int outsolins(FILE* fp, const solins_t* sol, const imup_t* imup, const insopt_t* insopt, solopt_t* solopt);
	EXPORT void kf_copyQ2sol(kf_t* inskf, const insopt_t* opt);
	EXPORT int matchgnssinfo(const prcopt_t* popt, kf_t* inskf, int* kpva_idx, pva_t* pva, int* krover_idx, int* kbase_idx,
	...
	EXPORT int insaidcs(rtk_t* rtk, const obsd_t* obs, int n, const int* ir, const int* iu);
	EXPORT int insaid_detslip1(rtk_t* rtk, const obsd_t* obs, int n, const int* ir, const int* iu);
	EXPORT int insaid_detslip_ppk(rtk_t* rtk, const int* sat_no, int ns);
	EXPORT int insaid_detslip(rtk_t* rtk, const prcopt_t* opt, const obsd_t* obs, const nav_t* nav, int* prn, double* x, double* P, int nv, int nu);
	EXPORT int insaid_repair(rtk_t* rtk, double v, int idamb, double freq);

	EXPORT int prn_inlist(int prn, int* prnlist, int ns);
	...
	EXPORT int pppar_main(int argc, char **argv);
	//EXPORT int pppar_main(/*int argc, char **argv*/);
	EXPORT int rtknavi_main();
	void TimerTimer(char** in_str);
	EXPORT int readantex(const char* file, pcvs_t* pcvs);
	EXPORT int process(prcopt_t* popt, filopt_t* fopt, solopt_t* sopt);
	EXPORT bool qualityControl_v1(rtk_t* rtk, const nav_t* navs, const int iter, const  int* v_flag, const int nv, int* exc);
	EXPORT int filter0(rtk_t* rtk, double* x, double* P, const double* H, const double* v,
		const double* R, int n, int m, int qc, const int kf_type, res_t* res, int tc);
	EXPORT double calStd_ex(const double* v, const int& n, bool* bused, double* ave, double* rms);
	static int getHVR_spp(const int iter, bool bElevCVG, int* sat, double* H, double* v, double* var,
		double* elev, int& nv, int& nx);
	extern void corr_meas0(rtk_t* rtk, const obsd_t* obs, const nav_t* nav, const double* azel,
	...
	//EXPORT void detslp_ewl(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav);
	...
	extern void outsol_ion(FILE* fp, sol_t* sol, const double* rb,
		const solopt_t* opt, const prcopt_t* popt, const solins_t* ins_sol);
	...
	extern int filter_nlwl(double* x, double* P, const double* H, const double* v,
